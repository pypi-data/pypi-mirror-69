...
# Basically useless but I also don't want to make it to easy
...
def authorized():
    def decorator(f):
        ...
        async def decorated_function(request, *args, **kwargs):
            is_authorized = request.args.get("key") == SecretKey or request.headers.get("Authorization") == SecretKey

            if is_authorized:
                return await f(request, *args, **kwargs)
            else:
                return sanic.response.json({'status': 'Unauthorized'}, 401)
