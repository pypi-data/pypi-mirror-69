...
def _inventory_from_headers(headers):
    ...
    inventory["exposureTime"] = _get_number_apply(headers['FPA_EXPO'], mode).mode[0]
    ...
    inventory["primaryProposalId"] = _get_unique(headers['PROPID'], singular=True)
    inventory["primaryExperimentId"] = _get_unique(headers['EXPERID'], singular=True)
