...
def spatial_model_from_header(header):
    ...
    return spatial_model_from_quantity(
...
class TransformBuilder:
    ...
    def make_temporal(self):
        ...
        self._transforms.append(
            time_model_from_date_obs(
                [e["DATE-OBS"] for e in self.slice_headers], self.header["DATE-BGN"]
            )
        )
    ...
    def make_spatial(self):
        ...
        name = name.split(" ")[0]
        axes_names = [
            (self.header[f"DWNAME{nn}"].rsplit(" ")[1]) for nn in (self.n, self._n(i + 1))
        ]
    ...
    def make_spectral_from_wcs(self):
        ...
        """
        return linear_spectral_model(
            self.header[f"CDELT{self.n}"] * u.nm, self.header[f"CRVAL{self.n}"] * u.nm
        )
