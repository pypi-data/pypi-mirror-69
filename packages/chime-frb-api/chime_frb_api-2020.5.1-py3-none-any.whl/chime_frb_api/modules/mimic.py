...
class Mimic(object):
    ...
    def register_injection(self, **kwargs) -> JSON:
        ...
        """
        return self.API.post(url="/v1/mimic/injection", json=kwargs)
    ...
    def register_detection(self, **kwargs) -> JSON:
        ...
        -------
        """
        return self.API.post(url="/v1/mimic/detection", json=kwargs)
    ...
    def get_simulated_event(self, uuid: str = None) -> JSON:
        ...
        """
        if uuid:
            return self.API.get(url="/v1/mimic/{}".format(uuid))
        else:
