...
class Algorithms:
    ...
    def __init__(self, verbose=False):
        ...
        to stdout, unless the per-function verbose flag is set to False.
        """
    ...
    def clip(self,
        ...
        self._verbose = self.verbose and verbose
        self._vprint(f"{_elapsed(t0)} - clipping from {_minmax(frame)} to [{lo:.2f}, {hi:.2f}]")
        return frame_out
    ...
    def linearize(self,
        ...
        if (blacklevel, whitelevel) != (0, 1):
            frame = self.clip(frame, blacklevel, whitelevel, verbose)
            t0 = time.time()
            frame -= blacklevel
    ...
    def ccm(self,
        ...
        if matrix is not None:
            frame = self.clip(frame, 0, 1, verbose=False)
            t0 = time.time()
    ...
    def tonemap(self,
        ...
        if mode is not None:
            if mode == "Reinhard":
                frame = self.clip(frame, 0, np.inf, verbose)
                t0 = time.time()
    ...
    def chroma_denoise(self,
        ...
        if strength > 0:
            ...
            frame = self.clip(frame * maxval + 0.5, 0, maxval, verbose)
            t0 = time.time()
    ...
    def gamma(self,
        ...
        if mode is not None:
            realmode = mode
            frame = self.clip(frame, 0, 1, verbose)  # can't handle values outside of [0, 1]
            t0 = time.time()
            ...
            if mode == "LUT":
                ...
                maxval = len(lut) - 1
                frame = self.quantize(frame, maxval, verbose=False)  # [0, 1] ==> [0, maxval]
    ...
    def quantize(self,
        ...
        """
        t0 = time.time()
        frame = self.clip(frame * maxval + 0.5, 0, maxval, verbose=False)
        frame = frame.astype(dtype)
        self._verbose = self.verbose and verbose
...
def _minmax(frame: np.ndarray) -> str:
