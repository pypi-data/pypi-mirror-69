...
namespace pyxie
{
	...
	static PyObject *camera_Render(camera_obj *self, PyObject *args, PyObject* kwargs)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|OiiOi", kwlist, &sco, &tex, &clearColor, &clearDepth, &clearColorValue)) return NULL;
	...
	static PyObject* camera_RenderShadow(camera_obj* self, PyObject* args, PyObject* kwargs)
	{
		static char* kwlist[] = { "showcase","texture","viewCamera", NULL };
		showcase_obj* sco;
		texture_obj* tex = nullptr;
		camera_obj* viewCam = nullptr;
		PyObject* clearColorValue = nullptr;
		if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOO", kwlist, &sco, &tex, &viewCam)) return NULL;

		if (sco && sco->ob_base.ob_type != &ShowcaseType) {
			PyErr_SetString(PyExc_TypeError, "1st parameter need showcase object.");
			return NULL;
		}
		if (tex && (tex->ob_base.ob_type != &TextureType)) {
			PyErr_SetString(PyExc_TypeError, "2st parameter need texture object.");
			return NULL;
		}
		if (viewCam && (viewCam->ob_base.ob_type != &CameraType)) {
			PyErr_SetString(PyExc_TypeError, "3st parameter need camera object.");
			return NULL;
		}
		...
		if (!renderContext) return NULL;
		{
			PyxieZoneScopedN("BeginScene");
			auto offscreen = tex ? tex->renderTarget : nullptr;
			renderContext->BeginScene(offscreen, Vec4(1,1,1,1), true, true);
		}
		{
			PyxieZoneScopedN("showcas Update");
			sco->showcase->Update(0.0f);
		...
		{
			PyxieZoneScopedN("camera Render");
			self->camera->RenderForShadow(viewCam->camera);
		}
		{
			PyxieZoneScopedN("showcas Render");
			sco->showcase->Render(1);
		}
		{
			PyxieZoneScopedN("EndScene");
			renderContext->EndScene();
	...
	PyObject* camera_getPosition(camera_obj* self)
	{
		...
		vmath_cpy(self->camera->GetCameraPosition().P(), 3, v3robj->v);
		v3robj->d = 3;
	...
	PyObject* camera_getRotation(camera_obj* self)
	{
		...
		vmath_cpy(self->camera->GetCameraRotation().P(), 4, quatobj->v);
		quatobj->d = 4;
	...
	PyMethodDef camera_methods[] = {
		...
		{ "shadowShoot", (PyCFunction)camera_RenderShadow, METH_VARARGS | METH_KEYWORDS, shoot_doc },
		{ NULL,	NULL }
	};
