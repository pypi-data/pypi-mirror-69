...
class PYXIE_EXPORT pyxieParticle : public pyxieFigure
{
	...
	public:
		pyxieParticle(b2Vec2* posBuff = NULL, b2Color* colorBuff = NULL, int count = 0, float size = -1, float scale = 1.0);
		~pyxieParticle();
		...
		inline static pyxieParticle *new_particle(b2Vec2 *posBuff = NULL, b2Color *colorBuff = NULL, int count = 0, float size = -1, float scale = 1.0)
		{
			return new pyxieParticle(posBuff, colorBuff, count, size, scale);
		}
