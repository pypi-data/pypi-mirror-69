...
namespace pyxie {
		...
		struct _ShaderDesc
		{
			...
			union {
				struct {
					...
					uint32_t DISCARD_COLOR_MAP_RGB : 1;				//カラーテクスチャのRGBチャネルが不要な場合
					uint32_t UNIFIED_COLOR : 1;						//DiffuseColorのグローバルパラメータ版(有効にするとDiffuseColorがなくなる)
					uint32_t USE_LIGHT_VIEW_PROJ : 1;				//Shadow用ViewProjectionを使う
				};
	...
	public:
		...
		void SetMakeShadow(bool enable);
		void SetUnifiedColor(bool enable);
		void SetLightViewProj(bool enable);


		inline bool IsMakeShadow() { return desc.MAKE_SHADOW_ENABLE; }
