...
class Estimate():
    ...
    def keras_rekfold(self, build_model, num_epochs, batch_size, k=5, n=3, verbose=0, shuffle=True, param=None, random_state=None):
        '''
        ...
        Repeated k-fold verification for the keras model and other similar models
...
class Search():
    ...
    def gridsearch(self, build_model, num_epochs, batch_size, param, k=5, n=1, select_method=('loss', 'min'), verbose=0, shuffle=True, random_state=None):
        '''
        参数
        ----
...
class Error():
    ...
    参数
    ----
    model：一个类，该类必须要有predict函数
    
    属性
    ----
    error_mat：一维ndarray数组，误差绝对值矩阵
    error_list：误差绝对值从大到小的排序
    mae：平均绝对误差
    mse：平均平方误差
    sae：绝对误差和
    sse：残差平方和
    ...
    Parameter
    ---------
    model: class, this class should have 'predict' function
    
    Atrributes
    ----------
    error_mat: one-dimension ndarray, absolute error matrix
    error_list: list of absolute values of errors sorted from large to small
    ...
    def fit(self, data, target):
        ...
        predict_target = self.__model.predict(data)
        self.error_mat = abs(target - predict_target)
        
        mat_index = np.arange(num_data)
        self.error_list = sorted(list(zip(self.error_mat, mat_index)), reverse=True)
    
        self.sse = sum(self.error_mat**2)
        self.sae = sum(self.error_mat)
        
        self.mse = self.sse / num_data
        self.mae = self.sae / num_data
