...
def get_store(ctx, token=False):
    ...
    if token:
        creds_file = TOKENS_FILENAME.format(context.get_client_id(ctx))
    else:
    ...
    pathlib.Path(os.path.dirname(store_file)).mkdir(parents=True, exist_ok=True)
...
def get_credentials(ctx, refresh=None):
    ...
    agilicus_scopes = context.get_scopes(ctx)
    ...
    if not crd:
        ...
        flags.noauth_local_webserver = not context.get_auth_local_webserver(ctx)
