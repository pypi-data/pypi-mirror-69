...
class PrestoTest(common.TestBase):
    ...
    def test_external_view(self):
        ...
        caught = None
        expected = "'some_test_user' does not have privileges to access: okera_system.*"
        with ctx.connect(host='localhost', port=12050,
                         presto_port=DEFAULT_PRESTO_PORT) as conn:
            try:
                results = conn.scan_as_json("select * from default.external_view")
    ...
    def test_no_timezone(self):
        ctx = context(dialect='presto', tz=None)
        ctx.enable_token_auth(token_str='root')
        with ctx.connect(host='localhost', port=12050,
                        presto_port=DEFAULT_PRESTO_PORT) as conn:
            results = conn.scan_as_json('select current_timezone() as tz')
    ...
    def test_specific_timezones(self):
        ...
        for tz in tzs:
            tz_name = tz[0]
            tz_offset = tz[1]
            ctx = context(dialect='presto', tz=pytz.timezone(tz_name))
            ctx.enable_token_auth(token_str='root')
            with ctx.connect(host='localhost', port=12050,
                            presto_port=DEFAULT_PRESTO_PORT) as conn:
                results = conn.scan_as_json('select current_timezone() as tz')
    ...
    def test_sample_and_limit_setting_via_session_props(self):
        ...
        rows = cursor.fetchall()
        self.assertEqual(len(rows), with_limit_and_sampling_row_length)
    ...
    def test_transform(self):
        ...
        user = 'analyst'
        sql = "SELECT * FROM recordservice.abac_db.user_account_data " +\
    ...
    def test_cte_view(self):
        ...
        user = 'analyst'
        sql = "select * from recordservice.chase.cte_view " +\
        ...
        with ctx.connect(host='localhost', port=12050,
            ...
            for db in dbs:
                ...
                for dataset in datasets:
                    ...
                    self.compare_json(presto_view_results, okera_ds_results, False,
                                            empty_struct_equals_null=False,
                                            batch_mode=False, required_type=None)
                    self.compare_json(presto_view_results, presto_ds_results, False,
                                            empty_struct_equals_null=False,
                                            batch_mode=False, required_type=None)
