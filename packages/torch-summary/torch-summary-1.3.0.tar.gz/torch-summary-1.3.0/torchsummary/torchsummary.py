...
def summary(model, input_data, *args, batch_dim = 0, branching = True, col_names = ("output_size", "num_params"), col_width = 25, depth = 3, device = None, dtypes = None, verbose = 1, **kwargs):
    """
    Summarize the given PyTorch model. Summarized information includes:
        1) output shape, 2) kernel shape, 3) number of the parameters
    ...
    Arguments:
        ...
        input_data (Sequence of Sizes or Tensors):
            ...
            - OR -
            Shape of input data as a List/Tuple/torch.Size (dtypes must match model input, default is FloatTensors).
    ...
    if isinstance(input_data, torch.Tensor):
        ...
        raise TypeError("Input type is not recognized. Please ensure input_data is valid.\n"
    ...
    try:
        ...
        print("Failed to run torchsummary, printing sizes of executed layers: {}".format(summary_list))
        raise
    finally:
        for hook in hooks:
            hook.remove()
...
def apply_hooks(module, orig_model, depth, summary_list, hooks, idx, batch_dim, curr_depth = 0):
    ...
    if curr_depth <= depth:
        for child in module.children():
            apply_hooks(child, orig_model, depth, summary_list, hooks, idx, batch_dim, curr_depth + 1
            )
