...
class DeepCrawlReport(ImmutableAttributesMixin):
    ...
    def load_report_rows(self, connection=None, filters=None):
    ...
    def get_report_rows(self, use_cache=True, connection=None, filters=None):
        connection = connection or deepcrawl.DeepCrawlConnection.get_instance()
        if self.report_rows and use_cache:
            return self.report_rows
        return self.load_report_rows(connection=connection, filters=filters)
