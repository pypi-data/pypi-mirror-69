...
class SpaceFighter:
    ...
    def makeMove(self, move, playerID=None):
        ...
        if "target" in move:
            ...
            if aiID != self.flippedAI:
                target[1] = max(target[1], self.state["ships"]['pos'][1] + shipDim_h + bulletDim_h + 1)
            else:
                target[1] = min(target[1], self.state["ships"]['pos'][1] + shipDim_h + bulletDim_h + 1)
    ...
    def tick(self):
        ...
        for key, object in self.state["ships"].items():
            ...
            if object["reload"] <= 1:
                ...
                newReload = max(3, cnsts['shipReload'] - (3 * self.state["ships"][key]['level']))
