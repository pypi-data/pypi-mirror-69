...
# Constant definition
# This is not how we should do this in python.
# Would be nice to get rid of this in the future.
OPTION_LOCATION = 0
OPTION_IP = 1
OPTION_DNS = 2
OPTION_CONTINENT = 3
...
def run_command(cmd: str) -> (int, str):
    ...
    :rtype: tuple
    """
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, shell=True, preexec_fn=os.setsid)
...
def get_servers_programs(ip_or_hostname: str, ssh=False) -> list:
    """
    ...
    cmd = 'ssh -o PasswordAuthentication=no -o UserKnownHostsFile=/dev/null ' \
          '-o StrictHostKeyChecking=no -o LogLevel=QUIET -o ConnectTimeout=2 ' \
...
def get_server_info(server_id: int, option: int, nodes: list) -> (dict, list):
    ...
    if isinstance(server_id, str):
        ...
        programs = get_servers_programs(ip_or_hostname, info_about_node_dic["sshAvailable"])
...
def update_availability_database_parent(dialog, nodes=None) -> None:
    ...
    dialog.gauge_start()
    try:
        pool = Pool(25, initializer=multi_processing_init,
...
def parallel_copy(dialog, source_path: str, hosts: list, destination_path: str) -> bool:
    ...
    dialog.gauge_update(100, "Completed")
    dialog.gauge_stop()
    # If at least one scp command failed, return True
    return not any(ret)
