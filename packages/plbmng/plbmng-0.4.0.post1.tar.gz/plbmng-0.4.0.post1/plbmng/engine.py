...
from plbmng.lib.library import update_availability_database_parent, plot_servers_on_map, \
    ...
    get_server_info, search_by_location, search_by_regex
...
class Engine:
    ...
    def filtering_options_gui(self, ssh_enabled: bool, ping_enabled: bool) -> tuple:
        """
        Filtering options menu.

        :param ssh_enabled: True if ssh filtering is enabled.
        :type ssh_enabled: bool
        :param ping_enabled: True if ping filtering is enabled.
        :type ping_enabled: bool
        :return: return code from dialog engine and a tag('1' or '2').
        :rtype: tuple
        """
    ...
    def copy_file(self):
        ...
        init = "/home/%s" % get_ssh_user()
        code, source_path = self.d.fselect(filepath="/home/", height=0, width=0)
        if code == self.d.OK:
        ...
        if not servers:
            ...
            return
        code, destination_path = self.d.inputbox(text=text, init=init)
        if code == self.d.OK:
    ...
    def access_servers_gui(self, checklist=False):
        ...
        while True:
            ...
            if code == self.d.OK:
                ...
                nodes = self.db.get_nodes(path=self.path)
                if tag == "1":
                    self.filtering_options_gui(self.db.get_ssh_enabled(), self.db.get_ping_enabled())
                # Access last server
                elif tag == "2":
                ...
                # Search by location
                    ...
                    ret = self.search_by_location_menu(checklist)
                    if checklist:
                        return ret
                elif tag == "6":
    ...
    def search_by_location_menu(self, checklist: bool):
        """
        ...
        if returned_choice is None:
            return
        choices = [(country, "") for country in countries.keys()]
        returned_choice = self.search_nodes_gui(choices)
        if returned_choice is None:
            return
        if not checklist:
