...
class KalmanFilter(object):
    def __init__(self, farm, ens_size=100, cv=0.5, zeta=0.9, xi=0.97, fn_info_file=None):
        """
    ...
    def _generate_prior_params(self, dist_func=np.random.normal):
        ...
        temp_df.iloc[:, :] = prior_param_k1_arr
    ...
    def _K(self, params, marg_cost_lst, marg_rev_lst, diagonalize_r=False):
        """Returns Kalman gain"""
        ...
        if diagonalize_r:
            r = np.diag(np.diag(r))
        dem = p + r
        inv_dem = np.linalg.inv(dem)
        return num.dot(inv_dem)

    def assimilate(self, obs, obs_var_scale_factor=None):
        # type: (dict) -> None
        """
        ...
        k = self._K(self.prior_params_k1.values, marg_cost_lst, marg_rev_lst, diagonalize_r=False)
