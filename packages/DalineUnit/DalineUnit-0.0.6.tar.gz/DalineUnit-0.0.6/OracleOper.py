import cx_Oracle
# 若出现”cx_Oracle.DatabaseError: ORA-12514: TNS: 监听程序当前无法识别连接描述符中请求的服务“错误，在sql plus中查询下面，然后将service name替换
...
#
# db = cx_Oracle.connect('system','Passw0rd','localhost:1521/ZHANG.ZKTEST')
# print(db.version)
# mycursor = db.cursor()
#
# str_sql = "select table_name from user_tables "
...
# for each in sql_res:
#     print('----------------------%s-----------------------------'%each)
#     str_sql = "SELECT column_name, data_type FROM all_tab_cols WHERE table_name = '%s'"%(each[0])
#     print(str_sql)
#     mycursor.execute(str_sql)
#     table_info = mycursor.fetchall()
#     for each_table_info in table_info:
#         print(each_table_info)
