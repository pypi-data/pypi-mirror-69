...
from fuzzyInject.interface_send import assemble_params, assemble_params_raw_other, assemble_params_formdata
    ...
    if request_result_list != [] and param_get_list != [] and request_result_url != []:
        for add_index in range(0, len(request_result_list)):
            ...
            if request_result_list[add_index] is None:
                ...
                return
            if 'status' not in request_result_list[add_index] or '"status":500' in request_results_list[add_index]:
                result_collections.append('FAIL')
            else:
