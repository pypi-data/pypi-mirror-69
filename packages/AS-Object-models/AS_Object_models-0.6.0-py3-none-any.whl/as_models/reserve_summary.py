...
class ReserveSummary(SalesInvBase):
    ...
    def update_reserve(self,item_reserve):
        updated_entry = jmespath.search(self.update_jpath+" | [0]",[item_reserve.get_dict()])
    ...
    def delete_reserve(self,item_reserve):
        newSumm = [x for x in self.summary if x != item_reserve.id]
        self.summary = newSumm
        self.update_ndb()
        return item_reserve.id
    
    def add_reserve(self,item_reserve):
        added_entry = jmespath.search(self.update_jpath+" | [0]",[item_reserve.get_dict()])
