...
class _AssertRewriter(ast.NodeVisitor):
    ...
    def visit_assertIs(self, node, positive=True):
        ...
        if isinstance(right, ast.Constant):
            if right.value in (True, False) and isinstance(right.value, bool):
                func = f"assert{right.value and positive}"
...
def main():
    ...
    parser.add_argument(
        "--blacklist",
        nargs="*",
        help="List of assertions to omit from refactoring process",
        default=[],
    )
    options = parser.parse_args()

    total_files, total_refactors = 0, []
    ...
    for test_file in options.path.glob(options.pattern):
        ...
        refactored_source, refactors = refactor_until_deterministic(
            source, blacklist=frozenset(options.blacklist)
        )
    ...
    if total_files == 0:
        ...
        for key, amount in retrieve_stats(total_refactors):
            print(
                "{:25}=> {:25}".format(*key),
        ...
        print(
            f"{len(total_refactors)} assertions (in {total_files} files) have been refactored."
        )
