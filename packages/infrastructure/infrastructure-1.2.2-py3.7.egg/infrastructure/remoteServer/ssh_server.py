# -*- coding: utf-8 -*-
# @Author: yongfanmao
# @Date:   2020-05-18 15:07:54
# @E-mail: maoyongfan@163.com
# @Last Modified by:   yongfanmao
# @Last Modified time: 2020-05-20 15:45:59
import paramiko
import threading
import time
from sshtunnel import SSHTunnelForwarder
from io import StringIO

class ConnectServerDM(object):
	"""
		通过跳板机连接远程server(支持办公环境已经服务器环境)
	"""
	def __init__(self,serverIP,ssh_pkey="/home/maoyongfan10020/.ssh/id_rsa",isServerEnv=True):
		"""
			isServerEnv 是从服务器还是办公环境访问跳板机
		"""
		self.serverIP = serverIP
		self.ssh_pkey = ssh_pkey
		if isServerEnv:
			self.dumpIp = "10.111.80.92"
		else:
			self.dumpIp = "47.96.131.92"
		# self.server = SSHTunnelForwarder(
		# 	ssh_address_or_host=(dumpIp,22),
		# 	ssh_username="maoyongfan10020",
		# 	ssh_pkey=ssh_pkey,
		# 	local_bind_address=('0.0.0.0', 10022),
		# 	remote_bind_address=(serverIP,22)
		# 	)
		# self.server.start()

		self.dump_key = """-----BEGIN RSA PRIVATE KEY-----
		
		-----END RSA PRIVATE KEY-----"""

		self.private_key = paramiko.RSAKey(file_obj=StringIO(self.dump_key))

		
		# self.client = paramiko.SSHClient()
		# self.client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
		# self.client.connect(self.server.local_bind_host, self.server.local_bind_port, username='maoyongfan10020', pkey=private_key)
		# self.shell = self.client.invoke_shell()

	def execute(self,operation,local_path="",remote_path="",command=""):
		with SHTunnelForwarder(
			ssh_address_or_host=(self.dumpIp,22),
			ssh_username="maoyongfan10020",
			ssh_pkey=self.ssh_pkey,
			local_bind_address=('0.0.0.0', 10022),
			remote_bind_address=(self.serverIP,22)
		) as server:
			transport = paramiko.Transport((server.local_bind_host, server.local_bind_port))
			transport.connect(username='maoyongfan10020',pkey=self.private_key)
			if operation == "upload":
				sftp = paramiko.SFTPClient.from_transport(transport)
				sftp.put(local_path, remote_path)
				transport.close()
				return "finish"
			elif operation == "download":
				sftp = paramiko.SFTPClient.from_transport(transport)
				sftp.get(remote_path,local_path)
				transport.close()
				return "finish"
			elif operation == "exec_command":
				ssh = paramiko.SSHClient()
				ssh._transport = transport
				# 执行命令
				stdin, stdout, stderr = ssh.exec_command(command)
				# 获取命令结果
				result = stdout.readlines()
				transport.close()
				return result				

	def upload(self,local_path,target_path):
		"""
			连接上传
		"""
		sftp = paramiko.SFTPClient.from_transport(self.__transport)
		# 将location.py 上传至服务器 /tmp/test.py
		sftp.put(local_path, target_path)
		# print(os.stat(local_path).st_mode)
		# 增加权限
		# sftp.chmod(target_path, os.stat(local_path).st_mode)
		# sftp.chmod(target_path, 0o755)  # 注意这里的权限是八进制的，八进制需要使用0o作为前缀

	def download(self,remote_path,local_path):
		sftp = paramiko.SFTPClient.from_transport(self.__transport)
		sftp.get(remote_path,local_path)

	def exec_command(self,command):
		"""
			command 
		"""
		ssh = paramiko.SSHClient()
		ssh._transport = self.__transport
		# 执行命令
		stdin, stdout, stderr = ssh.exec_command(command)
		# 获取命令结果
		result = stdout.readlines()
		return result

	# def close(self):
	# 	self.__transport.close()
	# 	self.server.close()

	# def __del__(self):
	# 	self.close()
