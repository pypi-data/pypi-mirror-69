...
from .private.task_imregrid import imregrid as _imregrid_t
...
class _imregrid:
    ...
    * the name of an image from which to get the coordinate system and shape.
    ...
    def __call__( self, imagename='', template='get', output='', asvelocity=True, axes=[ int(-1) ], shape=[ int(-1) ], interpolation='linear', decimate=int(10), replicate=False, overwrite=False ):
        schema = {'imagename': {'type': 'cReqPath', 'coerce': _coerce.expand_path}, 'template': {'type': 'cVariant', 'coerce': [_coerce.to_variant]}, 'output': {'type': 'cStr'}, 'asvelocity': {'type': 'cBool'}, 'axes': {'type': 'cIntVec', 'coerce': [_coerce.to_list,_coerce.to_intvec]}, 'shape': {'type': 'cIntVec', 'coerce': [_coerce.to_list,_coerce.to_intvec]}, 'interpolation': {'type': 'cStr'}, 'decimate': {'type': 'cInt'}, 'replicate': {'type': 'cBool'}, 'overwrite': {'type': 'cBool'}}
        doc = {'imagename': imagename, 'template': template, 'output': output, 'asvelocity': asvelocity, 'axes': axes, 'shape': shape, 'interpolation': interpolation, 'decimate': decimate, 'replicate': replicate, 'overwrite': overwrite}
        ...
        return _end_log( _logging_state_, 'imregrid', _imregrid_t( _pc.document['imagename'], _pc.document['template'], _pc.document['output'], _pc.document['asvelocity'], _pc.document['axes'], _pc.document['shape'], _pc.document['interpolation'], _pc.document['decimate'], _pc.document['replicate'], _pc.document['overwrite'] ) )
