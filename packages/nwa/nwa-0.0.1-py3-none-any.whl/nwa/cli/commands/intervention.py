"""Intervention module for CLI.

  Typical usage example:

  intervention_command = InterventionCommand()
  intervention_command.run()
"""
import time
...
class InterventionCommand(BaseCommand):
    """Initialise intervention command.

    Example:
        nwa intervention -f <json_file>
    """

    def __init__(self):
        super(InterventionCommand, self).__init__()
    ...
    def analyse(self, file: str = "", output: str = "") -> None:
        """Calls the intervention analysis with the provided graph data.
