...
class IndexMonitor:
    ...
    def _etf_statistic_to_df(self, statistics: list) -> pandas.DataFrame:
        ...
        df = pandas.DataFrame(
            data=np.array(
                [index_code, index_name, etf_code, etf_name, pe_less_prob_10y, pe_mean_div_rt,
                 pe_percent_signal,
                 pe_less_prob_10y_2m_ago, pe_max_rt, pe_latest, pe_mean, pe_max, pb_less_prob_10y,
                 ai_signal,
                 pe_volatility_3y, real_year, last_dt]).T,
            columns=['指数代码', '指数名称', 'ETF代码', 'ETF名称', 'PE分位', 'PE均值比', '估值水平',
