...
async def do_sync(sync_to: Arrow = None, max_bars: int = None,
    ...
    max_bars = max_bars or cfg.omega.sync.max_bars
    # noinspection PyPep8
    while code := await cache.sys.lpop(key_scope):
        try:
            await sync_for_sec(code, sync_to, max_bars, frame_type)
...
async def sync_for_sec(code: str, end: Arrow, max_bars: int,
                       frame_type: FrameType = None):
    counters = {frame: 0 for frame in cfg.omega.sync.frames}
    logger.info("syncing quotes for %s", code)

    if frame_type:
        sync_frame_types = [frame_type]
    else:
        sync_frame_types = [FrameType(v) for v in cfg.omega.sync.frames]

    for frame_type in sync_frame_types:
