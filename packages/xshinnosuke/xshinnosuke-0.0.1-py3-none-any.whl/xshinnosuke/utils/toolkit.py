...
class DataLoader:
    ...
    def make_batches(self, datas: List, batch_size: int, seed: int, shuffle: bool = False):
        np.random.seed(seed)
        m = datas[0].shape[0]
        if shuffle:
...
def im2col(inputs, out_h: int, out_w: int, kernel_h: int, kernel_w: int, stride: int):
...
def col2im(inputs_shape: tuple, pad_size: int, kernel_h: int, kernel_w: int, stride: int, dcol):
