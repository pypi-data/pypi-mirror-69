...
class Conv2D(Layer):
    def __init__(self, out_channels: int, kernel_size: int, use_bias: bool = False, stride: int = 1, padding: int = 0,
                 activation: str = None, kernel_initializer: str = 'Normal', bias_initializer: str = 'zeros',
                 input_shape: Tuple = None,
                 **kwargs):
        self.out_channels = out_channels
        self.kernel_size = self.__check_kernel_size(kernel_size)
    ...
    def __check_kernel_size(self, kernel_size):
    ...
    def compute_output_shape(self, input_shape: Tuple = None):
        ...
        if self.padding.__class__.__name__ == 'str':
            ...
            n_H = (n_H_prev - filter_h + 2 * self.padding) // self.stride + 1
            n_W = (n_W_prev - filter_w + 2 * self.padding) // self.stride + 1
...
class MaxPooling2D(Layer):
    ...
    def compute_output_shape(self, input_shape=None):
        ...
        if self.kernel_size == self.stride:
            ...
            self.mode = 'im2col'
        n_h, n_w = (n_h_prev - self.kernel_size) // self.stride + 1, (n_w_prev - self.kernel_size) // self.stride + 1
        return n_c, n_h, n_w

    def __call__(self, inbound):
        if isinstance(inbound, Variable):
            output = max_pool2d(inbound, self.kernel_size, self.stride)
            # output是一个Variable
            return output
    ...
    def forward(self, x: Variable = None, is_training=True, *args):
        if x is not None:
            self.input_data = x
        self.data = max_pool2d(self.input_data, self.kernel_size, self.stride)
        self.connect_init(self.data, is_training)
        return self.data
...
class AvgPooling2D(Layer):
    ...
    def __call__(self, inbound):
        if isinstance(inbound, Variable):
            output = avg_pool2d(inbound, self.kernel_size, self.stride)
            # output是一个Variable
            return output
    ...
    def forward(self, x: Variable = None, is_training=True, *args):
        if x is not None:
            self.input_data = x
        self.data = avg_pool2d(self.input_data, self.kernel_size, self.stride)
        self.connect_init(self.data, is_training)
        return self.data
