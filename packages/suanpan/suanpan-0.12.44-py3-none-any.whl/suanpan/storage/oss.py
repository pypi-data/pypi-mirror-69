...
class Storage(base.Storage):
    ...
    def download(self, name, path, bucketOrBucketName=None, ignores=None, workers=None):
    ...
    @runtime.retry(stop_max_attempt_number=3)
    def upload(self, name, path, bucketOrBucketName=None, ignores=None, workers=None):
        bucket = self.getBucket(bucketOrBucketName)
        ignores = ignores or self.DEFAULT_IGNORE_KEYWORDS
        uploadFunction = (
    ...
    @runtime.retry(stop_max_attempt_number=3)
    def copy(self, name, dist, bucketOrBucketName=None, workers=None):
        bucket = self.getBucket(bucketOrBucketName)
        copyFunction = (
            self.copyFile
            if self.isFile(name, bucketOrBucketName=bucket)
