...
class cisco_C4507R:
    def __init__(self,file):
        ...
        for line in read_file_list:
            #MEMORY
            #Memory Total
            if re.findall('^Total:\s+(\d+),\s+Used:\s+\d+',line):
                memory_break = True
                memory_total = re.findall('^Total:(\s+\d+),\s+Used:\s+\d+',line)
                memory_total = memory_total[0]
            #Memory Used
            if re.findall('^Total:\s+\d+,\s+Used:\s+(\d+)',line):
                memory_used = re.findall('^Total:\s+\d+,\s+Used:\s+(\d+)',line)
        ...
        for i in read_file_list:
            ...
            if re.findall('^Fantray : (.*)', i):
                ...
                #print(fan_cond)
            if re.findall('^Chassis (Temperature).*=.*',i):
                regex_temp = re.findall('^Chassis (Temperature).*=.*',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^Chassis Temperature.*=(.*)', i):
                regex_temp_cond = re.findall('^Chassis Temperature.*=(.*)', i)
