...
class cisco_C9500:
    def __init__(self,file):
        ...
        for i in read_file_list:
            if re.findall('^(Switch\s+\S+\s+FAN\s+\S+)\s+is\s+\S+',i):
                regex_fan = re.findall('^(Switch\s+\S+\s+FAN\s+\S+)\s+is\s+\S+',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('^Switch\s+\S+\s+FAN\s+\S+\s+is\s+(\S+)', i):
                regex_fan_cond = re.findall('^Switch\s+\S+\s+FAN\s+\S+\s+is\s+(\S+)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('^(Switch\s+\S+):\s+SYSTEM\s+TEMPERATURE\s+is\s+\S+',i):
                regex_temp = re.findall('^(Switch\s+\S+):\s+SYSTEM\s+TEMPERATURE\s+is\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^Switch\s+\S+:\s+SYSTEM\s+TEMPERATURE\s+is\s+(\S+)', i):
                regex_temp_cond = re.findall('^Switch\s+\S+:\s+SYSTEM\s+TEMPERATURE\s+is\s+(\S+)', i)
