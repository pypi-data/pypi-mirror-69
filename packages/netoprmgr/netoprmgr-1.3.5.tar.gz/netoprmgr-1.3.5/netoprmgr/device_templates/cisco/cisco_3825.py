...
class cisco_3825:
    def __init__(self,file):
        ...
        for line in read_file_list:
            ...
            elif re.findall('^Cisco IOS.*C3825.*Version (.*),',line):
                version = re.findall('^Cisco IOS.*C3825.*Version (.*),',line)
                version = version[0]
        ...
        for i in read_file_list:
            ...
            if re.findall('^.*Fan\s+\d+\s+(\S+)', i):
                ...
                #print(fan_cond)
            if re.findall('^(.*temperature) =\s+\S+\s+\S+',i):
                regex_temp = re.findall('^(.*temperature) =\s+\S+\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^.*temperature =\s+\S+\s+(\S+)', i):
                regex_temp_cond = re.findall('^.*temperature =\s+\S+\s+(\S+)', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('^(.*SYS\s+\S+)\s+\S+\s+\S+.',i):
                regex_psu = re.findall('^^(.*SYS\s+\S+)\s+\S+\s+\S+.',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(internal_psu)
            if re.findall('^.*SYS\s+\S+\s+\S+\s+(\S+).', i):
                regex_psu_cond = re.findall('^.*SYS\s+\S+\s+\S+\s+(\S+).', i)
