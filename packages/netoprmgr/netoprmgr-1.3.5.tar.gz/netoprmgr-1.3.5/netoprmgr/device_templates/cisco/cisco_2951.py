...
class cisco_2951:
    def __init__(self,file):
        ...
        for line in read_file_list:
            ...
            elif re.findall('^Cisco IOS.*, C2951.*Version (.*),',line):
                version = re.findall('^Cisco IOS.*, C2951.*Version (.*),',line)
                version = version[0]
        ...
        for i in read_file_list:
            if re.findall('^\s+(Fan\s+\S+)\s+\S+,',i):
                regex_fan = re.findall('^\s+(Fan\s+\S+)\s+\S+,',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('^\s+Fan\s+\S+\s+(\S+),', i):
                regex_fan_cond = re.findall('^\s+Fan\s+\S+\s+(\S+),', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('^(.*temperature):\s+\S+\s+Celsius,\s+\S+',i):
                regex_temp = re.findall('^(.*temperature):\s+\S+\s+Celsius,\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^.*temperature:\s+\S+\s+Celsius,\s+(\S+)', i):
                regex_temp_cond = re.findall('^.*temperature:\s+\S+\s+Celsius,\s+(\S+)', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('^.*(Internal Power Supply)\s+\S+\s+\S+\s+Status:\s+\S+',i):
                regex_in_psu = re.findall('^.*(Internal Power Supply)\s+\S+\s+\S+\s+Status:\s+\S+',i)
                in_psu = regex_in_psu[0]
                list_psu.append(in_psu)
                #print(internal_psu)
            if re.findall('^.*Internal Power Supply\s+\S+\s+\S+\s+Status:\s+(\S+)', i):
                regex_in_psu_cond = re.findall('^.*Internal Power Supply\s+\S+\s+\S+\s+Status:\s+(\S+)', i)
                in_psu_cond = regex_in_psu_cond[0]
                list_psu_cond.append(in_psu_cond)
                #print(internal_psu_cond)
            if re.findall('.*(External Redundant Power Supply)\s+is\s+.*',i):
                regex_ex_psu = re.findall('.*(External Redundant Power Supply)\s+is\s+.*',i)
                ex_psu = regex_ex_psu[0]
                list_psu.append(ex_psu)
                #print(internal_psu)
            if re.findall('.*External Redundant Power Supply\s+is\s+(.*)', i):
                regex_ex_psu_cond = re.findall('.*External Redundant Power Supply\s+is\s+(.*)', i)
                ex_psu_cond = regex_ex_psu_cond[0]
                list_psu_cond.append(ex_psu_cond)
                #print(internal_psu_cond)
