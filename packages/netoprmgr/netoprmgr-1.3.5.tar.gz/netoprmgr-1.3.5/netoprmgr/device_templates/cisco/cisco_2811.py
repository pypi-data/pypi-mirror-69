    ...
    def __init__(self,file):
        ...
        for i in read_file_list:
            if re.findall('.*(Fan\s+\d+)\s+.*',i):
                ...
                #print(fan)
            if re.findall('.*Fan\s+\d+\s+(.*)', i):
                regex_fan_cond = re.findall('.*Fan\s+\d+\s+(.*)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('.*(Temperature):\s+\d+\s+\S+\s+[\S+].*[\S+]',i):
                regex_temp = re.findall('.*(Temperature):\s+\d+\s+\S+\s+[\S+].*[\S+]',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('.*Temperature:\s+\d+\s+\S+\s+[\S+](.*)[\S+]', i):
                regex_temp_cond = re.findall('.*Temperature:\s+\d+\s+\S+\s+[\S+](.*)[\S+]', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('.*(Power Supply)\s+is\s+.*',i):
                regex_psu = re.findall('.*(Power Supply)\s+is\s+.*',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(temp)
            if re.findall('.*Power Supply\s+is\s+(.*)', i):
                regex_psu_cond = re.findall('.*Power Supply\s+is\s+(.*)', i)
