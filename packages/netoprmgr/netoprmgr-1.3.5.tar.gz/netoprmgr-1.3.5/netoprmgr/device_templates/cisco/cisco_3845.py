...
class cisco_3845:
    def __init__(self,file):
        ...
        for line in read_file_list:
            ...
            elif re.findall('^Cisco IOS.*, 3800.*Version (.*),',line):
                version = re.findall('^Cisco IOS.*, 3800.*Version (.*),',line)
                version = version[0]
        ...
        for i in read_file_list:
            ...
            if re.findall('^\s+Fan\s+\d+\s+(\S+)', i):
                ...
                #print(fan_cond)
            if re.findall('^\s+(\S+\s+temperature) =\s+\S+,\s+\S+',i):
                regex_temp = re.findall('^\s+(\S+\s+temperature) =\s+\S+,\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^\s+\S+\s+temperature =\s+\S+,\s+(\S+)', i):
                regex_temp_cond = re.findall('^\s+\S+\s+temperature =\s+\S+,\s+(\S+)', i)
