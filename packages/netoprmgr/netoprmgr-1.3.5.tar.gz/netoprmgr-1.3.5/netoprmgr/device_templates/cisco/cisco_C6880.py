...
class cisco_C6880:
    def __init__(self,file):
        ...
        for i in read_file_list:
            if re.findall('.*(switch\s+\S+\sfan-tray\s+\S+)\s+fan-fail:\s+\S+',i):
                regex_fan = re.findall('.*(switch\s+\S+\sfan-tray\s+\S+)\s+fan-fail:\s+\S+',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('.*switch\s+\S+\sfan-tray\s+\S+\s+fan-fail:\s+(\S+)', i):
                regex_fan_cond = re.findall('.*switch\s+\S+\sfan-tray\s+\S+\s+fan-fail:\s+(\S+)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('.*(switch\s+\S+\s+module\s+\S+\s+\S+let\S+)\s+temperature:\s+\S+',i):
                regex_temp = re.findall('.*(switch\s+\S+\s+module\s+\S+\s+\S+let\S+)\s+temperature:\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('.*switch\s+\S+\s+module\s+\S+\s+\S+let\S+\s+temperature:\s+(\S+)', i):
                regex_temp_cond = re.findall('.*switch\s+\S+\s+module\s+\S+\s+\S+let\S+\s+temperature:\s+(\S+)', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('.*(switch\s+\S+\s+power-supply\s+\S+)\spower-output-fail:\s+\S+',i):
                regex_psu = re.findall('.*(switch\s+\S+\s+power-supply\s+\S+)\spower-output-fail:\s+\S+',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(temp)
            if re.findall('.*switch\s+\S+\s+power-supply\s+\S+\spower-output-fail:\s+(\S+)', i):
                regex_psu_cond = re.findall('.*switch\s+\S+\s+power-supply\s+\S+\spower-output-fail:\s+(\S+)', i)
