    ...
    def __init__(self,file):
        ...
        for i in read_file_list:
            ...
            if re.findall('^Fantray\s+:\s+(\S+)', i):
                ...
                #print(fan_cond)
            if re.findall('^(\S+\s+air\s\S+let)\s+\S+\s+\S+\s+\S+',i):
                regex_temp = re.findall('^(\S+\s+air\s\S+let)\s+\S+\s+\S+\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^\S+\s+air\s\S+let\s+\S+\s+\S+\s+(\S+)', i):
                regex_temp_cond = re.findall('^\S+\s+air\s\S+let\s+\S+\s+\S+\s+(\S+)', i)
