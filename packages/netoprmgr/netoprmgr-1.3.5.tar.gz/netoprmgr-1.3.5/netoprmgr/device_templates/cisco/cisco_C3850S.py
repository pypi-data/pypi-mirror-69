...
class cisco_C3850S:
    def __init__(self,file):
        ...
        for line in read_file_list:
            #MEMORY
            #Memory Total
            if re.findall('.*System memory\s+:\s+(\d+)K\s\S+\s+\d+.*',line):
                memory_break = True
                memory_total = re.findall('.*System memory\s+:\s+(\d+)K\s\S+\s+\d+.*',line)
                memory_total = memory_total[0]
            #Memory Used
            if re.findall('.*System memory\s+:\s+\d+K\s\S+\s+(\d+).*',line):
                memory_used = re.findall('.*System memory\s+:\s+\d+K\s\S+\s+(\d+).*',line)
        ...
        for i in read_file_list:
            if re.findall('^(Switch\s+\d+\s+FAN\s+\d+)\sis\s+.*',i):
                regex_fan = re.findall('^(Switch\s+\d+\s+FAN\s+\d+)\sis\s+.*',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('^Switch\s+\d+\s+FAN\s+\d+\sis\s+(.*)', i):
                regex_fan_cond = re.findall('^Switch\s+\d+\s+FAN\s+\d+\sis\s+(.*)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('^.*(TEMPERATURE) is .*',i):
                regex_temp = re.findall('^.*(TEMPERATURE) is .*',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^.*TEMPERATURE is (.*)', i):
                regex_temp_cond = re.findall('^.*TEMPERATURE is (.*)', i)
