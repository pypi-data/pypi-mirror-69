...
class cisco_C6504:
    def __init__(self,file):
        ...
        for i in read_file_list:
            if re.findall('.*(fan-tray\s+\S+)\s+fan-fail:\s+\S+',i):
                regex_fan = re.findall('.*(fan-tray\s+\S+)\s+fan-fail:\s+\S+',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('.*fan-tray\s+\S+\s+fan-fail:\s+(\S+)', i):
                regex_fan_cond = re.findall('.*fan-tray\s+\S+\s+fan-fail:\s+(\S+)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('.*(module\s+\S+\s+\S+let)\s+temperature:\s+\S+',i):
                regex_temp = re.findall('.*(module\s+\S+\s+\S+let)\s+temperature:\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('.*module\s+\S+\s+\S+let\s+temperature:\s+(\S+)', i):
                regex_temp_cond = re.findall('.*module\s+\S+\s+\S+let\s+temperature:\s+(\S+)', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('.*(power-supply\s+\S+)\spower-output-fail:\s+\S+',i):
                regex_psu = re.findall('.*(power-supply\s+\S+)\spower-output-fail:\s+\S+',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(temp)
            if re.findall('.*power-supply\s+\S+\spower-output-fail:\s+(\S+)', i):
                regex_psu_cond = re.findall('.*power-supply\s+\S+\spower-output-fail:\s+(\S+)', i)
