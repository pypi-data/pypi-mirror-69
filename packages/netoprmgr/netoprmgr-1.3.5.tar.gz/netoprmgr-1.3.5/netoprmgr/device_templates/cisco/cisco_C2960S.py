...
class cisco_C2960S:
    def __init__(self,file):
        ...
        for i in read_file_list:
            if re.findall('^(FAN) is .*',i):
                regex_fan = re.findall('^(FAN) is .*',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('^FAN is (.*)', i):
                regex_fan_cond = re.findall('^FAN is (.*)', i)
            ...
            if re.findall('^.*TEMPERATURE is (.*)', i):
                ...
                #print(temp_cond)
            if re.findall('^(POWER) is .*',i):
                regex_psu = re.findall('^(POWER) is .*',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(temp)
            if re.findall('^POWER is (.*)', i):
                regex_psu_cond = re.findall('^POWER is (.*)', i)
