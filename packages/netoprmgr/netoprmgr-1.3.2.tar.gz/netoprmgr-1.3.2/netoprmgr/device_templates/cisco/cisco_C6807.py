    ...
    def __init__(self,file):
        ...
        for i in read_file_list:
            ...
            if re.findall('.*switch\s+\S+\sfan-tray\s+\S+\s+fan-fail:\s+(\S+)', i):
                ...
                #print(fan_cond)
            if re.findall('.*(switch\s+\S+\s+module\s+\S+\s+\S+let)\s+temperature:\s+\S+',i):
                regex_temp = re.findall('.*(switch\s+\S+\s+module\s+\S+\s+\S+let)\s+temperature:\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('.*switch\s+\S+\s+module\s+\S+\s+\S+let\s+temperature:\s+(\S+)', i):
                regex_temp_cond = re.findall('.*switch\s+\S+\s+module\s+\S+\s+\S+let\s+temperature:\s+(\S+)', i)
