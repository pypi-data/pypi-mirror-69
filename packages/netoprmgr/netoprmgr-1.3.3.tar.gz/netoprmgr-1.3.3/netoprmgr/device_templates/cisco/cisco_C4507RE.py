    ...
    def __init__(self,file):
        ...
        for line in read_file_list:
            ...
            #get device model
                ...
                model = re.findall('^cisco\s+(\S+).*processor.*with.*bytes',line)
                model = model[0]                
            #get ios version
            elif re.findall('boot\ssystem\s\S+\s(\S+)',line):
                iosversion = re.findall('boot\ssystem\s\S+\s(\S+)',line)
            ...
            elif re.findall('^Cisco IOS Software,\s+IOS-XE\s+\S+\sCatalyst 4500.*Version\s+(\S+).*',line):
                version = re.findall('^Cisco IOS Software,\s+IOS-XE\s+\S+\sCatalyst 4500.*Version\s+(\S+).*',line)
                version = version[0]
        ...
        for line in read_file_list:
        #CPU
            if re.findall('.*Core\s+\d+\S+\s+CPU utilization for five seconds:\s+(\d+)',line):
                cpu_action = 'start'
            if cpu_action == 'start' and re.findall('.*Core\s+\d+\S+\s+CPU utilization for five seconds:\s+(\d+)',line):
                cpu_value = re.findall('.*Core\s+\d+\S+\s+CPU utilization for five seconds:\s+(\d+)',line)
                cpu_list.append(int(cpu_value[0]))
            elif cpu_action == 'start':
                break        

        total = (sum(cpu_list))/(len(cpu_list))
        interrupt = '0'
        ...
        for line in read_file_list:
            #MEMORY
            #Memory Total
            if re.findall('^System memory\s+\S+\s+(\d+)K\s+total,\s+\d+K\s+used',line):
                memory_break = True
                memory_total = re.findall('^System memory\s+\S+\s+(\d+)K\s+total,\s+\d+K\s+used',line)
                memory_total = memory_total[0]
            #Memory Used
            if re.findall('^System memory\s+\S+\s+\d+K\s+total,\s+(\d+)K\s+used',line):
                memory_used = re.findall('^System memory\s+\S+\s+\d+K\s+total,\s+(\d+)K\s+used',line)
        ...
        for i in read_file_list:
            if re.findall('^(Fantray)\s+:\s+\S+',i):
                regex_fan = re.findall('^(Fantray)\s+:\s+\S+',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('^Fantray\s+:\s+(\S+)', i):
                regex_fan_cond = re.findall('^Fantray\s+:\s+(\S+)', i)
