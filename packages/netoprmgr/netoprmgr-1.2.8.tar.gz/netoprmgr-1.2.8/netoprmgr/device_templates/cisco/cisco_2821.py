...
class cisco_2821:
    def __init__(self,file):
        ...
        for line in read_file_list:
            ...
            elif re.findall('^.*isco.*,\s+Version (.*),',line):
                version = re.findall('^.*isco.*,\s+Version (.*),',line)
                version = version[0]
        ...
        for i in read_file_list:
            if re.findall('.*(Fan\s+\d+)\s+\S+',i):
                regex_fan = re.findall('.*(Fan\s+\d+)\s+\S+',i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('.*Fan\s+\d+\s+(\S+)', i):
                regex_fan_cond = re.findall('.*Fan\s+\d+\s+(\S+)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('.*(Temperature):\s+\d+\s+\S+\s+[\S+]\S+[\S+]',i):
                regex_temp = re.findall('.*(Temperature):\s+\d+\s+\S+\s+[\S+]\S+[\S+]',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('.*Temperature:\s+\d+\s+\S+\s+[\S+](\S+)[\S+]', i):
                regex_temp_cond = re.findall('.*Temperature:\s+\d+\s+\S+\s+[\S+](\S+)[\S+]', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('(.*Power Supply)\s+is\s+\S+',i):
                regex_psu = re.findall('(.*Power Supply)\s+is\s+\S+',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(temp)
            if re.findall('.*Power Supply\s+is\s+(\S+)', i):
                regex_psu_cond = re.findall('.*Power Supply\s+is\s+(\S+)', i)
