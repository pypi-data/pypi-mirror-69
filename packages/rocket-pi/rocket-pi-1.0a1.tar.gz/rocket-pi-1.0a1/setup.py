...
def adjust_rocketlauncher(setupDir):
    ...
    if not os.path.isfile("RIB_App"):
       ...
       print("Installation of the the demonstration project max7219 is done. Enjoy, dot_test launches at start ...")
       return -1


    body = body % (setupDir + "/rocketlauncher/rocketlauncherD.sh", setupDir + "/rocketlauncher")
    text_file = open(filename, "w")
    text_file.write(body)
    text_file.close()
