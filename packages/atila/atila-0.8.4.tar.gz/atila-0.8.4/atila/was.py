...
class WAS (wsgiappservice.WAS):
    ...
    def Map (self, tasks, *keys):
        def mapped_api (was, tasks, keys, status):
            results = tasks.fetch ()
        ...
        if isinstance (tasks, str):
            status = tasks
            tasks, keys = keys [0], keys [1:]
        assert tasks.count () == len (keys), "Count of tasks and keys not nmatch"
        return tasks.then (partial (mapped_api, keys = keys, status = status))
