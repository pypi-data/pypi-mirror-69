...
class WAS (wsgiappservice.WAS):
    ...
    def ThreadFuture (self, target, *args, **kargs):
    ...
    def Static (self, uri):
        if not uri.startswith ('/'):
            uri = self.static (uri)
        return self.response.Static (uri)
    ...
    # Map responses ------------------------------------
    def _mapped_api_callback (self, was, tasks, keys, status, template_file = None):
        results = tasks.fetch ()
        if tasks.count () == 1 and not isinstance (results, (list, tuple)):
            results = [results]
        data = {}
        ...
        if template_file:
            return was.render (template_file, data)
        else:
            return was.API (status, data)

    def _create_tasks (self, *commits, **corequests):
        keys = []
        ts = []
        for k, v in corequests.items ():
            keys.append (k)
            ts.append (v)
        for k in commits:
            keys.append (None)
            ts.append (k)
        return keys, len (ts) == 1 and ts [0] or self.Tasks (*ts)

    def Map (self, *commits, **corequests):
        status = "200 OK"
        if commits and isinstance (commits [0], str):
            status, commits = commits [0], commits [1:]
        keys, tasks = self._create_tasks (*commits, **corequests)
        respfunc = partial (self._mapped_api_callback, keys = keys, status = status)
        return tasks.then (respfunc)

    def render_or_Map (self, template_file, *commits, **corequests):
        if self.request.acceptable ('application/json'):
            return self.Map (*commits, **corequests)
        keys, tasks = self._create_tasks (*commits, **corequests)
        respfunc = partial (self._mapped_api_callback, keys = keys, status = '200 OK', template_file = template_file)
        return tasks.then (respfunc)
