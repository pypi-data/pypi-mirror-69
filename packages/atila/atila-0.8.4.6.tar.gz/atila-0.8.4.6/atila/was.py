...
class WAS (wsgiappservice.WAS):
    ...
    # Map responses ------------------------------------
    def _mapped_api_callback (self, was, tasks, status, template_file = None):
        data = tasks.dict ()
        if template_file:
    ...
    def Map (self, *commits, **corequests):
        ...
        respfunc = partial (self._mapped_api_callback, status = status)
    ...
    def render_or_Map (self, template_file, *commits, **corequests):
        ...
        tasks = self.Tasks (*commits, **corequests)
        respfunc = partial (self._mapped_api_callback, status = '200 OK', template_file = template_file)
        return tasks.then (respfunc)
