...
def test_call_api_retry_statuses(mocker, mock_get_api_key):
    ...
    retry_mock = mocker.patch('urllib3.util.retry.time')

    client = api_client.APIClient("https://httpbin.org")
    # sleeptime is 0 on 1st retry, then backoff * 2^(retry-1)
    _calls = [mock.call.sleep(0.2), mock.call.sleep(0.4), mock.call.sleep(0.8), mock.call.sleep(1.6)]

    def test_status_code(client, status_code):

        retry_mock.reset_mock()

        with pytest.raises(RetryError):
            client.call_api("GET", f"/status/{status_code}")

        assert retry_mock.mock_calls == _calls

    for status in client.STATUS_FORCELIST:
        test_status_code(client, status)
...
def test_create_ingest(mock_request, client):
    response = {
        ...
        "fw_user": "fw_user",
        "config": {"src_fs": "/tmp"},
        'strategy_config': {'dicom': 'dicom', 'root_dirs': 0, 'no_subjects': False, 'no_sessions': False, 'strategy_name': 'folder'},
        "status": 'created',
        "history": [],
...
def assert_ingest(ingest: T.IngestOutAPI, ingest_json):
    ...
    assert ingest.created.strftime(
        "%Y-%m-%d %H:%M:%S") == ingest_json["created"]
    # TODO assert strategy_config


def assert_mock_call(mocked_function, method, path, base_url=_BASE_URL, **kwargs):
    calls = mock.call(method, base_url + path, timeout=(30, 60), **kwargs)
