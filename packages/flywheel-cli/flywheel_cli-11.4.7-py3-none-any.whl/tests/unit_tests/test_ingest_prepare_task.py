...
def prepare_task():
    ...
    prep_task = prepare.PrepareTask(
        db=mock.Mock(**{
            "batch_writer_update_container.return_value.batch_size": 999
        }),
        task=task,
...
@pytest.mark.parametrize("skip_existing,item_exists", [
    (True, False),
    (False, True),
    (False, False),
    (True, True),
...
@pytest.mark.parametrize("detect_duplicates,error_cnt", [
    (True, 0),
    (True, 1),
    (True, 6),
    (False, 0),
