...
class DuplicateFilepathInUploadSet(BaseItemError):
    code = "DD01"
    message = "File Path Conflict in Upload Set"


class DuplicateFilepathInFlywheel(BaseItemError):
    code = "DD02"
    message = "File Path Conflict in Flywheel"


def get_item_error_by_code(code: str):
    errors_map = {cls.code: cls for cls in BaseItemError.__subclasses__()}
    error_cls = errors_map.get(code)
    if not error_cls:
        # pass requested error code to help debugging
        return BaseItemError(code=code)
    return error_cls()
