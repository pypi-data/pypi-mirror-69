...
def test_sdk_client_upload_not_signed_one_chunk(mocker):
    flywheel_mock = mocker.patch('flywheel_cli.ingest.utils.flywheel.Client')
...
def test_sdk_client_upload_not_signed_chunked(mocker):
    uploader = mock.Mock()
    filespec = mock.Mock()
    flywheel_mock = mocker.patch('flywheel_cli.ingest.utils.flywheel.Client')
    flywheel_mock.return_value.upload_file_to_cont_name = uploader
    filespec_mock = mocker.patch('flywheel.FileSpec', return_value=filespec)
    mocker.patch('os.fstat', return_value=AttributeDict({'st_size': utils.SDKClient.MAX_IN_MEMORY_XFER}))
    file_mock = mock.MagicMock()
    file_mock.read.return_value = 'file_content'
