...
def test_summary(db_client, data):
    ...
    data.create("ItemError", item_id=item_id, code=error.code)
    data.create("ItemError", item_id=item_id, code=error.code)
...
def test_audit_logs(db_client, data):
    ...
    unknown_error = errors.BaseItemError
...
def test_add_transaction(db_client, data):
    assert db_transactions.add.transaction_type == "DEFERRED"
...
def test_get_transaction(db_client, data):
    assert db_transactions.get.transaction_type == "DEFERRED"
...
def test_get_all_transaction(db_client, data):
    assert db_transactions.get_all.transaction_type == "DEFERRED"
...
def test_update_transaction(db_client, data):
    assert db_transactions.update.transaction_type == "DEFERRED"
...
def test_bulk_transaction(db_client, data):
    assert db_transactions.bulk.transaction_type == "DEFERRED"
...
def test_start_transaction(db_client, data):
    assert db_transactions.start.transaction_type == "IMMEDIATE"
...
def test_review_transaction(db_client, data):
    assert db_transactions.review.transaction_type == "IMMEDIATE"
...
def test_abort_transaction(db_client, data):
    assert db_transactions.abort.transaction_type == "IMMEDIATE"
...
def test_next_task_transaction(db_client, data):
    assert db_transactions.next_task.transaction_type == "IMMEDIATE"
...
def test_get_progress_transaction(db_client, data):
    assert db_transactions.get_progress.transaction_type == "DEFERRED"
...
def test_get_summary_transaction(db_client, data):
    assert db_transactions.get_summary.transaction_type == "DEFERRED"
...
def test_get_report_transaction(db_client, data):
    assert db_transactions.get_report.transaction_type == "DEFERRED"
...
def test_start_singleton_transaction(db_client, data):
    assert db_transactions.start_singleton.transaction_type == "IMMEDIATE"
    ingest_id = data.create("Ingest")
...
def test_has_unfinished_tasks_transaction(db_client, data):
    assert db_transactions.has_unfinished_tasks.transaction_type == "DEFERRED"
    ingest_id = data.create("Ingest")
    tasks = list(db_client.get_all('Task'))
    assert len(tasks) == 0
...
def test_resolve_subject_transaction(db_client, data):
    assert db_transactions.resolve_subject.transaction_type == "IMMEDIATE"
...
def test_set_ingest_status_transaction(db_client, data):
    assert db_transactions.set_ingest_status.transaction_type == "IMMEDIATE"
    ingest_id = data.create("Ingest")
...
def test_fail_ingest_transaction(db_client, data):
    assert db_transactions.fail_ingest.transaction_type == "IMMEDIATE"
    ingest_id = data.create("Ingest")
...
def test_load_subject_csv_transaction(db_client, data):
    assert db_transactions.load_subject_csv.transaction_type == "IMMEDIATE"
