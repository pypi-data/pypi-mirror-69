...
from cwclientlib.cwproxy import (
    CWProxy, SignedRequestAuth, NoResultError, NoUniqueEntity)
...
class CWProxyClientTC(CubicWebServerTC):
    ...
    def test_exist(self):
        ...
        self.assertTrue(self.client.exist(
            'CWUser'))
        self.assertTrue(self.client.exist(
            'CWUser', login='babar'))
        self.assertTrue(self.client.exist(
            'CWUser', eid=eid))
        self.assertTrue(self.client.exist(
            'CWUser', eid=eid, login='babar'))
        self.assertFalse(self.client.exist('CWNothing'))
        self.assertFalse(self.client.exist(
            'CWUser', login='Sauron', upassword='DOMINATION'))
    ...
    def test_count(self):
        ...
        self.assertEqual(
            self.client.count('CWUser', login='Richard Stallman'), 1)
    ...
    def test_find_one(self):
        ...
        self.assertEqual(
            self.client.find_one('CWUser', login='Richard Stallman'),
            eid_rms)
        self.assertEqual(
            self.client.find_one('CWUser', login='Jimmy Wales'),
            eid_jim)
    ...
    def test_handle_request(self):
        ...
        self.assertEqual(request.url,
                         '{}cwuser/{}'.format(self.config['base-url'], eid))
    ...
    def test_handle_request_bad_parameter(self):
        with self.assertRaisesRegex(TypeError,
                                    "got unexpected keyword argument 'url'"):
