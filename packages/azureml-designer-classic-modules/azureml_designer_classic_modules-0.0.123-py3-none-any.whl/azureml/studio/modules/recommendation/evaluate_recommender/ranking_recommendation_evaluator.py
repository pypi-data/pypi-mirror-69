...
class BaseRankingRecommendationEvaluator(BaseRecommendationEvaluator):
    ...
    def _check_users_valid(self, scored_data: DataTable, test_data: DataTable):
        ...
        if not scored_data.data_frame[self.task.user_column].isin(test_users).all():
            ErrorMapping.throw(
                InvalidDatasetError(dataset1=scored_data.name, reason=f"cannot find some user(s) in {test_data.name}"))
