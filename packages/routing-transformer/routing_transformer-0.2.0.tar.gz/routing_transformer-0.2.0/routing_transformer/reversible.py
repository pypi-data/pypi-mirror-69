...
class _ReversibleFunction(Function):
    ...
    def forward(ctx, x, blocks, args):
        ...
        f_aux_loss = torch.tensor(0., device=x.device)
        g_aux_loss = torch.tensor(0., device=x.device)
        ...
        ctx.y = x.detach()
        ctx.blocks = blocks
        return x, f_aux_loss, g_aux_loss
    ...
    def backward(ctx, dy, dl_f, dl_g):
        ...
        for block, kwargs in zip(ctx.blocks[::-1], args[::-1]):
            y, dy = block.backward_pass(y, dy, dl_f, dl_g, **kwargs)
        return dy, None, None
