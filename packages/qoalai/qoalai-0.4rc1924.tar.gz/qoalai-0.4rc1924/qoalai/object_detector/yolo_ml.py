...
class Yolo(ObjectDetector):
    def __init__(self, 
        ...
        if self.is_multilabel: 
            self.output_placeholder1 = tf.placeholder(tf.float32, shape=(None, 13, 13, 3*(5 + num_of_class + 6)))
            self.output_placeholder2 = tf.placeholder(tf.float32, shape=(None, 26, 26, 3*(5 + num_of_class + 6)))
            self.output_placeholder3 = tf.placeholder(tf.float32, shape=(None, 52, 52, 3*(5 + num_of_class + 6)))
        else: 
    ...
    def build_net(self, input_tensor, 
        ...
        with tf.variable_scope('yolo_v3_model'):
            self.build_yolov3_net2(inputs=input_tensor, network_type=network_type, is_training=is_training)
    ...
    def optimize(self, subdivisions, 
        ...
        for i in range(iterations):
            ...
            for j in range (subdivisions):
                ...
                #print (total, obj, noobj, ctr, size, class_l)
                if self.is_multilabel:
                    ...
                    tmp = self.session.run(self.test, feed_dict)
                    #print (tmp)
                    ...
                    for aaa in tmp: 
                        for bbb in aaa: 
                                ...
                                #print ("==============")
                            for ccc in bbb: 
                                if np.sum(ccc) > 0. and np.sum(ccc) < 1.: 
                                    print ("---------", ccc)
                ...
                if self.is_multilabel: 
                    ...
                    print ("> Train sub", j, ': iou: ', round(iou_avg*100, 1), 
                            'obj acc: ', round(obj_acc*100, 1), 'noobj acc: ', round(noobj_acc*100, 1), 
                            'class acc: ', round(class_acc*100, 1))
