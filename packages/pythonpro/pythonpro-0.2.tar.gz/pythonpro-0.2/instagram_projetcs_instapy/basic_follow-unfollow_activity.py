...
with smart_run(session):
    ...
    """
    session.follow_user_followers(['andreavaldirisos', 'ojo_vallenato_oficial', 'eyevallenat0'], amount=10,
    ...
    """
    # session.follow_user_followers(['andreavaldirisos', 'ojo_vallenato_oficial', 'eyevallenat0'], amount=800,
    #                               randomize=False, interact=False)

    """ Second step of Unfollow action - Unfollow not follower users...
    """
    session.unfollow_users(amount=500, instapy_followed_enabled=(True, "nonfollowers"),
                           style="FIFO",
                           unfollow_after=12 * 60 * 60, sleep_delay=601)

    """ Clean all followed user - Unfollow all users followed by InstaPy...
    """
    session.unfollow_users(amount=500, instapy_followed_param=(True, "all"),
