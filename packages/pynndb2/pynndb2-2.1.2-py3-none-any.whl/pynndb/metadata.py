...
class MetaData:
    ...
    def fetch_index(self, table_name: str, index_name: str, txn: TXN=None) -> Doc:
        ...
        """
        return Doc(self._table.deserialise(txn.get(self.path_index(table_name, index_name), db=self._table._db)))
    ...
    def store_index(self, table_name: str, index_name: str, value: Doc, txn: TXN=None) -> None:
        ...
        """
        txn.put(self.path_index(table_name, index_name), self._table.serialise(value.doc), db=self._table._db)
    ...
    def fetch_config(self, table_name: str, txn: TXN=None) -> Doc:
        ...
        raw = txn.get(key, db=self._table._db)
    ...
    def store_config(self, table_name: str, value: Doc, txn: TXN=None) -> None:
        ...
        txn - a transaction to wrap the operation
        """
        txn.put(self.path_config(table_name), self._table.serialise(value.doc), db=self._table._db)
