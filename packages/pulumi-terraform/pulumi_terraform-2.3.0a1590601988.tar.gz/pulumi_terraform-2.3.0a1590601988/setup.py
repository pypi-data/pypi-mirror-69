...
class InstallPluginCommand(install):
    def run(self):
        install.run(self)
        try:
            check_call(['pulumi', 'plugin', 'install', 'resource', 'terraform', 'v2.3.0-alpha.1590601988+gd1feb2e'])
            ...
            if error.errno == errno.ENOENT:
                ...
                `pulumi plugin install resource terraform v2.3.0-alpha.1590601988+gd1feb2e`
                """)
