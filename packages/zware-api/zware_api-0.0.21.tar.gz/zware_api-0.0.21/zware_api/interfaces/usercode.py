...
class zwUserCode(zwInterface):
    ...
    async def get_code(self, user_id, active=False):
    ...
    async def remove_single_code(self, user_id, verify=True):
        ...
        return await self.set_codes([user_id], [str(self.STATUS_UNOCCUPIED)], verify=verify)

    async def disable_single_code(self, user_id, code, verify=True):
        """Set a single code to occupied/disabled status."""
        return await self.set_codes([user_id], [str(self.STATUS_UNOCCUPIED)], codes=[code], verify=verify)
