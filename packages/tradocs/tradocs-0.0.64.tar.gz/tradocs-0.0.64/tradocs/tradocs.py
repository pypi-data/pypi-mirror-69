...
def ProcessFiles(sourceFile):
    ...
    if fileName[-3:] in ('yml', '.md'):
        ...
        for i in range (len(targetLangs)):
            print('\033[92m {}\033[00m'.format('\rTraslating "' + sourceFile + '" from "' + sourceLang + '" to "' + targetLangs[i] + '"'), end='')
            ...
            file.close()
        return
    print('\033[94m {}\033[00m'.format('\rCopying "' + sourceFile + '"'), end='')
    for i in range (len(targetLangs)):
...
def Translate(text, langCombo):
    global reqs
    with click.progressbar() as reqs:
