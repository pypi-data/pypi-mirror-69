...
class CmdSession(object):
    def __init__(self, program, prompt):
        self.child = pexpect.spawn(program, encoding='utf-8', timeout=1)
        self.atprompt = False
        self.prompt = prompt
        self.flush()
...
def run_session(command, debug=False):
...
if __name__ == "__main__":
    cmd = "pyditz -SPHXV -c ui.name=Dilbert,ui.email=dilbert@cubicle.com"
    run_session(cmd, True)
