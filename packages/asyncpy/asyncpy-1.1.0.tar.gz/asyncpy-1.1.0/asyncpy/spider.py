...
# from asyncpy.settings import CONCURRENT_REQUESTS
...
class Spider(SpiderHook):
    ...
    concurrency: int = 20 # CONCURRENT_REQUESTS
    ...
    def __init__(
        ...
        **spider_kwargs,
        ...
        # set logger
        if self.custom_settings and self.custom_settings.get('LOG_FILE'):
    ...
    def start(
        ...
        **spider_kwargs,
        ...
        # if 'MIDDLEWARE' in dir(settings):
        #     mw_list = settings.MIDDLEWARE
        #     if mw_list:
        #         for path in mw_list:
        #             clast = path.split('.')[0]
        #             ret = importlib.import_module(clast)    # 动态导入爬虫模块
        #             middleware = ret.middleware
        #
        # pipelines_list = []
        # if 'PIPELINES' in dir(settings):
        #     pl_list = settings.PIPELINES
        #     if pl_list:
        #         for path in pl_list:
        #             modlt = path.split('.')[0]
        #             clast = path.split('.')[1]
        ...
        #spider_ins = cls(middleware=middleware, loop=loop, **spider_kwargs,pipelines=pipelines_list)
        ...
        while True:
            ...
            if self.request_queue.empty():
                ...
                for task_result in results:
                    ...
                    # if "CLOSESPIDER_TIMEOUT" in dir(settings):
                    #     date1 = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    #     date1 = datetime.strptime(date1, "%Y-%m-%d %H:%M:%S")
                    #     if (date1 - date2).total_seconds()>settings.CLOSESPIDER_TIMEOUT:
                    #         self.logger.warning("<Running time over ClOSE_TIMEOUT,So Spider Closed>")
                    #         break
