...
def outbreak_prediction(data,p,d,q,std=0.02,a=["ARIMA-GRNN","ARIMA"]):
    if a=="ARIMA-GRNN":
...
#            All_prediction_data.append(ARIMA_GRNN_data)
        All_prediction_data=ARIMA_GRNN_Pre(data=data,p=p,d=d,q=q,std=std)
    if a=="ARIMA":
#        outbreak_before=outbreak_index-4
#        outbreak_after=outbreak_index+3
#        All_prediction_data=[]
...
#            All_prediction_data.append(ARIMA_data)
        All_prediction_data=ARIMA_Pre(data=data,p=p,d=d,q=q)
