def here():
    ...
    videoExts = [".avi", ".flv", ".wmv", ".mov", ".mp4", ".webm", ".vob", ".mng",
                 ".qt", ".mpg", ".mpeg", ".3gp"]
    video = [file for file in files if os.path.splitext(
        file)[1].lower() in videoExts]
    print("Total Video files moved = ", len(video))

    audioExts = [".aac", ".aa", ".aac", ".dvf", ".m4a", ".m4b", ".m4p", ".mp3",
                 ".msv", "ogg", "oga", ".raw", ".vox", ".wav", ".wma"]
    audio = [file for file in files if os.path.splitext(
        file)[1].lower() in audioExts]
    print("Total Audio files moved = ", len(audio))

    archiveExts = [".a", ".ar", ".cpio", ".iso", ".tar", ".gz", ".rz", ".7z",
                   ".dmg", ".rar", ".xar", ".zip"]
    archives = [file for file in files if os.path.splitext(
        file)[1].lower() in archiveExts]
    print("Total Archive files moved = ", len(archives))

    exeExts = [".exe"]
    exes = [file for file in files if os.path.splitext(file)[
            1].lower() in exeExts]
    print("Total Exe files moved = ", len(exes))

    pdfExts = [".pdf"]
    pdfs = [file for file in files if os.path.splitext(file)[
            1].lower() in pdfExts]
    print("Total PDF files moved = ", len(pdfs))

    plaintextExts = [".txt", ".in", ".out"]
    plaintexts = [file for file in files if os.path.splitext(
        file)[1].lower() in plaintextExts]
    print("Total Text files moved = ", len(plaintexts))

    htmlExts = [".html5", ".html", ".htm", ".xhtml"]
    htmls = [file for file in files if os.path.splitext(file)[
        1].lower() in htmlExts]
    print("Total HTML files moved = ", len(htmls))

    cppExts = [".cpp"]
    cpps = [file for file in files if os.path.splitext(file)[
            1].lower() in cppExts]
