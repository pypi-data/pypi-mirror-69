...
def merge_with_propositions_loads_then_with_shipments_loads(
    ...
    return (
        ...
        .rename(columns={"hazardous": "loads_hazardous"})
    )
...
def merge_shipments_with_loads(shipments_df: pd.DataFrame, loads_df: pd.DataFrame):
    ...
    shipments_df = shipments_df[(~shipments_df.duplicated(subset=["shipment_id"])) | (shipments_df["shipment_id"].isnull())]  # drop duplicates without dropping nan
    return shipments_df
