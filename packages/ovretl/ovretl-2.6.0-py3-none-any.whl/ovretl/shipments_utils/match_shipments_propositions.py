...
def select_best_proposition_id(shipment_propositions: pd.DataFrame):
    ...
    if len(shipment_propositions[mask_accepted]) > 0:
        return select_column_of_most_recent_entity(shipment_propositions[mask_accepted])
    if len(shipment_propositions[mask_sent]) > 0:
        return select_column_of_most_recent_entity(shipment_propositions[mask_sent])
    if len(shipment_propositions[mask_purchase_ready]) > 0:
        return select_column_of_most_recent_entity(shipment_propositions[mask_purchase_ready])
