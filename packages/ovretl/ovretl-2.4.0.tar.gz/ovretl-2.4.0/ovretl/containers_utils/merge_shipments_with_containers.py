...
def merge_with_propositions_containers_then_with_shipments_containers(
    ...
    shipments_df = pd.merge(
        shipments_df,
        propositions_containers_df[["teus", "tc", "hazardous", "proposition_id"]],
        how="left",
        on="proposition_id",
    )

    shipments_df_with_teus = shipments_df[~shipments_df["teus"].isnull()]
    shipments_df_without_teus = shipments_df[shipments_df["teus"].isnull()].drop(["teus", "tc", "hazardous"], axis=1)

    shipments_df_without_teus = pd.merge(
        shipments_df_without_teus,
        shipments_containers_df[["teus", "tc", "hazardous", "shipment_id"]],
        how="left",
        on="shipment_id",
    )
    return (
        ...
        .rename(columns={"hazardous": "containers_hazardous"})
    )
...
def merge_shipments_with_containers(shipments_df: pd.DataFrame, containers_df: pd.DataFrame):
    ...
    shipments_df = shipments_df.sort_values(by=["teus", "tc"], ascending=False).drop_duplicates(subset=["shipment_id"])
    return shipments_df
