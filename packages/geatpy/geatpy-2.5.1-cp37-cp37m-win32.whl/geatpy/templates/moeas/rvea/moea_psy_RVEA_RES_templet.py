...
参考文献:
    ...
    def reinsertion(self, population, offspring, refPoint):
        ...
        # 得到非支配个体
        [levels, criLevel] = self.ndSort(population.ObjV, None, 1, population.CV, self.problem.maxormins) # 非支配排序，1表示只排序到第一层即非支配个体所在的层级
        population = population[np.where(levels == 1)[0]]
        # 选择个体保留到下一代
        [chooseFlag, ans] = ea.refgselect(population.ObjV, refPoint, self.problem.M * ((self.currentGen + 1) / self.MAXGEN)**self.a, population.CV, maxormins = self.problem.maxormins) # ans表示不使用该返回结果
    ...
    def run(self, prophetPop = None): # prophetPop为先知种群（即包含先验知识的种群）
        ...
        population = population[ea.refselect(population.ObjV, levels, criLevel, NIND, uniformPoint, self.problem.maxormins)] # 根据参考点选择个体
