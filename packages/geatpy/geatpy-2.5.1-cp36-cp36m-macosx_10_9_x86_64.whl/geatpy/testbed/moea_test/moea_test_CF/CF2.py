...
class CF2(ea.Problem): # 继承Problem父类
    ...
    def aimFunc(self, pop): # 目标函数
        ...
        f2 = 1 - np.sqrt(np.abs(x1)) + 2 * np.mean((Vars[:, J2] - np.cos(6 * np.pi * x1 + (J2 + 1) * np.pi / self.Dim))**2, 1, keepdims = True)
        pop.ObjV = np.hstack([f1, f2]) # 把求得的目标函数值赋值给种群pop的ObjV
        tmp = f2 + np.sqrt(np.abs(f1)) - np.sin(2 * np.pi * (np.sqrt(np.abs(f1)) - f2 + 1)) - 1
