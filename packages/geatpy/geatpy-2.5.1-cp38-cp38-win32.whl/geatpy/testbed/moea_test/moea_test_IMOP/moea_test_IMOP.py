...
if __name__ == '__main__':
    ...
    if PF is not None and NDSet.sizes != 0:
        ...
        HV = ea.indicator.HV(NDSet.ObjV, PF, Parallel = True) # 计算HV指标
    ...
    if PF is not None:
        metricName = [['IGD'], ['HV']]
        [NDSet_trace, Metrics] = ea.indicator.moea_tracking(myAlgorithm.pop_trace, PF, metricName, problem.maxormins, Parallel = True)
        # 绘制指标追踪分析图
