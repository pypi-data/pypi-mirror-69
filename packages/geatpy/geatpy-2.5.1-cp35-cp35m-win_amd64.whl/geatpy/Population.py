...
"""
    ...
    def __getitem__(self, index):
        ...
        if type(index) != slice and type(index) != np.ndarray and type(index) != list:
            raise RuntimeError('error in Population: index must be a 1-D array. (index必须是一个一维的向量。)')
        if type(index) == slice:
            NIND = (index.stop - (index.start if index.start is not None else 0)) // (index.step if index.step is not None else 1)
        else:
    ...
    def __add__(self, pop):
        ...
        return Population(self.Encoding, 
                          ...
                          np.vstack([self.FitnV, pop.FitnV]) if self.FitnV is not None and pop.FitnV is not None else None,
