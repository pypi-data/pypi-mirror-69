...
class C3_DTLZ4(ea.Problem): # 继承Problem父类
    ...
    def aimFunc(self, pop): # 目标函数
        ...
        f = np.hstack([np.fliplr(np.cumprod(np.cos(Vars[:,:self.M-1]**alpha * np.pi / 2), 1)), ones_metrix]) * np.hstack([ones_metrix, np.sin(Vars[:, range(self.M - 2, -1, -1)]**alpha * np.pi / 2)]) * (1 + g)
        # 计算违反约束程度矩阵的值
        CV = 1 + 3/4 * f**2 - np.sum(f**2, 1, keepdims = True)
    ...
    def calReferObjV(self): # 设定目标数参考值（本问题目标函数参考值设定为理论最优值，即“真实帕累托前沿点”）
        ...
        referenceObjV /= np.sqrt(np.sum(referenceObjV**2, 1, keepdims = True) - 3 / 4 * np.max(referenceObjV**2, 1, keepdims = True))
