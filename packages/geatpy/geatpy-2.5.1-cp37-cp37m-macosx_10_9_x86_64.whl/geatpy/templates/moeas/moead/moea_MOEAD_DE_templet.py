...
算法描述:
    ...
    稍有不同的是，本算法模板中的差分进化部分严格采用DE/rand/1/bin的模式，
    详见参考文献[2]，而参考文献[1]中采用的是一个非标准的DE。
...
参考文献:
    ...
    [2] Rainer Storn, Siemens Ag, Z F E T Sn, Otto-hahn Ring, and D Muenchen. 
    ...
    def __init__(self, problem, population):
        ...
        if population.Encoding == 'RI':
            self.F = 0.5 # 差分变异的F
            self.recOper = ea.Xovbd(XOVR = 1, Half = True) # 生成二项式分布交叉算子对象，这里的XOVR实际就是二项式分布交叉算法中的Cr
    ...
    def run(self, prophetPop = None): # prophetPop为先知种群（即包含先验知识的种群）
        ...
        while self.terminated(population) == False:
            ...
            for i in range(population.sizes):
                ...
                # 进行进化操作
                offspring.Chrom = pop_Chrom + self.F * (population.Chrom[r1, :] - population.Chrom[r2, :]) # 差分变异
                offspring.Chrom = self.recOper.do(np.vstack([pop_Chrom, offspring.Chrom])) # 重组
