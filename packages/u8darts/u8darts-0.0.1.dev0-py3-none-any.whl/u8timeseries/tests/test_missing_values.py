...
class MissingValuesTestCase(unittest.TestCase):
    ...
    def test_fill_quad(self):
        ...
        self.assertEqual(self.series5, round(auto_fillna(seriesE, interpolate_method='quadratic'), 7))
        self.assertEqual(self.series6, round(auto_fillna(seriesF, first=0, last=-1, interpolate_method='quadratic'), 2))
        # extrapolate values outside
        self.assertEqual(self.series5,
                         round(auto_fillna(seriesF, interpolate_method='quadratic', fill_value='extrapolate'), 2))
