...
from audit_trail.models import (
    Entity, Field, StorageBin, FieldDiff,
    ContentType, Xaction,
    ...
    def update_or_create(self, *args, defaults=None, **kwargs):
        ...
        with transaction.atomic(using=self.db):
            try:
                ...
                if created:
                    prev_val, curr_val = self._get_audit_values(obj,
                                                                {**defaults, **kwargs})
...
class AuditTrail(object):
    def contribute_to_class(self, cls, name):
        ...
        def create_model_fields(sender, **kwargs):
            self.entity, created = Entity.objects.update_or_create(
                entity_name=cls._meta.model_name,
        ...
        def pre_save_handler(sender, instance, **kwargs):
            ...
            if request:
                try:
                    request.session[cache_name] = sender.objects.select_related().get(
                        pk=instance.pk)
        ...
        def m2m_signal_handler(sender, instance, action, reverse, model, pk_set,
                               **kwargs):
            if not reverse and (action == 'post_remove' or action ==
                ...
                xaction_type = 'U'
                audit = CoreAudit(type(instance), instance, xaction_type, prev_val,
                                  curr_val)
...
class CoreAudit(object):
    ...
    def __init__(self, audit_model, obj, xaction_type, prev_val, curr_val, *args,
                 **kwargs):
        with transaction.atomic():
            self.audit_model = audit_model
            self.managed_fields = list(
                filter(lambda x: x.name in self.audit_model._meta.managed_fields,
                       self.audit_model._meta.get_fields()) if hasattr(
                    self.audit_model._meta,
                    'managed_fields') else self.audit_model._meta.get_fields())
    ...
    def make_xaction(self):
        msg_dict = {self.audit_model._meta.model_name: self.obj}
        display_text = self.audit_model._meta.display_format.format(**msg_dict)

        self.xaction = Xaction.objects.create(
            entity=self.entity,
    ...
    def make_field_diff(self, field):
        if hasattr(self.audit_model._meta,
                   'sensitive_fields') and field.name in self.audit_model._meta.sensitive_fields:
            self.curr_val[field.name] = self.prev_val[field.name] = '( Hidden )'
        if self.audit_model._meta.get_field(field.name).choices:
            self.curr_val[field.name] = dict(
                self.audit_model._meta.get_field(field.name).choices) \
                .get(self.curr_val.get(field.name))
            self.prev_val[field.name] = dict(
                self.audit_model._meta.get_field(field.name).choices) \
    ...
    def check_parent(self):
        if hasattr(self.audit_model._meta, 'parent_field'):
            ...
            parent_entity = Entity.objects.get(
                entity_name=self.audit_model._meta.get_field(
                    parent_field).rel.to._meta.model_name)
            child_field, created = Field.objects.get_or_create(name=parent_field,
                                                               entity=self.entity)
            parent_child_mapping, created = ParentEntityChildFieldMap.objects.get_or_create(
            ...
            XactionParentEntityChildFieldMap.objects.create(xaction=self.xaction,
                                                            ...
                                                            field_val=self.__getattribute__(
                                                                parent_field))
