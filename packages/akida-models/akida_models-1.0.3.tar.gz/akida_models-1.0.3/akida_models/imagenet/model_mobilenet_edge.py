...
It comes with a set of pretrained weights (`weights` params set to "edgeready")
on a Mobilenet 0.5 that can be run on Akida hardware.
    ...
    Args:
        ...
        alpha (float): controls the width of the model.
            ...
            * If `alpha` = 1, default number of filters from the paper are used
              at each layer.
        weights (str): one of `None` (random initialization), 'edgeready' for
