...
from ..utils.constants import LOGConstants, CodeType, PluginsParamDictDef
...
class Plugin(object):
    def __init__(self, path_to_def_file, path_def_file_id, data_source, name, user_function_object=None, volume_path=None, code_type="BINOUT"):
        '''
        ...
        if code_type == "BINOUT":
            ...
            try:
                ...
                DataContainer.init_data_source_nodout(madymo_data, self._logger, self._dynasaur_definitions)
            except:
    ...
    def _get_sample_offset(self, param_dict):
        ...
        for key in self._data.keys():
            ...
            elif start_time is not None and end_time == None:
...
class PluginInterface(metaclass=abc.ABCMeta):
    ...
    def calculate(self, param_dict):
        ...
        self.calculate_and_store_results(param_dict_data)
