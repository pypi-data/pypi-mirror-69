...
class Dataset:
	...
	def load(self):
		...
		if self.name=="Covid 19":
			self.load_Covid_19()

		if self.name=="eRisk anx":
			self.load_eRisk_anx()
	...
	def load_pubmed_rct20k(self):
		...
		if not os.path.isdir(self.path_dir):
			...
			wget.download(url_train, out=self.path_dir)
			wget.download(url_test, out=self.path_dir)
			wget.download(url_dev, out=self.path_dir)
		if self.train:
			...
			for row in rows:
				...
				if len(row)==2:
					...
					self.labels_train.append(row[0])
		if self.test:
			path_file=os.path.join(self.path_dir,'test.txt')
			fi = open(path_file, "r")
			rows = fi.readlines()
			for row in rows:
				row=row.split('	')
				if len(row)==2:
					...
					self.labels_test.append(row[0])
		if self.dev:
			path_file=os.path.join(self.path_dir,'dev.txt')
			fi = open(path_file, "r")
			rows = fi.readlines()
			for row in rows:
				row=row.split('	')
	...
	def load_Covid_19(self):
		...
		self.path_dir=os.path.join(self.path,'Covid_19')
		if not os.path.isdir(self.path_dir):
			os.mkdir(self.path_dir)
			url_train = 'https://ai2-semanticscholar-cord-19.s3-us-west-2.amazonaws.com/latest/metadata.csv'
			if self.verbose:
				print("Downloading and extracting Covid 19...")
			out_file=os.path.join(self.path_dir,'metadata.csv')
			wget.download(url_train, out=out_file,bar=bar_progress)

		if self.train:
			path_file=os.path.join(self.path_dir,'metadata.csv')
			fi = open(path_file, "r")
			reader = csv.DictReader(fi, delimiter = ',')
			for row in reader:
				#on conserve les labels unique en multiclass!
				if len(row['source_x'].split(';'))==1:
					self.documents_train.append(row['title']+' '+row['abstract'])
					self.labels_train.append(row['source_x'])
	...
	def load_eRisk_anx(self):
		...
		if not os.path.isdir(self.path_dir):
			...
			url_train = 'https://ai2-semanticscholar-cord-19.s3-us-west-2.amazonaws.com/latest/metadata.csv'
			if self.verbose:
				print("Downloading and extracting eRisk anx...")
			out_file=os.path.join(self.path_dir,'metadata.csv')
			wget.download(url_train, out=out_file,bar=bar_progress)
