...
class Runner:
    ...
    def test_comments(self, config):
        ...
        exec_result = util.execute(['cloc', self.tested_path, "--csv", "--quiet"], self.tmp_dir)
    ...
    def _test_run_executable(self, base_command, config, child_limit=0):
        ...
        timeout = util.ConfigTools.get_timeout(config, EXECUTABLE_TIMEOUT)
        child_limit = max(config.child_process_limit, 0) + child_limit if hasattr(config, "child_process_limit") else child_limit
        memory_limit = util.ConfigTools.get_memory_limit(config, MAX_VIRTUAL_MEMORY)

        result = None
        try:
            result = self._get_results(output_file, base_command, child_limit=child_limit, executable_timeout=timeout, memory_limit=memory_limit)
    ...
    def _get_results(self, output_file, base_command, executable_timeout=EXECUTABLE_TIMEOUT, memory_limit=MAX_VIRTUAL_MEMORY, child_limit=0):
        safety_counter = 0
        skip = 0
        partial_result = []
        known_exceptions = []
        ...
        while safety_counter < 32:
            ...
            exec_result = util.execute(base_command + [output_file.absolute().as_posix(), str(skip), secret_location], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT, env_add=env, child_limit=child_limit)
            ...
            if not result:
                if skip < len(partial_result):
                    known_exceptions[skip] = exec_result.exception
                    skip += 1
                    continue
                else:
                    break
            ...
            if len(partial_result) < len(result):
                ...
                known_exceptions = [None for _ in partial_result]
            else:
        ...
        for r, exception in zip(partial_result, known_exceptions):
            if r["mark"] == "":
                r["mark"] = "0"
    ...
    def _get_exception_message(self, exception):
        ...
        exception_messages = {
            ...
            util.ProcessOutOfMemoryError: "Execution ran out of memory. This could indicate a possible inefficiency or memory leak",
            util.ProcessChildLimitHitError: "Execution interupted because too many child process were started"
        }
