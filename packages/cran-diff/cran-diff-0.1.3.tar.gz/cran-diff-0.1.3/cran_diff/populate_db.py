...
def read_news(package, version):
    ...
    #If news exists, this has been added to the news folder
    news_file = f"./news/{package}_{version}.csv"
    categories = []
    text_list = []
    if os.path.exists(news_file):
        ...
        #No news detected: return empty strings
        categories.append("")
        text_list.append("")
...
def database_insert(session, package, version, date, title, description, url, bugreport, maintainer, imports, suggests, exports, types, functions, arguments, defaults, categories, text):
    ...
    #Determine whether package is new or updated
    num_occurences = (session.query(Packages.id)
            .filter(Packages.name == package).count())
    if num_occurences == 1:
        news_type = "new"
    else:
        news_type = "update"
    for i in range(len(categories)):
        news_info = News(package_id=id_num, type=news_type, category=categories[i], text=text[i])
