...
@aws_cli.command(
    name="aws-setup",
    short_help="Use this to configure your AWS account Access.",
    help="""Use this command to set your the AWS Credentials that Anyscale
will use to access your account. This command creates an IAM role
in your account that Anyscale will assume in order to launch resources
in your account.
    """,
)
@click.option(
...
def anyscale_ssh(session_name: str) -> None:
    ...
    resp = send_json_request(
        "/api/v2/sessions/{}/ssh_key".format(session["id"]), {}, method="POST"
    )
    ...
    resp = send_json_request("/api/v1/sessions/{}/head_ip".format(session["id"]), {})
