...
class ConfigApp(App):
    ...
    def __init__(self, mgr):
        ...
        self.add_argument("-d", "--domain", default="app",
                help="configuration data domain(global, project, app:default)")
    ...
    def perform(self, args):
        ...
        if args.param:
            for item in args.param:
                ...
                if len(param) == 2:
                    self.set_config(param[0], param[1])

                cfg = self.get_config(param[0])
                if isinstance(cfg, appdict):
                    print("sub-items : %s" % ", ".join(cfg.keys()))
