...
class DepartureNode(Node):
    ...
    def update_forward(self, fwd):
        self._pathfwd.update(fwd)

    def add_edge(self, edge, arrivalnode):

        edge._bid = self._num_branches
        self._num_branches += 1

        self._branches.append((edge, arrivalnode))
        ...
        while ready_paths or self._edgeproxy_active or just_arrived:
            ...
            if ready_paths:
                if PY3 and args.multiproc:
                    ...
                    out, fwd = edge.run(self.manager, departure._pathfwd)
