...
def push_asset_to_TW(asset, args):
    ...
    resp = requests.get(asset_url + asset_id + "/" + auth_data, verify=False)
    if resp.status_code != 200:
        logging.info("Creating new asset [%s]", asset_id)
        # Asset does not exist so create one with POST
        resp = requests.post(asset_url + auth_data, json=asset, verify=False)
        if resp.status_code == 200:
        ...
        resp = requests.put(asset_url + asset_id + "/" + auth_data, json=asset, verify=False)
        if resp.status_code == 200:
...
def run_scan(asset_id_list, pj_json, args):
    if args.no_scan is not True:
        ...
        if run_va_scan:
            ...
            resp = requests.post(scan_api_url, json=scan_payload, verify=False)
            if resp.status_code == 200:
