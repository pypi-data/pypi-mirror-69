...
def compute_signals_kpi(data=None, mapping_data=None, print_turnover = False, keep_only_one_per_signal_family = False):
    ...
    for sig in all_signals:
        for transac_cost in [True, False]:
            ...
            freqly_df = signal_utility.reconstitute_signal_perf(data=temp_df, transaction_cost=transac_cost,print_turnover=print_turnover)
            sharpe_strat = metrics.sharpe(freqly_df['perf_return'].dropna(), period=252 * 24 * 60, from_ret=True)
            sharpe_under = metrics.sharpe(freqly_df['close_return'].dropna(), period=252 * 24 * 60, from_ret=True)
            kpis.append(
                {
