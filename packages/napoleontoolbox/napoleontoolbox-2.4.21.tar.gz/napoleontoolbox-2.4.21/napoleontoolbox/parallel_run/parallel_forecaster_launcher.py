...
class ForecasterParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', ss=[10], ns=[21], s_evals = [5], calibration_steps=[1000], optimize_thresholds=[True], signal_idiosyncratic_paramaters=[None], table_max_size = 2000):
        self.args = []
        self.key_args=[]
        self.counter = 1
        self.seed = 0
                    ...
                    for calibration_step in calibration_steps:
                        for optimize_threshold in optimize_thresholds:
                            for signal_type, params  in signal_idiosyncratic_paramaters.items():
                                for param in params:
                                    self.args.append((self,self.seed,n,s,s_eval,calibration_step,optimize_threshold,signal_type,json.dumps(param, sort_keys=True)))
                                    self.key_args.append((self.seed,n,s,s_eval,calibration_step,optimize_threshold,signal_type,json.dumps(param, sort_keys=True)))
                                    self.counter = self.counter + 1
        self.key_args.sort()
        self.counter = 1
        for me_arg in self.key_args:
            continuous_saving_key, two_states_saving_key, three_states_saving_key = self.create_saving_key(*me_arg)
    ...
    def create_saving_key(self,seed, n, s, s_eval, calibration_step, optimize_threshold, signal_type, idios_string):
        ...
        return continuous_saving_key, two_states_saving_key, three_states_saving_key
