...
class Connection(object):
    ...
    def _monitor_work_timeout(self):
        ...
        # 之所以使用True而不是self.app.enabled
        # 假设worker处理进入了死循环，而此时调用了kill -hup master
        # 就会导致 self.app.enabled 为False，从而无法再检测worker处理超时，也无法再reload
        while True:
            time.sleep(1)
    ...
    def _handle(self):
        while self.app.enabled and self.closed():
    ...
    def _handle_request(self, request):
        ...
        # 即使业务要求中断，也要走完after_request
        if not request.interrupted:
