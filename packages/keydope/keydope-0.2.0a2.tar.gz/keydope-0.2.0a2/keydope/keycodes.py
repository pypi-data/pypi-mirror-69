...
# Disable pylint invalid name warnings for enum members in Key.
...
KEY_CODES = set(key.value for key in Key)
...
class Action(enum.Enum):
    ...
    def is_pressed(self) -> bool:
        return self in (Action.PRESS, Action.REPEAT)
...
class KeyAction:
    ...
    def __str__(self):
        return '{} {}'.format(self.key.name.capitalize(),
                              self.action.name.capitalize())

    def __repr__(self):
        return 'KeyAction({}, {})'.format(self.key, self.action)
