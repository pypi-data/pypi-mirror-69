...
if __name__ == "__main__":
    ...
    if DELIVERED_POT or ALL:
        spill_history(wagascianpy.plotting.plotter.BsdPotPlotter(
            output_path=os.path.join(OUTPUT_PATH, "{}bsd_pot_history.pdf".format(OUTPUT_STRING)),
    ...
    if ACCUMULATED_POT or ALL:
        spill_history(wagascianpy.plotting.plotter.WagasciPotPlotter(
            output_path=os.path.join(OUTPUT_PATH, "{}wagasci_pot_history.pdf".format(OUTPUT_STRING)),
            bsd_database=BSD_DATABASE,
            bsd_repository=BSD_FILES_DIR,
            wagasci_database=WAGASCI_DATABASE,
    ...
    if BSD_SPILL or ALL:
        spill_history(wagascianpy.plotting.plotter.BsdSpillPlotter(
            output_path=os.path.join(OUTPUT_PATH, "{}bsd_spill_history.pdf".format(OUTPUT_STRING)),
            bsd_database=BSD_DATABASE,
            bsd_repository=BSD_FILES_DIR,
            wagasci_database=WAGASCI_DATABASE,
            t2krun=T2KRUN,
    ...
    if WAGASCI_SPILL_HISTORY or ALL:
        for detector in wagascianpy.plotting.detector.Topology():
            topology = wagascianpy.plotting.detector.Topology()
            ...
            spill_history(wagascianpy.plotting.plotter.WagasciSpillHistoryPlotter(
                output_path=os.path.join(OUTPUT_PATH, "{}wagasci_spill_history_{}.pdf".format(OUTPUT_STRING,
                                                                                              detector_name)),
                bsd_database=BSD_DATABASE,
                bsd_repository=BSD_FILES_DIR,
                wagasci_database=WAGASCI_DATABASE,
    ...
    if WAGASCI_FIXED_SPILL or ALL:
        for detector in wagascianpy.plotting.detector.Topology():
            ...
            spill_history(wagascianpy.plotting.plotter.WagasciFixedSpillPlotter(
                output_path=os.path.join(OUTPUT_PATH, "{}wagasci_fixed_spill_history_{}.pdf".format(OUTPUT_STRING,
                                                                                                    detector_name)),
                bsd_database=BSD_DATABASE,
                bsd_repository=BSD_FILES_DIR,
                wagasci_database=WAGASCI_DATABASE,
    ...
    if WAGASCI_SPILL_NUMBER or ALL:
        for detector in wagascianpy.plotting.detector.Topology():
            ...
            spill_history(wagascianpy.plotting.plotter.WagasciSpillNumberPlotter(
                output_path=os.path.join(OUTPUT_PATH, "{}wagasci_spill_number_{}.pdf".format(OUTPUT_STRING,
                                                                                             detector_name)),
                bsd_database=BSD_DATABASE,
                bsd_repository=BSD_FILES_DIR,
                wagasci_database=WAGASCI_DATABASE,
