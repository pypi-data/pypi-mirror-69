...
class DKCloudCommandRunner(object):
    ...
    @staticmethod
    def print_node(
            user: str, kitchen_list: List[JSONData], name: str, prefix: str, box_char: str,
            output: List[str]
    ) -> None:
        connector = '─ '
    ...
    @check_api_param_decorator
    def secret_list(
            dk_api: DKCloudAPI, path: Optional[str], global_secrets: bool, kitchen_name: Optional[str]
    ) -> str:
    ...
    @check_api_param_decorator
    def secret_write(
            dk_api: DKCloudAPI, path: str, value: str, kitchen: Optional[str]
    ) -> DKReturnCode:
...
"""
    ...
    @staticmethod
    def _get_recipe_delete_local(
            recipe_status_dict: JSONData, recipe_path: str, delete_local: bool, yes: bool
    ) -> str:
        delete_msg = ''
        if not delete_local:
            return delete_msg
    ...
    @staticmethod
    def _merge_file(
            dk_api: DKCloudAPI, kitchen_name: str, recipe_name: str, recipe_path: str,
            folder_name: str, file_info: JSONData
    ...
    @staticmethod
    def _get_recipe_new(
            dk_api: DKCloudAPI, kitchen: str, recipe_name_param: str, recipe_path: str
    ) -> DKReturnCode:
        rc = DKReturnCode()
        try:
    ...
    @staticmethod
    def is_recipe_status_clean(
            dk_api: DKCloudAPI, kitchen: str, recipe: str, recipe_path_param: Optional[str]
    ) -> bool:
    ...
    @staticmethod
    def _get_all_files(
            dk_api: DKCloudAPI, path: str, b64_encode_binary_files: bool = False
    ) -> JSONData:
        result = {}
    ...
    @check_api_param_decorator
    def update_file(
            dk_api: DKReturnCode, kitchen: Optional[str], recipe_name: Optional[str],
            recipe_dir: Optional[str], message: Optional[str],
    ...
    @check_api_param_decorator
    def add_file(
            dk_api: DKCloudAPI, kitchen: Optional[str], recipe_name: Optional[str], message: Optional[str],
    ...
    def get_compiled_order_run(
            dk_api: DKCloudAPI, kitchen: Optional[str], recipe_name: Optional[str],
    ...
    def get_compiled_file(
            dk_api: DKCloudAPI, kitchen: Optional[str], recipe_name: Optional[str],
            variation_name: Optional[str], file_path: str
    ) -> DKReturnCode:
        ret = DKReturnCode()
        try:
            current_path = os.getcwd()
    ...
    @staticmethod
    def recipe_variation_list(
            dk_api: DKCloudAPI, kitchen: str, recipe_name: str, get_remote: bool
    ) -> DKReturnCode:
        rc = DKReturnCode()
        try:
            if get_remote:
    ...
    def kitchen_merge_preview(
            dk_api: DKCloudAPI, from_kitchen: Optional[str], to_kitchen: Optional[str],
...
where 'file_path' is the path as stated in the Merge Preview Results"""
    ...
    @check_api_param_decorator
    def file_diff(
            dk_api: DKCloudAPI, kitchen: Optional[str], recipe: Optional[str], recipe_dir: str,
    ...
    @check_api_param_decorator
    def file_merge(
            dk_api: DKCloudAPI, file_path: Optional[str], from_kitchen: Optional[str],
    ...
    def delete_one_order(
            dk_api: DKCloudAPI, kitchen: Optional[str], order_id: Optional[str]
    ) -> DKReturnCode:
    ...
    @staticmethod
    @check_api_param_decorator
    def kitchen_settings_update(
            dk_api: DKCloudAPI, kitchen: Optional[str], filepath: Optional[str]
    ) -> DKReturnCode:
