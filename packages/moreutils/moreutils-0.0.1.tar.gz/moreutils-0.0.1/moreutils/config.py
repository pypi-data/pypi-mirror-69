...
and have an optional default, or `.expect(*keys)` which raises an exception if
the config value is not found.
...
class Config:
    ...
    def expect(cls, *keys):
        ...
        data, found = cls._get(keys)
        if not found:
            raise ConfigKeyError(f'cant find config value from keys: {keys!r}')
        return data
