...
class VacBot():
    def __init__(self, user, domain, resource, secret, vacuum, continent, server_address=None, monitor=False, verify_ssl=True):
        ...
        self.thread_build_livemap = threading.Thread(target=self.build_liveMap, daemon=False, name="schedule_thread_buildlivemap")
    ...
    def build_liveMap(self):
        _LOGGER.debug("BuildLiveMap begin")
    ...
    def request_all_statuses(self):
        ...
        if not self.thread_build_livemap.isAlive():
            self.thread_build_livemap = threading.Thread(target=self.build_liveMap, daemon=False, name="schedule_thread_buildlivemap")
            self.thread_build_livemap.start()
