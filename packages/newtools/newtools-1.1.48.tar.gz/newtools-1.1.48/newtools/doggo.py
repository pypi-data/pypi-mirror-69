# (c) 2012-2018 The company formerly known as Dativa, all rights reserved
# -----------------------------------------
...
from newtools.optional_imports import pyarrow, boto3
...
class PandasDoggo:
    ...
    def s3_client(self):
        if not self._s3_client:
            self._s3_client = boto3.Session().client('s3')
    ...
    def save(self, df, path, file_format=None, *args, **kwargs):
        ...
        if file_format == 'csv':
            return self.save_csv(df, path, *args, **kwargs)

        elif file_format in ('parquet', 'pq'):
            return self.save_parquet(df, path, *args, **kwargs)

    def save_csv(self, df, path, *args, **kwargs):
        """Alias for .save(df, format='csv')"""
        fm = FileDoggo(path, mode='wb', client=self.s3_client)
        with fm as f:
            f.write(df.to_csv(None, *args, **kwargs).encode('utf-8'))

    def save_parquet(self, df, path, *args, **kwargs):
...
class FileDoggo:
   ...
   |:     ,:';,'         '         `.   ;`   `-.
    ...
    def __init__(self, path, mode='rb', is_s3=None, client=None, compression=None):
        ...
        if self.is_s3:
            # allows you to specify s3 for paths that don't begin with s3:// for whatever unearthly reason you might want
