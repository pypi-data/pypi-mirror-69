...
class Chk:
    ...
    def validate_config(self):
        ...
        if not os.path.isfile(self.config_path):
            ...
            config.set('main', 'url_w', 'http://wiki.example.com/?q=HOSTNAME')
        ...
        # Load
            ...
            self.url_w = config.get('main', 'url_w', fallback=None)
    ...
    def open_url_w(self):
        ...
        if not self.url_w:
            self.status = f"{Chk.fetch_time()} You need to configure an 'url_w'. See docs."
            return

        url = self.url_w.replace('HOSTNAME', self.target)

        self.status = f"{Chk.fetch_time()} Opening host in website {url}"
        webbrowser.open(url, new=2)

    def execute_command_on_host(self):
        ...
        if not focus_widget:
            return
        self.target = focus_widget.base_widget.widget_list[1].text
        cmd = f"x-terminal-emulator --command='ssh {self.target}' &"
