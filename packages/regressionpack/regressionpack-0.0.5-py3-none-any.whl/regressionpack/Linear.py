...
class Linear(AbstractRegression):
    ...
    def ParmDim(self, value:int):
        ...
        assert value % self.X.ndim != self.FitDim, "Cannot use the same dimension for both ParmDim and FitDim"
        self._ParmDim = value % self.X.ndim
    ...
    def _computeXtXinv(self):
        ...
        """
        order = [x for x in range(self.X.ndim)]
        order[self.FitDim] = self.ParmDim
        order[self.ParmDim] = self.FitDim
        self._Xt = self.X.transpose(order)
        self._XtXinv = MatInv(MatMul(self._Xt, self.X, self.Axis), self.Axis)

    def Fit(self):
        self._computeXtXinv()
        self._Beta = MatMul( MatMul(self._XtXinv, self._Xt, self.Axis), self.Y, self.Axis )
    ...
    def Eval(self, x:np.ndarray):
        ...
        input array x. 
        """
        assert x.ndim == self.X.ndim, "Must match the number of dimensions of the training data"
        assert all([x.shape[i] == self.X.shape[i] for i in range(self.X.ndim) if i != self.FitDim]), "All dimensions other than the fit dimension must match"
