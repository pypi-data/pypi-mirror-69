...
class Calculator:
    ...
    def get_index_pct_data_df(index_id:str='hs300',
                           index_val:str=None,
                           start_date:datetime.date=datetime.date(2015,4,1),
                           end_date:datetime.date=datetime.date(2020,4,1),
                           windows:int=5*242,
        ...
        # load data
        index_val_data = DerivedDataApi().get_index_valuation_develop(index_ids=[index_id], start_date=start_date, end_date=end_date)
    ...
    @staticmethod
    def roll_pct(x, pct_value):
        df = pandas.DataFrame(x)
        df['pct_val'] = df[0].rank(pct=True)
        df['diff'] = abs(df.pct_val - pct_value)
        return df.sort_values('diff').iloc[0][0]
    ...
    def get_index_pct_data(dates:pandas.core.series.Series,
                              ...
                              windows:int,
                              min_periods:int):
        df = pandas.DataFrame(values, index=dates).sort_index()

        df['select_pct'] = df[0].rolling(window= windows, min_periods=min_periods).apply(partial(Calculator.roll_pct, pct_value=pct_value), raw=True)
        return df[['select_pct']]
    ...
    def get_taa_detail(index_id:str='hs300',
        ...
        index_val_data = DerivedDataApi().get_index_valuation_develop(index_ids=[index_id], start_date=start_date, end_date=end_date)
        index_val_data = index_val_data.set_index('datetime').reindex(dts).fillna(method='ffill').reset_index().set_index(['datetime','index_id'])
        
        fake_weight = AssetWeight(**{index_id:1})
        taa_helper = TAAHelper(taa_params=taa_param)
        for dt in dts:
            asset_pct = index_val_data.loc[dt]
            taa_helper.on_price(dt=dt, asset_price=None, cur_saa=fake_weight, asset_pct=asset_pct,select_val={index_id:index_val})
