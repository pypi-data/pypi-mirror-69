...
class Trainer:
	...
	def __init__(
		...
		region='us-east-2',
		image_repo="cloudframedotio/trainers"
		):
		"""
		"""
		self.name = name
	...
	def run_job(
		...
		"""
		"""
		image = "{0}:{1}".format(self.image_repo, self.image_tag)
		name = self.name
		model_result_path = self.model_result_path
		bucket = self.bucket
		...
		commands = [
			...
			"-s={}".format(src_path),
			"-t={}".format(trainer_function),
			"-r={}".format(repo),
			"-b={}".format(bucket),
			"-m={}".format(model_result_path),
			"-n={}".format(model_name),
			"-d={}".format(data),
			"-a={}".format(branch),
			"-g={}".format(target),
		...
		logger.info("The model object will be stored in {0}/{1}/{2}.pkl".format(bucket, model_result_path, model_name))
		self.model_location = "{0}/{1}.pkl".format(self.model_result_path, model_name)
		return