...
def rcc(half1_map, half2_map, kernel_size, norm=False, lig=False, \
    ...
    #write_mrc(halfmapscc * cc_mask,'rcc_hfmaps_smax'+str(kernel_size)+'.mrc',uc,origin)
    write_mrc(fullmapcc * cc_mask,'rcc_fullmap_smax'+str(kernel_size)+'.mrc',uc,origin)
    ...
    if model is not None:
        ...
        if model.endswith(('.pdb','.ent')):
            if model_resol is None:
                ...
                if not norm:
                    ...
                    halffsc,_,_,_,_,_ = emda.fsc.halfmaps_fsc_variance(hf1,hf2,bin_idx,nbin)
                    dist = np.sqrt((halffsc - 0.143)**2)
                    map_resol = res_arr[np.argmin(dist)]
                    print('map will be calculated up to '+ str(map_resol) + 'A')
            else:
        ...
        if norm:
            ...
            mapmodelcc = get_3d_realspmapmodelcorrelation(normfull,normmodel,kern_sphere_soft)
        if not norm:
            print('Calculating model-map correlation...\n')
...
class NormalizedMaps:
    ...
    def get_normdata(self):
        ...
        if self.bin_idx is None:
            self.get_parameters()
        self.binfsc,_,_,_,_,self.e0 = fsc.halfmaps_fsc_variance(self.hf1,self.hf2,
