...
__all__ = ("DEBUG2", "LoggerCreator", "errhelper", "childlogger_helper", "ChildLoggerHelper")
DEBUG2 = 11  # suppress all 3rd part libraries debug
...
class LoggerCreator(object):
    ...
    It configure given logger to add features listed below.
    Given logger is assumed to be the root logger of the program
    Added logging features features:

    - set given level to given logger
    - log in file and stdout with differents formatting
    ...
    def __init__(self,
        ...
        self.logger = logger
        if root:
            LoggerCreator.ROOT_LOGGER = logger
...
class ChildLoggerHelper(object):
    """
    This class provides the same feature of childlogger_helper function,
    but at __init__ level.
...
# functions horthands that you SHOULD use
def errhelper(exc: Exception, logger: logging.Logger, msg=""):
    """
...
def childlogger_helper(childlogger, cls, self=None):
    """
    This helper functions return a sublogger and assign it to given cls and object of self (if given).

    This is intended as shorthands to join 2 or 3 lines into just 1 like those
