...
This module provides some classes to manage and create plugin modules.
See classes documentation for more informations about plugins behaviors.

Content of this module:
    - PluginManager: A class for manage plugins
    - PluginBase: an abstract class that every plugin must inhiterit.
"""
...
class PluginManager(ChildLoggerHelper):
    ...
    loggername = "core.plugin.manager"
    ...
    def load_plugin(self, plugincls: PluginBase, suppress_raise: bool = False):
        ...
        if issubclass(plugincls, parsers.BaseParser) and hasattr(
            ...
            self.logger.debug("Load plugin: %s", plugincls.__name__)
        else:
            ex = errhelper(
                TypeError(
    ...
    def load_module(self, path: str, is_folder=False):
        ...
        try:
            ...
            return
        self.logger.log(11, "Load module from %s", path)
    ...
    def fetch_folder(self,
        ...
        """
        Fetch a folder for finding plugin modules. By default it controls only
