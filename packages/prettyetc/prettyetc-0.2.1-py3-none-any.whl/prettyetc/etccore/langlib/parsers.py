...
See classes documentation for more informations about single class.
...
from . import (ArrayField, BoolField, DictField, Field, FloatField, IntField,
               NameField, RootField, StringField)

__all__ = ("BaseParser", "PrimitiveParser", "DictParser", "StringFieldParser", "BadInput")
...
class BadInput(ChildLoggerHelper, Exception):
    ...
    A default string representation of all of these informations can be found in
...
class BaseParser(ChildLoggerHelper, abc.ABC):
    ...
    In particular a parser process begin with
    an ascii text string, or a stream containing that,
    some parsing will be done and the result must be a
    :class:`~prettyetc.etccore.langlib.root.RootField`
    (or an its subclass) object that represents the given
    configuration text as a :class:`~prettyetc.etccore.langlib.Field`
    tree.
    ...
    def parse_field(self,
        ...
        :raises TypeError: If the given data type is not supported.
            ...
            .. danger::
                ...
                can cause crash or unespected behaviour.
...
class DictParser(PrimitiveParser):  # pylint: disable=W0223
    """
