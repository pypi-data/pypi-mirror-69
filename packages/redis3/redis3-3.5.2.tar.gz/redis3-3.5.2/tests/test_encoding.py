...
from redis3._compat import unichr, unicode
from redis3.connection import Connection
...
class TestEncoding(object):
    @pytest.fixture()
    def r(self, request):
        return _get_client(redis3.Redis, request=request, decode_responses=True)
...
class TestEncodingErrors(object):
    def test_ignore(self, request):
        r = _get_client(redis3.Redis, request=request, decode_responses=True,
...
class TestCommandsAreNotEncoded(object):
    @pytest.fixture()
    def r(self, request):
        return _get_client(redis3.Redis, request=request, encoding='utf-16')
...
class TestInvalidUserInput(object):
    def test_boolean_fails(self, r):
        with pytest.raises(redis3.DataError):
            r.set('a', True)
