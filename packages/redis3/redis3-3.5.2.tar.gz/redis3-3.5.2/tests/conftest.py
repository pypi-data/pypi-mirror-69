...
def _get_info(redis_url):
    client = redis3.Redis.from_url(redis_url)
    info = client.info()
...
@pytest.fixture()
def r(request):
    with _get_client(redis3.Redis, request) as client:
        yield client
...
def mock_cluster_resp_ok(request, **kwargs):
    r = _get_client(redis3.Redis, request, **kwargs)
