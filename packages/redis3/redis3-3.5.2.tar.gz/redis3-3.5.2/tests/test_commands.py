...
from redis3._compat import (unichr, ascii_letters, iteritems, iterkeys,
                            itervalues, long, basestring)
from redis3.client import parse_info
from redis3 import exceptions
...
class TestResponseCallbacks(object):
    ...
    def test_response_callbacks(self, r):
        assert r.response_callbacks == redis3.Redis.RESPONSE_CALLBACKS
        assert id(r.response_callbacks) != id(redis3.Redis.RESPONSE_CALLBACKS)
...
class TestRedisCommands(object):
    def test_command_on_invalid_key_type(self, r):
        r.lpush('a', '1')
        with pytest.raises(redis3.ResponseError):
            r['a']
