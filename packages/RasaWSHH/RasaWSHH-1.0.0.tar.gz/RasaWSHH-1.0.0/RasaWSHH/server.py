...
import RasaWSHH
import RasaWSHH.core.utils
from RasaWSHH.utils.common import raise_warning, arguments_of
import RasaWSHH.utils.endpoints
import RasaWSHH.utils.io
from RasaWSHH import model
...
from RasaWSHH.core.agent import Agent, load_agent
from RasaWSHH.core.brokers.broker import EventBroker
from RasaWSHH.core.channels.channel import (
    CollectingOutputChannel,
    OutputChannel,
    UserMessage,
)
from RasaWSHH.core.domain import InvalidDomain
from RasaWSHH.core.events import Event
from RasaWSHH.core.lock_store import LockStore
from RasaWSHH.core.test import test
from RasaWSHH.core.tracker_store import TrackerStore
from RasaWSHH.core.trackers import DialogueStateTracker, EventVerbosity
from RasaWSHH.core.utils import AvailableEndpoints
from RasaWSHH.nlu.emulators.no_emulator import NoEmulator
from RasaWSHH.nlu.test import run_evaluation
from RasaWSHH.utils.endpoints import EndpointConfig
...
if typing.TYPE_CHECKING:
    ...
    from RasaWSHH.core.processor import MessageProcessor
...
def _create_emulator(mode: Optional[Text]) -> NoEmulator:
    ...
    if mode is None:
        ...
        from RasaWSHH.nlu.emulators.wit import WitEmulator

        return WitEmulator()
    elif mode.lower() == "luis":
        from RasaWSHH.nlu.emulators.luis import LUISEmulator

        return LUISEmulator()
    elif mode.lower() == "dialogflow":
        from RasaWSHH.nlu.emulators.dialogflow import DialogflowEmulator
...
def add_root_route(app: Sanic):
    ...
    async def hello(request: Request):
        ...
        return response.text("Hello from Rasa: " + RasaWSHH.__version__)
...
def create_app(
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text):
        ...
        until_time = RasaWSHH.utils.endpoints.float_arg(request, "until")
    ...
    async def train(request: Request) -> HTTPResponse:
        ...
        RasaWSHH.utils.io.write_text_file(rjs["config"], config_path)

        if "nlu" in rjs:
            nlu_path = os.path.join(temp_dir, "nlu.md")
            RasaWSHH.utils.io.write_text_file(rjs["nlu"], nlu_path)

        if "stories" in rjs:
            stories_path = os.path.join(temp_dir, "stories.md")
            RasaWSHH.utils.io.write_text_file(rjs["stories"], stories_path)

        domain_path = DEFAULT_DOMAIN_PATH
        if "domain" in rjs:
            domain_path = os.path.join(temp_dir, "domain.yml")
            RasaWSHH.utils.io.write_text_file(rjs["domain"], domain_path)
        ...
        try:
            ...
            from RasaWSHH import train as train_model
    ...
    async def evaluate_stories(request: Request):
        ...
        stories = RasaWSHH.utils.io.create_temporary_file(request.body, mode="w+b")
        use_e2e = RasaWSHH.utils.endpoints.bool_arg(request, "e2e", default=False)
    ...
    async def evaluate_intents(request: Request):
        ...
        nlu_data = RasaWSHH.utils.io.create_temporary_file(request.body, mode="w+b")
