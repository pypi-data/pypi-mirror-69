...
DEFAULT_CONFIG = os.path.join(ROOT_DIR, 'conf', 'config.xlsx')
...
class CMS(object):
    ...
    def select_sub_project(self, projectnum):
        """
            子项目列表
        """
    ...
    def select_stage(self, subprojectnum):
        """
            分期查询
        """
    ...
    def select_contract(self, contractno):
        """
            合同查询
        """
    ...
    def select_quotation_product(self, quotationid):
        """
            合同报价查询
        """
    ...
    def select_quoprocess(self, kfquotationproductid):
        """
            工序查询
        """
    ...
    def get_quotation_data(self, contractno):
        ...
        contract = self.select_contract(contractno)
        quotation_products = self.select_quotation_product(contract['quotationid'])

        for quotation in quotation_products:
            pcode = quotation['pcode']

            if not quotation['totalproductdata']:
                continue

            totalproductdata = float(quotation['totalproductdata'] or 0)

            if '上机' not in quotation['processnames']:  # 纯分析数据量设为0
                totalproductdata = 0
            ...
            quotation_data[pcode]['totalproductdata'] = totalproductdata

            # 工序列表
            quoprocesses = self.select_quoprocess(quotation['kfquotationproductid'])
...
def main(**args):
    ...
    if config:
        logger.info('\033[32mConfig Data: \n{}\033[0m'.format(
    ...
    for n, info in enumerate(projects, 1):
        ...
        sub_projects = cms.select_sub_project(info['projectnum'])
        ...
        for sub_info in sub_projects:  # 每个子项目编号为一行
            ...
            stages = cms.select_stage(sub_info['subprojectnum'])
...
def parser_add_cms(parser):
    ...
    parser.add_argument(
        '-c', '--config', help='the config excel [%(default)s]', default=DEFAULT_CONFIG)
    parser.add_argument(
        '-o', '--out', help='the output filename [%(default)s]', default='cms.project.xlsx')
    parser.add_argument(
