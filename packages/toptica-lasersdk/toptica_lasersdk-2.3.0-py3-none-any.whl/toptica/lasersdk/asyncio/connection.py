...
class MonitoringLineProtocol(asyncio.Protocol):
    ...
    def data_received(self, data):
        ...
        while line:
            self._logger.debug('{}:{} - MON RX: {}'.format(self._host, self._monitoring_port, repr(line)))
...
class NetworkConnection(Connection):
    ...
    async def open(self) -> None:
        ...
        self._logger.debug("Opening network connection to '{}:{},{}'".format(self._host, self._command_port, self._monitoring_port))
    ...
    async def close(self) -> None:
        ...
        self._logger.debug("Closing network connection to '{}'".format(self._host))
    ...
    async def write_command_line(self, message: str) -> None:
        ...
        """
        self._logger.debug('{}:{} - CMD TX: {}'.format(self._host, self._command_port, repr(message)))
    ...
    async def read_command_line(self) -> str:
        ...
        try:
            ...
            if result.endswith('\n> '):
                self._logger.debug('{}:{} - CMD RX: "{}"'.format(self._host, self._command_port, result[:-3]))
                return result[:-3]
            return str()
    ...
    async def write_monitoring_line(self, message: str) -> None:
        ...
        """
        self._logger.debug('{}:{} - MON TX: {}'.format(self._host, self._monitoring_port, repr(message)))
...
class SerialConnection(Connection):
    ...
    async def open(self) -> None:
        ...
        """
        try:
            self._logger.debug("Opening serial connection to '{}' with {} baud".format(self._port, self._baudrate))
    ...
    async def close(self) -> None:
        ...
        if self._serial is not None:
            self._logger.debug("Closing serial connection to '{}'".format(self._port))
            self._serial.close()
            self._serial = None
    ...
    async def write_command_line(self, message: str) -> None:
        ...
        """
        self._logger.debug('{} - CMD TX: {}'.format(self._port, repr(message)))
    ...
    async def read_command_line(self) -> str:
        ...
        if result.endswith('\r\n> '):
            self._logger.debug('{} - CMD RX: "{}"'.format(self._port, result[:-4]))
            return result[:-4]

        if result.endswith('\n> '):
            self._logger.debug('{} - CMD RX: "{}"'.format(self._port, result[:-3]))
            return result[:-3]
