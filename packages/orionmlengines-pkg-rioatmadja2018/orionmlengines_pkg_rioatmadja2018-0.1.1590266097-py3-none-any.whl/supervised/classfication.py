...
from utils.constants import DEFAULT_CLF
from utils.ml_tools import get_random_features
...
class Classify_ML(object):
    ...
    def get_dimensions(self, vector) -> tuple:
        """
    ...
    def train_test(self, random_state: int = np.random.randint(100, 150)) -> Dict:
        ...
        if not self.get_dimensions(self.feature_matrix) and not self.get_dimensions(self.response_vector):
            raise ValueError(
