...
IMPORT_HttpOperationError_GROUP = [
    "Http request failed",
    "Could not access the specified path due to permission or read-only properties. Failed to authorize",
    "Failed to create or update the dataset. The dataset has been created or updated concurrently",
    "DataPath object has no file paths, URLs or SQL queries that enable fetching data from the datastore",
    "Authentication failed. Error when attempting to list contents of",
    "The provided path is not valid or the files could not be accessed. No files were found using path",
    "No such device or address",
    "Requested file(s) does not exist or specified credential does not have access to read",
