...
class DataWriter(DataReader):
    ...
    def add_etl_pkey(self):
        """
            Generate a concatination of every column, for each row, casted as
            string and then create an md5 hash representation of that
            concatination. This will act as a primary key, useful for cases
            when pkey is either hard to determine or the column(s) you rely
            on as pkey is null.
        """
        self.data_df['etl_pkey_raw'] = \
            self.data_df.astype(str).values.sum(axis=1)
    ...
    def set_primary_key_variables(self):
        """
            ...
            If auto_set_pkey was passed in, run self.add_etl_pkey().
        ...
        elif self.arg_primary_key_ls == 'auto_set_pkey':
            self._logger.warning(f'Will create/use etl_pkey as primary key, '
                                 f'which is a hash representation of every'
                                 f' column concatinated.')
            self.primary_key_exists = False
            self.arg_primary_key_str = 'etl_pkey'
            self.current_primary_key_str = 'etl_pkey'
            self.add_etl_pkey()
        else:
            self.primary_key_exists = False
