...
class SequenceGenerator(object):
    ...
    def _generate(self, src_tokens, src_lengths, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            probs, avg_attn_scores = self._decode_one(tokens[:, :step + 1], self.model, encoder_out, incremental_state, log_probs=True)
            if step == 0:
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_state, log_probs):
        with torch.no_grad():
            if incremental_state is not None:
                decoder_out = list(model.decoder(tokens, encoder_out, incremental_state=incremental_state))
            else:
    ...
    def get_translations(self, loader, maxlen_a, maxlen_b, use_cuda=False,
                         prefix_size=0, remove_bpe=None, nbest=1, ignore_case=True):
        translations = self.generate_batched_itr(loader,
                                                 maxlen_a=maxlen_a,
                                                 maxlen_b=maxlen_b,
                                                 cuda=use_cuda,
        ...
        for sample_id, src_tokens, target_tokens, hypos in translations:
            ...
            for i, hypo in enumerate(hypos[:min(len(hypos), nbest)]):
                ...
                if has_target and i == 0:
                    ...
                    sys_tok = bleu_tokenize(hypo_str.lower() if ignore_case else hypo_str)
                    ref_tok = bleu_tokenize(target_str.lower() if ignore_case else target_str)
