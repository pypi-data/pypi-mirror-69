...
print("CUDA home is {}".format(CUDA_HOME))
dir_ = "mlbench_core/models/pytorch/transformer/modules/strided_batched_gemm"
strided_batched_gemm = CUDAExtension(
                        name='mlbench_core.models.pytorch.transformer.modules.strided_batched_gemm',
                        sources=[os.path.join(dir_, 'strided_batched_gemm.cpp'), os.path.join(dir_, 'strided_batched_gemm_cuda.cu')],
                        extra_compile_args={
                                'cxx': ['-O2',],
                                'nvcc':['--gpu-architecture=compute_70','--gpu-code=sm_70','-O3','-U__CUDA_NO_HALF_OPERATORS__', '-U__CUDA_NO_HALF_CONVERSIONS__']
                        }
) if CUDA_HOME is not None else None
...
setup(
    ...
    version="2.4.0-dev212",
    zip_safe=False,
    ext_modules=[strided_batched_gemm] if CUDA_HOME is not None else [],
    cmdclass={
