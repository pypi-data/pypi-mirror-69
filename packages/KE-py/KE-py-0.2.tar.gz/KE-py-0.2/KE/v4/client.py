...
class KE4(KE3):
    ...
    def __init__(self, host, port=7070, username='ADMIN', password='KYLIN', debug=False):
        """创建KE4 Client客户端实例
    ...
    def projects(self, name=None, offset=0, size=1000, exact=True):
        ...
        """
        params = {'page_offset': offset, 'page_size': size, 'exact': exact}
        json_obj = self.fetch_json('/projects/', params=params)
        projs_json = json_obj['data']['value']
        ...
        if name:
            ...
            return
        return [Project.from_json(self, json_obj=p) for p in projs_json]
    ...
    def query(self, sql, project, offset=0, limit=100):
        body = {'sql': sql, 'offset': offset, 'limit': limit, 'project': project}
        json_obj = self.fetch_json(uri='/query/', body=body, method='POST')
        return Query.from_json(client=self, json_obj=json_obj['data'])
