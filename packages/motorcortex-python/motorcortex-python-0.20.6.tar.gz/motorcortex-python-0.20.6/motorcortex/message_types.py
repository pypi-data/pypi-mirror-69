    ...
    def encode(self, value):
        if type(value) is str:
            try:
                ...
                print("Failed to encode: %s" % value)
                value = 0
...
class MessageTypes(object):
    ...
    def load(self, proto_hash_pair_list = None):
        ...
        for proto_hash_pair in proto_hash_pair_list:
            with open(proto_hash_pair['hash']) as data_file:
                ...
                if ismodule(proto):
                    ...
                    # ModuleHashListPairContainer(SourceFileLoader(str(count), proto).load_module(), hash))
                else:
                    print("Failed to load module:", proto)
                    return
                count = count + 1
        ...
        for module_hash_pair in module_hash_pair_list:
            for hash_type_pair in module_hash_pair.hash_list:
                ...
                # ClassName2Type = None
                # if hasattr(module._sym_db, '_classes'):
                #     ClassName2Type = module._sym_db._classes
                # elif hasattr(module._sym_db, '_symbols'):
                #     ClassName2Type = module._sym_db._symbols
                # else:
                #     print("Failed to find dictinary with type names!")
                module

                # try:
                #     type = ClassName2Type[hash_type_pair['type']]
                #     setattr(type, "decode_value", 0)
                #     self.__types_by_hash[hash] = type
                # except KeyError:
                #     for msg in ClassName2Type:
                #         type = ClassName2Type[msg]
                #         msg_name = type.DESCRIPTOR.full_name
                #         if (msg_name == hash_type_pair['type']):
                #             setattr(type, "decode_value", 0)
                #             self.__types_by_hash[hash] = type
            ...
            for module_hash in self.__module_hash_pair_list:
                ...
                if module_name == module_hash_pair.module.DESCRIPTOR.package:
                    print ("Module %s already exists, replacing it with a new version" % (module_name))
    ...
    def __loadPrimitives(self, name, object):
        try:
            ...
            print("Loaded types from %s" % (name))
        except:
            pass
    ...
    def __loadEnum(self, enum_name, object, module):
        try:
            ...
            print("Loaded enumerator %s" % (enum_name))
        except:
            pass
