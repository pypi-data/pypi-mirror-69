...
def random_circuit(unitary=False):
    qubits = random.randint(2, _MAX_QUBITS)
...
async def execute_jobs(backend_name, num_circuits, gpu):
    qctic_provider = QCticProvider.from_env()
    qctic_backend = qctic_provider.get_backend(backend_name)

    circuits = [
        random_circuit(unitary=backend_name == QCticUnitarySimulator.NAME)
