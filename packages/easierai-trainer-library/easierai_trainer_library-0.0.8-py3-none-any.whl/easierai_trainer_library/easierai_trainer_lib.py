...
class Trainer:
    ...
    def initialize_config(self, connection_params, config_file_path="./config.ini", eslib=None, classifier=False):
        ...
        self.keras_callbacks = [
            callbacks.EarlyStopping(monitor=constants.VAL_LOSS, min_delta=0, patience=20, verbose=0, mode='auto', baseline=None), 
    ...
    def extract_data_from_elk(self, entity, index_data, time_window_to=None, data_size=20000, query_size=2000):
        """
        ...
        data_size : Max number of elements to extract. Defaults to 20000. 
        ...
        if entity is not None and entity != index_data:
            if self.time_index != '':
                ...
                return pandas.DataFrame(aux).apply(pandas.to_numeric, errors='ignore', axis=0, raw=True)
            else:
                ...
                return self.get_dataset_from_docs(aux)
        else:    
    ...
    def set_up_timeseries_data(self, df, dataset_features, inference_features=None, classifier=False):
        ...
        t = pandas.to_datetime(df[self.time_index])
        delta_df = t.diff().dt.total_seconds()
...
def easier_train(trainer, entities, index_data=None, inference_type=constants.ESTIMATOR):
    ...
    for entity in entities:
        ...
        with tf.Graph().as_default():
            ...
            # -------------------------------------- Data extraction --------------------------------------
            df = trainer.extract_data_from_elk(entity, index_data, time_window_to)
            ...
            metadata['elapsed_time'] = end_time - start_time
            metadata['test'] = {
                'size': x_test.shape[0],
                #'first': x_test[0],
