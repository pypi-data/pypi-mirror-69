...
def get_x_ticks(ticks,width):
    ...
    if isinstance(tick0,datetime.datetime) or isinstance(tick0,np.datetime64):
        ...
        if(dhs.size / len(dhs_set) >3):
            ...
            index1 = pd_times.index.hour.isin(hour_list)
            times_used = times[index1]
            times_strs_used = get_time_str_list(times_used,row=2)
            xtick_labels = []
            k = 0

            for i in range(len(times)):
                if index1[i]:
                    xtick_labels.append(times_strs_used[k])
                    k =k+1
                else:
                    xtick_labels.append("")
            xticks = (times - times[0])/np.timedelta64(1, 'h')/dhs_u0
        else:
            #无规律，需穷举
