...
class padds(_algorithm):
    ...
    def roulette_wheel(self, metric):
        ...
        return np.array(list(range(length)))[levels][0]
    ...
    def calc_initial_pareto_front(self, its):
        ...
        for i in range(its):
            ...
            id, params, model_simulations = self.simulate((list(range(len(self.parameter_current))), self.parameter_current))
...
def crowd_dist(points):
    ...
    indicies = np.array(list(range(length_x)))[levels]
    max_f[indicies] += 1
    ...
    index_column = np.array(list(range(length_y))).reshape((length_y, 1))
    ...
    non_endpointIndx = np.array(list(range(length_y))).reshape((length_y, 1))
...
def dsearchn(x, y):
    ...
    for line in range(y.shape[0]):
        ...
        IDX.append(np.array(list(range(length)))[found_min_dist_ind][0])
    return np.array(IDX)
...
class HVC():
    ...
    def hype_indicator_sampled(self,points, bounds, nrOfSamples):
        ...
        ROWS = np.transpose(np.array([list(range(nrOfSamples))]))
        ...
        Index = np.array(list(range(S.shape[0])))
        Logical = dominated == 1
    ...
    def hv_apprx(self,points):
        ...
        indexis = np.array(list(range(p_xlen))) + 1
    ...
    def hv_exact(self,points):
        ...
        non_endpointIndx = np.array(list(range(p_xlen)))
...
def chc(points):
    ...
    other_facets_ind = np.array(list(range(norm.shape[0])))  # All facets
