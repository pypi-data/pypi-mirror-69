...
class MediaReview:
    ...
    def write(
        ...
        for n in self.notes:
            note = ET.Element(
                "note", {"date": n.date, "author": n.author, "parent": n.parent or ""}
            )
            ...
            for a in n.contents:
                ...
                for attribute, value in getmembers(a):
                    if not attribute.startswith("__"):
                        if attribute == "path_to_image":
                            # Set path_to_image as relative
                            relative_path = folder.joinpath(n.date, Path(value).name)
                            content.set("path_to_image", str(relative_path))
                            # Keep files to copy
                            if include_attached_files:
                                files_to_copy.append((Path(value), relative_path))
                        else:
                            content.set(attribute, str(value))
