...
class MediaReview:
    ...
    def __init__(self, media: Union[str, Path], status: Status = None, notes: List[Note] = None, ):
        """Review of a media.
    ...
    def write(self, path: Union[str, Path] = '', include_attached_files: bool = True) -> Path:
        ...
        review = ET.Element('review', {'media_path': str(self.media), 'media_signature': 'd41d8cd98f00b204e9800998ecf8427e'})
        ...
        files_to_copy = []
        for n in self.notes:
            note = ET.Element('note', {'date': n.date, 'author': n.author, 'parent': n.parent or ''})

            # Contents
            contents = ET.SubElement(note, 'contents')
            for a in n.contents:
                # Dynamically set parameters from members
                content = ET.SubElement(contents, a.__class__.__name__)
                for member in getmembers(a):
                    if not member[0].startswith('__'):
                        if type(member[1]) is Content.Image:
                            content.set('path_to_image', member[1].path_to_image)
                            # Keep files to copy
                            if include_attached_files:
                                files_to_copy.append(Path(member[1].path_to_image))
                        else:
                            content.set(member[0], str(member[1]))
            ET.SubElement(note, 'metadata')
            notes.append(note)

        #   Write to folder
        folder = Path(path).joinpath(f"{self.media.name}.orio")
        if not folder.is_dir():
            folder.mkdir()

        # Write to file
        file = folder.joinpath('review.orio')

        # create a new XML file with the results
        from xml.dom import minidom
        xmlstr = minidom.parseString(ET.tostring(review, encoding="utf-8")).toprettyxml(indent="   ")
        with file.open("w") as f:
            f.write(xmlstr)

        # Copy files if there have to be copied
        for f in files_to_copy:
            files_dir = folder.joinpath('files')
            if not files_dir.is_dir():
                files_dir.mkdir()

            shutil.copy2(f, files_dir.joinpath(f.name))
