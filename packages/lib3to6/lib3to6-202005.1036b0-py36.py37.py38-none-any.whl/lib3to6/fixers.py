...
def _update_index_elts(elts, known_ids):
...
def _update_index(idx, known_ids):
    val = idx.value
...
def _update_annotation_refs(node, attrname, known_ids):
...
def _remove_forward_references(node, known_ref_names):
