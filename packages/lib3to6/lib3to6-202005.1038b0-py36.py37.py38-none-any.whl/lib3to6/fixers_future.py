...
class RemoveUnsupportedFuturesFixer(fb.FixerBase):
    ...
    def __call__(self, cfg, tree):
        ...
        for i, node in enumerate(tree.body):
            is_doc_string = isinstance(node, ast.Expr) and (isinstance(node
                .value, ast.Constant) or isinstance(node.value, ast.Str))
            if is_doc_string:
                continue
            ...
            if not is_future_import:
                break
            new_names = [alias for alias in node.names if alias.name in
                supported_futures]
