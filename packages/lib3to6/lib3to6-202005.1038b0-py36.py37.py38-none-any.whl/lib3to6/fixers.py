        ...
        for arg, default in zip(kwonlyargs, kw_defaults):
            ...
            if default is None:
                node_value = ast.Subscript(value=ast.Name(id=kw_name, ctx=
                    ast.Load()), slice=ast.Index(value=ast.Constant(s=
                    arg_name)), ctx=ast.Load())
                ...
                node_value = ast.Call(func=ast.Attribute(value=ast.Name(id=
                    ...
                    args=[ast.Constant(s=arg_name), default], keywords=[])
        ...
        if isinstance(node, ast.Dict):
            ...
            for kw in node.keywords:
                if kw.arg is None:
                    ...
                    chain_val = ast.Dict(keys=[ast.Constant(s=kw.arg)],
                        values=[kw.value])
                chain_values.append(chain_val)
        else:
    ...
    def visit_ClassDef(self, node):
        ...
        for assign in node.body:
            ...
            if not isinstance(tgt, ast.Name):
                continue
            elts.append(ast.Tuple(elts=[ast.Constant(s=tgt.id), assign.
                annotation], ctx=ast.Load()))
        return ast.Assign(targets=[ast.Name(id=node.name, ctx=ast.Store())],
            value=ast.Call(func=func, args=[ast.Constant(s=node.name), ast.
            List(elts=elts, ctx=ast.Load())], keywords=[]))
