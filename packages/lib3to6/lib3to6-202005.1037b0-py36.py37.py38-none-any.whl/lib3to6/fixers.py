...
def _update_index_elts(elts, local_ref_names, known_ref_names):
    for i in range(len(elts)):
        ...
        if isinstance(elt, ast.Subscript):
            ...
            msg = 'Error fixing forward ref with type {0}'.format(type(elt))
            raise NotImplementedError(msg)


def _update_index(idx, local_ref_names, known_ref_names):
    val = idx.value
    if isinstance(val, ast.Name):
        ...
        msg = 'Error fixing forward ref with type {0}'.format(type(val))
        raise NotImplementedError(msg)
...
def _update_annotation_refs(node, attrname, local_ref_names, known_ref_names):
    anno = getattr(node, attrname)
    is_non_ref_anno = anno is None or isinstance(anno, ast.Constant
        ) or isinstance(anno, ast.NameConstant)
    if is_non_ref_anno:
        return
    if isinstance(anno, ast.Name):
        is_forward_ref = (anno.id in local_ref_names and anno.id not in
            known_ref_names)
        if is_forward_ref:
        ...
        msg = 'Error fixing forward ref with type {0}'.format(type(anno))
        raise NotImplementedError(msg)


def _remove_forward_references(node, local_ref_names, known_ref_names):
    for sub_node in ast.iter_child_nodes(node):
        if isinstance(sub_node, ast.FunctionDef):
            _update_annotation_refs(sub_node, 'returns', local_ref_names,
                known_ref_names)
            for arg in sub_node.args.args:
                _update_annotation_refs(arg, 'annotation', local_ref_names,
                    known_ref_names)
            ...
            if kwarg:
                _update_annotation_refs(kwarg, 'annotation',
                    local_ref_names, known_ref_names)
            vararg = sub_node.args.vararg
            if vararg:
                _update_annotation_refs(vararg, 'annotation',
                    local_ref_names, known_ref_names)
        elif isinstance(sub_node, ast.AnnAssign):
            _update_annotation_refs(sub_node, 'annotation', local_ref_names,
                known_ref_names)
        if hasattr(sub_node, 'body'):
            _remove_forward_references(sub_node, local_ref_names,
                known_ref_names)
