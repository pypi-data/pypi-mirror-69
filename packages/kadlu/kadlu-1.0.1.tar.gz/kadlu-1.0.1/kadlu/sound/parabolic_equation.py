...
def thomson_starter(k0, kz, dz, zs, theta1):
    ...
    psi = np.exp(-1j * np.pi / 4.) * scimath.sqrt(8 * np.pi) * np.sin(kz * zs) / scimath.sqrt(scimath.sqrt(k0**2 - kz**2))
...
def smooth_index_refr(z, zb, n2, n2b, L):
    ...
    """
    return n2 + 0.5 * (n2b - n2) * (1 + np.tanh((np.abs(z) - zb) / L))
...
class TransmissionLoss():
    ...
    def calc(self, source_depth, rec_depth=[.1], 
        vertical=False, aperture=86, progress_bar=True):
        ...
        self._init_output(num_sources=len(source_depth), rec_depth=rec_depth)
        ...
        ax_h = {'source_depth':source_depth, 'receiver_depth':rec_depth, 'azimuthal_axis':q, 'radial_axis':r[1:]}
        if not self._do_vertical: 
            ...
            tl_v = 20 * np.ma.log10(np.abs(self._field_vert))  # OBS: this computation is rather slow
            z = self.grid.z[:int(self.grid.nz/2)]
            ax_v = {'source_depth':source_depth, 'vertical_axis':z, 'radial_axis':r, 'azimuthal_axis':q}
            self.tl_h, self.ax_h = tl_h, ax_h
    ...
    def plot_vert(self, angle=0, source_depth_idx=0, show_bathy=True):
        ...
        tl = self.tl_v[source_depth_idx,:,:,angle_idx]
        # bathymetry
        if show_bathy:
            def bathy_r(r): 
        ...
        fig = plot_transm_loss_vert(tl, z, r, bathy_r)
        return fig
    ...
    def _solve_pe(self, source_depth, rec_depth, aperture, progress_bar, return_field=False):
        ...
        for i in tqdm(range(nr-1), disable = not progress_bar):# PE marching
            ...
            U_R = prop_refr(x=dr, k0=k0, n=n) # refractive propagation
            psi = np.fft.fft(U_R * np.fft.ifft(psi, axis=1), axis=1)  # refractive propagation, full step
            psi = UD * psi # diffractive propagation, half-step
    ...
    def _max_depth(self, bathy, r_max):
        """ Find the maximum depth in the computational domain.
            ...
            Returns:
                : float
                    Maximum depth in meters
        """
        q = np.linspace(0, 2*np.pi, 100)[:,np.newaxis]
        r = np.linspace(0, r_max, 1000)[np.newaxis,:]
        x = np.cos(q) * r
        y = np.sin(q) * r
        return np.max(bathy(x,y))
    ...
    def _save_output(self, step_no, r, psi, sqrt_rho, rec_depth):
        ...
        if step_no > 0:
            ...
            self._field_horiz[:,:,:,step_no] = F * G * np.exp(1j * self.k0 * r) / np.sqrt(r)
        ...
        if self._do_vertical:
            ...
            self._field_vert[:,:,step_no,:] = psi_z[:, :n, :]
            
    def _init_output(self, num_sources, rec_depth):
        ...
        self._field_vert = np.empty(shape=(ns, int(nz/2), nr, nq), dtype=complex)