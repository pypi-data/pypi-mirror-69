...
def kewley_sl_func(**kwargs):
    ...
    """    
    return np.squeeze(_kewley_interp(x=kwargs['freq'], y=kwargs['windspeed']))
...
def source_level(freq, x, y, area, ocean, sl_func):
    ...
    """
    kwargs = {'freq': freq,
              'windspeed': ocean.windspeed_xy(x=x, y=y),
...
def geophony(freq, depth, sl_func=kewley_sl_func, 
    ...
    for i in tqdm(range(N), disable = not progress_bar): 
        ...
        # ocean data
        dlat, dlon = _delta_lat_lon(lat, 1.2*kwargs['propagation_range'])
        s = lat - dlat
        n = lat + dlat
        w = lon - dlon
        e = lon + dlon
        ocean = Ocean(south=s, north=n, west=w, east=e, **kwargs)

        # sound speed
        ss = SoundSpeed(ssp=kwargs['ssp']) if 'ssp' in kwargs.keys() else SoundSpeed(ocean=ocean)
        ...
        if len(z) == 0:
            ...
            tl, ax = transm_loss.calc(source_depth=z, rec_depth=rec_depth, progress_bar=False)
            tl = tl[:,0,:,:] 
