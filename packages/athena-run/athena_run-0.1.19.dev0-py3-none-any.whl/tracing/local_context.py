...
def _print_trace_urls(traces):
    if len(trace["traces"]) > 0:
        for trace in trace["traces"]:
            log.warning("Access the trace for your execution on Athena: %s",
                    "https://athena.ml/trace/{}".format(trace["id"])
...
class LocalContext(object):
    ...
    def __init__(self, trace_id=None, span_id=None, service="Default", dry_run=False, file_output=None):
        if span_id is None:
