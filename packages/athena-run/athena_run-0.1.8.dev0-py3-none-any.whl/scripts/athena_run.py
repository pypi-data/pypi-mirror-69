...
def _upload_file(path):
    ...
    log.info("Uploaded %s to %s", path, url)
...
def main():
    ...
    parser.add_argument(
            "--local", action='store_true', help="Run in local mode",
            required=False
    )
    
    parser.add_argument(
            "--upload", type=str, help="Upload trace file at PATH",
            required=False
    )

    parser.add_argument(
            "--output", type=str, help="Output trace to file at PATH. Implies --local",
            required=False
    )
