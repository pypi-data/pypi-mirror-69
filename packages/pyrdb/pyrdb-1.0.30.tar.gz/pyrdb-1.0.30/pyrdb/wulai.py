...
def db_kk_insert(conn,app_id,data):
    sql = "insert into t_km_kk (Fapp_id,Fkn_id,Fkn_name,Fkk_id,Fkk_name) values('caasb','123','kntest','456','kktest')" % data
    sqlserver.sql_insert(conn,sql)
# 判断标准答是否为全部
def db_is_kk_new(conn,app_id,kn_name,kk_name):
    sql = "select 1 from t_km_kk where Fapp_id ='%s' and Fkn_name='%s' and Fkk_name='%s' " % (app_id,kn_name,kk_name)
    data = sqlserver.sql_select(conn,sql)
    ncount = len(data)
    if ncount >0:
        res = False
...
def db_kk_select(conn,app_id,kn_name,kk_name):
    sql = "select Fapp_id,Fkn_id,Fkn_name,Fkk_id,Fkk_name from t_km_kk where Fapp_id ='%s' and Fkn_name='%s' and Fkk_name='%s' " % (app_id, kn_name, kk_name)
    data = sqlserver.sql_select(conn, sql)
    ncount = len(data)
    if ncount > 0:
        res = data[0]
