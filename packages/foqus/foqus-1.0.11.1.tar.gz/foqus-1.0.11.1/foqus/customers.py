...
def is_user_allowed(user_apikey):
    dbase = PostgreSQL()
    try:
        user = dbase.get_customer_from_apikey(apikey=user_apikey)[0]
    except Exception as e:
    ...
    expiration = dbase.get_expiration_from_apikey(apikey=user_apikey)[0]
    if time.time() > expiration:
        logger.info("User '" + user + "' is not allowed because APIKEY has expired. Expiration date: " + time.ctime(
            int(dbase.get_expiration_from_customer(user)[0])))
        dbase.close_connection()
        return False
    dbase.close_connection()
    return True
