...
import numpy as np
import pandas as pd
from take_forecast.pipelines.tune.nodes import seasonality_test
from take_forecast.pipelines.tune.nodes import stationarity_test
from take_forecast.pipelines.tune.nodes import autocorrelation_test
from take_forecast.pipelines.tune.nodes import seasonal_autocorrelation_test
from take_forecast.pipelines.tune.nodes import model_selection
from take_forecast.pipelines.tune.nodes import decompose_seasonal
from take_forecast.pipelines.tune.nodes import transform_to_stationary
from take_forecast.pipelines.tune.nodes import analyse_autocorrelation
from take_forecast.pipelines.tune.nodes import analyse_seasonal_autocorrelation
from take_forecast.pipelines.tune.nodes import split_train_test
from take_forecast.pipelines.tune.nodes import tune_sarima
...
def test__seasonality_test(ts: TS, threshold: float, s_start: int, s_stop: int):
	...
	assert isinstance(qstats, float)
...
def test__model_selection__non_seasonal(ts: TS, pdq: tp.Tuple[int, int, int], sPQ: tp.Tuple[int, int, int]):
...
def test__model_selection__seasonal(ts: TS, pdq: tp.Tuple[int, int, int], sPQ: tp.Tuple[int, int, int]):
	kwargs = model_selection(ts, pdq, sPQ)
	assert isinstance(kwargs, dict)
	assert len(kwargs) == 2
