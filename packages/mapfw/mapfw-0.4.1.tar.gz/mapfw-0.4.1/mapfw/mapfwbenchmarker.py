    ...
    def run(self, solver: Callable[[Problem], List] = None):
        ...
        for problem_id in self.benchmarks:
            ...
            while self.status["state"] == "RUNNING":
                if self.timeout:
                    solve_func = lambda problem: func_timeout(self.timeout / 1000, self.solver, args=(problem,))
                else:
                    solve_func = lambda problem: self.solver(problem)
                time_func = lambda problem: time_fun(problem,solve_func)

                with Pool() as p:
                    solutions = list(tqdm(p.imap(time_func,self.problems)))
