...
class Trace:
    ...
    def __getattr__(self, item):
        return getattr(self.trace, item)

    def __repr__(self):
        return repr(self.trace)

    def _check_headers(self):
        if len(self.trace.trace_header) != TRACE_HEADER_SIZE:
            raise ValueError("Header not requested")

    def get_source_group_scalar(self):
        self._check_headers()
        return _read_header_short(self.trace.trace_header, TraceField.SourceGroupScalar)

    def get_x(self):
        self._check_headers()
        x = float(_read_header_int(self.trace.trace_header, TraceField.CDP_X))
    ...
    def get_y(self):
        self._check_headers()
        y = float(_read_header_int(self.trace.trace_header, TraceField.CDP_Y))
        scale_factor = self.get_source_group_scalar()
        if scale_factor > 0:
            y = y * scale_factor
        elif scale_factor < 0:
    ...
    def get_inline(self):
        self._check_headers()
        return _read_header_int(self.trace.trace_header, TraceField.INLINE_3D)

    def get_xline(self):
        self._check_headers()
        return _read_header_int(self.trace.trace_header, TraceField.CROSSLINE_3D)
