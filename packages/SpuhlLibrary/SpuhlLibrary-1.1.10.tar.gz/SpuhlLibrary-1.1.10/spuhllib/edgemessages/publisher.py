...
class Publisher:
    ...
    def send_message(self, values, output=None):
        ...
        loop.run_until_complete(self._send_message(message, output_queue))
        loop.close()
        self.last_sent_values = values
        self.datetime_last_message = datetime.now()

    async def _send_message(self, message, output_queue):
        await self.module_client.send_message_to_output(message, output_queue)

    def register_input_listener(self, input_listener):
        """
        Registers a input listener which must be called when a message with the specified input queue name of the
        listener arrives.
