...
#Todo: Check that the size of the drawing canvas is well calculated in the case where only one or two embers are drawn
...
def makember(infile=None, outfile=None, prefcsys='CMYK'):
    ...
    # - - - - - - - - - - - - - - - - - - - - - - -
        ...
        bfparams = ['project_name', 'project_source', 'project_version',
                    'haz_name', 'haz_unit', 'haz_top_value', 'haz_top_valid', 'haz_map_factor', 'haz_map_shift',
                    'leg_title', 'leg_pos']
        for row in rows:
    ...
    if len(egr.getgp('haz_name')) > 0: 
        ...
        c.saveState()
        c.rotate(90)
        c.drawCentredString(ybas, -xbas, egr.getgp('haz_name'))
        c.restoreState()
    ...
    for gbes in glbes:
        ...
        P.wrap(xavlenght, 60 * mm)
        P.drawOn(c, xbas, ybas + be_y + egr.getgp('be_top_y')*0.33)
        ...
        for be in gbes:
            ...
            if egr.getgp('show_confidence') == 'True':
                ...
                # this is how it was done in all known examples so far, but can it be made more general?
                for ilev in np.arange(int(len(be.hazl)/2))*2:
                    if not (emb.isempty(be.conf[ilev])):
                        ...
                        ygap = egr._y * 0.005 # "gap" between lines showing the colour transition ranges
                        ...
                        ychi = ybas + min (egr.scale(be.hazl[ilev+1]) - ygap, egr._y)
    ...
    # drawlegend returns any additional horizontal space which the legend occupied.
