...
class ExonIntronCollection20081130(PolyCollection, LineCollection):
	"""
	2010-4-12
		It became outdated as matplotlib is upgraded. use the new ExonIntronCollection below.
	2008-11-30
	...
	def draw(self, renderer):
		...
		if hasattr(transform, 'freeze'):
			transform.freeze()
		if transoffset and hasattr(transoffset, 'freeze'):
			transoffset.freeze()
...
class ExonIntronCollection(PathPatch):
	"""
	2010-4-12
		ExonIntronCollection has been broke since the new matplotlib (0.99.0).
		The new one has the same interface as ExonIntronCollection
	"""
	def __init__(self, annotated_box_ls, y=0, width=0.001, linewidths = None, box_line_widths = None,
				 colors	= None,
				 antialiaseds = None,
		...
		2010-4-13
			...
			block_type='exon' has the whole "width" as its height
			block_type='intron' has width/2.0 as its height
			box_type = 'exon' or 'intron' and is_translated= 0 or 1. They are optional. Default is box_type ='intron', is_translated=0.
			
			linewidths controls the width of lines connecting exons or the arrow line, just one float number.
			
			argument rotate_xy to specify an anti-clockwise-90-degree rotation.
				default rotate_xy=False, gene models sit on the y-axis, strand direction matching the x-axis.
				if rotate_xy=True, gene models sit on the x-axis, strand direction matching the y-axis.
			if argument picker is True, it'll allow picker_event.
			argument alpha applies to both boxes and lines.
			
			box_line_widths is ignored.
			arguments edgecolors and facecolors in __init__() are ignored.
		"""
		if linewidths is None   :
			linewidths   = (mpl.rcParams['lines.linewidth'], )
		...
		for i in range(no_of_blocks):
			...
			if block_type=='exon':
				if is_translated==1:
					block_height = width/2.0
				else:
					block_height = width/4.0
				vert = [[block_start, block_height+y], [block_start, -block_height+y], [block_end, -block_height+y], [block_end, block_height+y], [block_start, block_height+y]]
				code = [Path.MOVETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.CLOSEPOLY,]
				if rotate_xy:
					vert = [[row[1], row[0]] for row in vert]
				vert_ls.extend(vert)
				code_ls.extend(code)
			else:
				vert = [(block_start, y), (block_end,y)]
				if rotate_xy:
					vert = [[row[1], row[0]] for row in vert]
		...
		if strand is not None:
			...
			if strand==1 or strand=='plus':
				...
				arrow_offset = 0
				arrow_end = box_end
			vert = [(arrow_end+arrow_offset, y+width), (arrow_end, y+width), (arrow_end,y+width/4.0),(arrow_end,y+width/4.0)]
			if rotate_xy:
				vert = [[row[1], row[0]] for row in vert]
			code = [Path.MOVETO, Path.CURVE4, Path.CURVE4, Path.LINETO,]
			vert_ls.extend(vert)
			code_ls.extend(code)
		from matplotlib import path
		barpath = Path(vert_ls, code_ls)
		PathPatch.__init__(self, barpath, facecolor='red', edgecolor='gray', alpha=alpha, picker=picker, linewidth=linewidths)
