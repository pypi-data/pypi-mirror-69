...
format. The plot() in Another submodule doesn't have the parameter, this the
most difference between the two sub modules. If it's pandas.Dataframe format
we convert it to string by calling df.to_csv(sep = ' ') in the module::
...
further call::
    ...
    sub1 = pyplot.make_subplot(df.to_csv(sep = ' '),
...
.. code-block:: python
    ...
    sub2 = pyplot.make_subplot(df.to_csv(sep = ' '),
