...
class Binomial(Distribution):
    ...
    def pdf(self, k):
        ...
        desriable_probability=pow(self.p, k)
        intermediate_undersiable=1-self.p
        underiable_power=self.n-k
        undesirable_probability=pow(intermediate_undersiable, underiable_power)
        num=math.factorial(self.n)/(math.factorial(k) * math.factorial(self.n-k)) 
        pdf=num * desriable_probability * undesirable_probability
        return pdf       
