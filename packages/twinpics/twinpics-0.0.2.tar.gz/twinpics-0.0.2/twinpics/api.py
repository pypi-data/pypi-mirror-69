...
#-------------------------------------Request to extract metadata for each user-------------------------------#

def body_users(user_list):
    len_user_list = len(user_list)
    body_tweets = json.dumps({
	  "screen_names": user_list,
	  "item_limit": len_user_list,
	  "show_params": [
		"created_at",
		  "default_profile",
...
def evaluate_NLP_profiles(df):
	for i in range(len(df)):
		tweets = df.iloc[i]["text_tweets_days_list"]
