...
def NLP_analysis(data, keyword_corpus, hashtag_corpus):
	...
	if len(tweets_df[tweets_df["terrorist_suggestion"]==1]) == 0:
		...
		if len(tweets_df[tweets_df["terrorist_suggestion"]==2]) == 0:
			...
			print(tweets_df.groupby("terrorist_suggestion").size())
			if len(tweets_df[tweets_df["terrorist_suggestion"]==1]) == 0:
	...
	if(data["per_terr_hash_key"] >0.5) & (data["per_hash_key"]>0.15):
		data["suggestion"]="Terrorist"
	return data,tweets_df
	
def more_tweets_required(df,df_graph_features,consumerKey,consumerSecret,accessToken,accessTokenSecret):
