...
def evaluate_NLP_profiles(df):
	df_evaluate = df[(df["evaluate_NLP"] !="No") & df["evaluate_NLP"] !="More Data"]
	node_list_moreData = list(df_all_features[df_all_features["evaluate_NLP"]=="More Data"])
	if len(node_list_moreData) >0:
		df_metadata_moreData, list_more_Data_noData = sm.user_timeline_extraction(node_list_moreData,consumerKey,consumerSecret,accessToken,accessTokenSecret,5,"fichero_guardar_noData.csv")
		df_metadata_moreData = select_possible_fakes(df_metadata_moreData)
	...
	for i in range(len(df_evaluate)):
		tweets = df_evaluate.iloc[i]["text_tweets_days_list"]
		tweets_join = [j for i in tweets for j in i]
		tweets_df = pd.DataFrame(tweets_join)
		tweets_df.to_csv("tweets_profile_example.csv",index=False)
	return df_evaluate