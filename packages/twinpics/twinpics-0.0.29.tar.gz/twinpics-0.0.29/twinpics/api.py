...
def NLP_analysis(data, keyword_corpus, hashtag_corpus):
	...
	data["per_hash_key"] = (len(tweets_df[tweets_df["terrorist_suggestion"]==1]) / len(tweets_df)).replace([np.inf, -np.inf], np.nan).fillna(0)
	tweets_df["terrorist_suggestion"][(tweets_df["terrorist_suggestion"]==1) & (tweets_df["polarity"] < -0.1) & (tweets_df["subjectivity"]>0.6)]= 2
	data["per_terr_hash_key"] = (len(tweets_df[tweets_df["terrorist_suggestion"]==2]) / len(tweets_df[tweets_df["terrorist_suggestion"]==1])).replace([np.inf, -np.inf], np.nan).fillna(0)
