        ...
        for topic in frames[isograd]:
            ...
            for name, frame in frames[isograd][topic].items():
                ...
                if method == "STA":
                    ...
                    sta_frame.to_csv(sta_path, index=False)
                    dsc_frame.to_csv(dsc_path, index=False)
...
def gather_meta(topic, name, method, isograd, wavelengths, path, frame):
    ...
    if method == "IR":
        meta_frame = ir_meta(topic, name, isograd, wavelengths, path, frame)
    if method == "MS":
...
def ir_meta(topic, name, isograd, wavelengths, path, frame):
    meta_frame = base_meta_frame(isograd, name, path, topic)
...
# data_set = [(k, *t) for k, v in tracer.items() for t in v]
