...
def mpsd2rts(f,mpsd,seed=None,time_shift=0.):
    ...
    # fourier spectrum
    tf = np.zeros(n,dtype=np.complex)
...
##        if (True):
        ...
        if(verbose):
            print (("h1 [ppm2/muHz], tau1 [s], c1, h2 , tau2 , c2: %g, %g, %g, %g, %g, %g, %g") % (h1,tau1*1e6,c1,h2,tau2*1e6,c2,taue))
...
def osc_spectrum(starID, f,modes,numax,deltanu,teff,verbose=False,a=0.,b=1.,plot=False,win=0, rot_period_sur = -1., incl = 0., path='./'):
...
### !!! A ADAPTER POUR LES MS !!!
    ...
    for i in range(nm): # loop over the modes (n,l)
        ...
        ell = int(modes[i,1])
        if (rot_period_sur > 0):
                nlm = 2*ell + 1 # total numver of the multiplets 
                lm1 = -ell
                lm2 = ell
        else:
                nlm = 1
...
def gen_adipls(starID, filename, teff, dt,T, mag , seed = None, pn_ref = 7.7, wn_ref = 0. , mag_ref = 6.,verbose = False, GST = 1 , a=0 , b=1.  ,  plot=False, rot_period_sur = -1., incl = 0., activity = None, granulation = True , path= './' , oscillation=True):
    '''

    (time,lc,nu,pds,mpds,par,seed) = gen_adipls( starID, filename, teff, dt,T, mag , seed = None, pn_ref = 7.7, wn_ref = 0. , mag_ref = 6., verbose = False, GST = 1 , a=0 , b=1.  ,  plot=False) 
    
    Generate a simulated light-curve using eigenfrencies computed with ADIPLS

    INPUTS:

    starID : any number to identify the simulation, this is used only for the plots and the generated output files 
    filename: name of the grand summary file (.gsm) generated by ADIPLS
...
##    deltanu = deltanusun * (mass/radius**3)
    ...
    OS,(Amax,Hmax,Gamma,width_env),modespar = osc_spectrum(starID, f,modes,numax,deltanu,teff,verbose=verbose,a=a,b=b, plot=plot, rot_period_sur = rot_period_sur , incl = incl , path = path )
    ...
    if activity is None:
        AS = 0.
        ASf = 0.
        activity_par = {'sigma': 0., 'tau': 0.}
    else:
        AS = activity_spectrum(f,activity,verbose=verbose)
        ASf = activity_spectrum(2.*nyq-f,activity)
        activity_par = {'sigma': activity[0], 'tau': activity[1]}
        
    # mean power spectrum [ppm2/Hz]
    mpsd =  ( (OS * (b2m**2) * ( (oscillation == True)*1.) + GS  + GSf + AS + ASf ) * 1e6  *   (np.sinc(0.5*f/nyq))**2   +  2.*W**2 )  # [ppm^2/Hz] 
...
##    osc_par = (numax,deltanu,Amax,Hmax,Gamma,width_env,a,b,c)
    ...
    return time,ts,f,ps*1e-6,mpsd*1e-6,opar,seed


def gen_up(starID, numax,dt,T, mag , delta_nu =  -1. , mass = -1. , seed = None, pn_ref = 7.7, wn_ref= 0., mag_ref = 6.,verbose = False, teff = 4800., DPI = -1.,  q = 0. , GST = 0 , incl = -1 , rot_f = 0., path= '', granulation = True , oscillation=True):
    '''
    Generate a simulated light-curve on the basis of the Universal Pattern
    ...
    # mean power spectrum [ppm2/Hz]
    mpsd =   (UPm * (b2m**2) * ( (oscillation == True)*1.)  + GS  + GSf  ) * 1e6 *(np.sinc(0.5*f/nyq))**2   +  2.*W**2   #  
...
def ExtractSystematicDataMagRange(filename,Vp,version=1,DriftLevel='medium',Verbose=False):
    '''
...
def SimSystematicError(Sampling,N,Data,seed=None,full=None,time_shift=0.,version=1):
    ...
    t = np.arange(N,dtype=np.float)*Sampling # time in seconds
