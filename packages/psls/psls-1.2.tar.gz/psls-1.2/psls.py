...
def usage():
    ...
    print ("--extended-plots : an extended set of plots are displayed")
...
if (UP):
    ...
    time,ts,f,ps,mps_nf,opar,_ =  sls.gen_up(StarID, numax,Sampling,Duration, StarVMag,delta_nu =  delta_nu, mass = -1. , seed =seeds[0] , pn_ref = 0.,  wn_ref= 0., mag_ref = 6.,verbose = Verbose, teff = StarTeff, DPI = DPI,  q = q , GST = 1 , incl = Star['Inclination'] , rot_f = Star['CoreRotationFreq'] , path = OutDir  , granulation = (Granulation['Enable'] == 1)  , oscillation = OscEnable)
...
if (Systematics['Enable']):
    if(SystematicDataVersion>0):
        ...
        DriftLevel = None
    DataSystematic = sls.ExtractSystematicDataMagRange(Systematics['Table'],StarVpMag,version=SystematicDataVersion,DriftLevel=DriftLevel,Verbose=Verbose) 
    ...
    for i in range(NGroup):
        ...
        for j in range(NCamera): 
            ...
            if(DataSystematic is not None):
                ...
                resLC,rawLCvar,_,_, tmp2= sls.SimSystematicError(Sampling,nt,DataSystematic,seed=seeds[i*NCamera+1+j],full=True,version=SystematicDataVersion)
                resLC = (resLC-1.)
                full_ts[:,i,j,4] = tmp2
                cumul += resLC 
                tsSC *= (1. + resLC)
