...
def get_changed_models():
    ...
    models = [model for model in models if get_new_fields(model)]
    return models
...
def model_sync(data, model):
    ...
    for item in data:
        is_deleted = item.get('is_deleted') or item.get('is_delete')

        has_is_deleted = bool(getattr(model, 'is_deleted', False))
        has_is_delete = bool(getattr(model, 'is_delete', False))

        if not (has_is_deleted or has_is_delete):
            if is_deleted:
