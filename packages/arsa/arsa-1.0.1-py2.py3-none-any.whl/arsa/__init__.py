...
class APublicKey(object):
    ...
    def get_public_key(self) -> _rsa.RsaKey:
        ...
        :return: RSAKey类型公钥对象
        """
        return _rsa.importKey(self.__public_key)
    ...
    @staticmethod
    def import_public_key(public_key: str, key_length: int):
...
class APrivateKey(object):
    ...
    @staticmethod
    def import_private_key(private_key: str, key_length: int):
        return APrivateKey(_rsa.import_key(
            b'-----BEGIN RSA PRIVATE KEY-----\n' + private_key.encode() + b'-----END RSA PRIVATE KEY-----'), key_length)
