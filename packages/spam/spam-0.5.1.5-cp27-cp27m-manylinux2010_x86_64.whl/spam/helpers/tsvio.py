...
def readStrainTSV(fileName):
    ...
    -------
        Dictionary containing:
            fieldDims: nx3 array of n points coordinates (ZYX)

            volStrain: nx3 array of n points with volumetric strain computed under the hypotesis of large strains (if computed)

            devStrain: nx3 array of n points with deviatoric strain computed under the hypotesis of large strains (if computed)

            vollss: nx3 array of n points with volumetric strain computed under the hypotesis of small strains (if computed)

            devss: nx3 array of n points with deviatoric strain computed under the hypotesis of small strains (if computed)

            rz, ry, rx: nx3 array of n points with the components of the rotation vector (if computed)

            zz, zy, zx: nx3 array of n points with the components of the zoom vector (if computed)

            Uzz, Uzy, Uzx, Uyy, Uyx, Uxx: nx3 array of n points with the components of the right-hand stretch tensor (if computed)

            ezz, ezy, ezx, eyy, eyx, exx: nx3 array of n points with the components of the strain tensor in small strains (if computed)
    ...
    if 'rz' in keys:
        rz = numpy.zeros((nPoints, 1))
        ry = numpy.zeros((nPoints, 1))
        rx = numpy.zeros((nPoints, 1))
        rz[:, 0] = f['rz']
        ry[:, 0] = f['ry']
        rx[:, 0] = f['rx']
        output['rz'] = rz
    ...
    if 'zz' in keys:
        zz = numpy.zeros((nPoints, 1))
        zy = numpy.zeros((nPoints, 1))
        zx = numpy.zeros((nPoints, 1))
        zz[:, 0] = f['zz']
        zy[:, 0] = f['zy']
        zx[:, 0] = f['zx']
        output['zz'] = zz
    ...
    if 'Uzz' in keys:
        Uzz = numpy.zeros((nPoints, 1))
        Uzy = numpy.zeros((nPoints, 1))
        Uzx = numpy.zeros((nPoints, 1))
        Uyy = numpy.zeros((nPoints, 1))
        Uyx = numpy.zeros((nPoints, 1))
        Uxx = numpy.zeros((nPoints, 1))
        Uzz[:,0] = f['Uzz']
        Uzy[:,0] = f['Uzy']
        Uzx[:,0] = f['Uzx']
        Uyy[:,0] = f['Uyy']
    ...
    if 'ezz' in keys:
        ezz = numpy.zeros((nPoints, 1))
        ezy = numpy.zeros((nPoints, 1))
        ezx = numpy.zeros((nPoints, 1))
        eyy = numpy.zeros((nPoints, 1))
        eyx = numpy.zeros((nPoints, 1))
        exx = numpy.zeros((nPoints, 1))
        ezz[:,0] = f['ezz']
        ezy[:,0] = f['ezy']
        ezx[:,0] = f['ezx']
        eyy[:,0] = f['eyy']
