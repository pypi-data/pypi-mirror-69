...
class ereg(QWidget):
    def __init__(self, _images, enterPhi, binning, names):
        ...
        self.iterations = 0
        self.images[1] = spam.DIC.applyPhi(self.images[2], Phi=self.Phis[1])
    ...
    #     spam.deformation.computePhi(transformation)
    #
    #     # modify phi that has to be applied then apply it
    #     self.Phis[1]   = spam.deformation.computePhi(transformation)
    #     self.images[1] = spam.DIC.applyPhi(self.images[2], Phi=self.Phis[1])
    ...
    def saveTSV(self):
        ...
        if self.nameEntry.text() != "":
            ...
            TSVheader = "Zpos\tYpos\tXpos\tF11\tF12\tF13\tZdisp\tF21\tF22\tF23\tYdisp\tF31\tF32\tF33\tXdisp\tbin\treturnStatus"
            centre = (numpy.array(self.images[1].shape)-1)/2.0
            slices = numpy.array([[centre[0]], [centre[1]], [centre[2]],
                                  [self.Phis[1][0, 0]], [self.Phis[1][0, 1]],
    ...
    def reset(self):
        ...
        self.images[1] = spam.DIC.applyPhi(
...
class QtCropWidget(QWidget):
    def __init__(self, images, phi, crop, binning, names):
        ...
        self.binning = binning
        self.indexImDisp = 0
        self.images[0] = spam.DIC.applyPhi(
...
class FinalStep(QWidget):
    ...
    def starCorrelation(self):
        ...
        ### 2019-09-25 EA: Warining, this has changed a lot, now it's imF which is moving consistent with
        ...
        while (iterationNumber <= maxIterations and deltaPhiNorm > minDeltaPhi) or iterationNumber <= 1:
            ...
            # Compute operators
            DICToolkit.computeDICoperatorsGM(imF[margin].astype("<f4"),
                                             imGdef[margin].astype("<f4"),
                                             imGdefGradZ[margin].astype("<f4"),
                                             imGdefGradY[margin].astype("<f4"),
                                             imGdefGradX[margin].astype("<f4"),
            ...
            currentTransformation = spam.deformation.decomposePhi(phiCrop)

            # transform imF
            imGdef = spam.DIC.applyPhi(imG, Phi=phiCrop, interpolationOrder=1)

            # compute residuals
            residualField = numpy.zeros_like(imG[margin], dtype="<f4")
            phaseField = numpy.zeros_like(imG[margin], dtype="<u1")
            DICToolkit.computeGMresidualAndPhase(imF[margin].astype("<f4"),
                                                 imGdef[margin].astype("<f4"),
                                                 phaseDiagram.astype("<u1"),
            ...
            p, _, _ = numpy.histogram2d(imF[margin].ravel(), imGdef[margin].ravel(), bins=BINS, range=[[0.0, BINS], [0.0, BINS]], normed=False, weights=None)
            LL = numpy.sum(numpy.log(p[numpy.where(p > 0)]))
            ...
            self.viewerIm1.setImage(qimage2ndarray.array2qimage(imF[margin][middleSlice, :, :], normalize=True))
            self.viewerIm2.setImage(qimage2ndarray.array2qimage(imGdef[margin][middleSlice, :, :], normalize=True))
            self.viewerCheckerBoard.setImage(qimage2ndarray.array2qimage(cGM.checkerBoard(imF[margin][middleSlice, :, :], imGdef[margin][middleSlice, :, :]), normalize=True))
            #
        ...
        # 2019-09-25: Since the world is all backwards and the rest of the interfact deforms im1, flip PhiCrop here
        phiCrop = numpy.linalg.inv(phiCrop)
        ...
        phiCropInitialCentre[:3, -1] = spam.deformation.decomposePhi(phiCrop, PhiCentre=imCentre, PhiPoint=cropCentre).get('t', [0., 0., 0.])
        ...
        for k, v in spam.deformation.decomposePhi(phiTotal).items():
        ...
        im1Def = spam.DIC.applyPhi(self.images[0], Phi=phiTotal)
        ...
        self.result = {'transformation': spam.deformation.decomposePhi(phiTotal),
                       'Phi': phiTotal,
                       # 'PhiCentre': PhiCentre,
    ...
    def saveFinalResults(self):
        if self.nameEntry.text() != "":
            ...
            Phi = self.result['Phi']
            TSVheader = "Zpos\tYpos\tXpos\tF11\tF12\tF13\tZdisp\tF21\tF22\tF23\tYdisp\tF31\tF32\tF33\tXdisp\tbin"
            centre = (numpy.array(self.images[1].shape)-1)/2.0
            slices = numpy.array([[centre[0]], [centre[1]], [centre[2]],
