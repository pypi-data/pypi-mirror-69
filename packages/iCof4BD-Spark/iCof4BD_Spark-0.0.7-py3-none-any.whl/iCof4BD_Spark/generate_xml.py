...
def generate_xml(web_statics, slaves):
    ...
    for index, stage in enumerate(list(reversed(web_statics.stages_info_unskipped))):
        ...
        if (len(stage['parent_stages'])) | (
                (index > 0) & (
                stage['Job_id'] != list(reversed(web_statics.stages_info_unskipped))[index - 1]['Job_id'])):
        ...
        if len(stage['parent_stages']):
                            ...
                            if type(parent_real_stage['equal_stage']) is list:
                                if ParentsStageID.text:
                                    ParentsStageID.text = ParentsStageID.text + ',' + '1' + str(
                                        "%03d" % int(stage['stage_id']))
                                    InputRDDID.text = InputRDDID.text + ',' + '1' + str(
                                        "%03d" % int(stage['stage_id']))
                                    InputRDDPartitionNum.text = InputRDDPartitionNum.text + ',' + stage['Task_Raw']

                                else:
                                    ParentsStageID.text = '1' + str("%03d" % int(stage['stage_id']))
            ...
            if (index > 0) & (
                ...
                ParentsStageID.text = '1' + str(
                    "%03d" % int(list(reversed(web_statics.stages_info_unskipped))[index - 1]['stage_id']))
