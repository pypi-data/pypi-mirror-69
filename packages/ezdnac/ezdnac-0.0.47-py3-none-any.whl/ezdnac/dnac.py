...
class DnaC():
    ...
    def auth(self):
        if self.authToken is None:
            ...
            AuthURL = "https://" + self.ip + ":" + self.port + self.authBaseUrl + "auth/token"
            headers = {}
            try:
                response = requests.post(AuthURL, headers=headers, verify=self.verifySSL, auth=(self.uid, self.pw), timeout=self.timeout)
            except:
    ...
    def getSites(self, **kwargs):
        ...
        response = requests.request("GET", url, headers=headers, data=payload, verify=self.verifySSL, timeout=self.timeout)
        data = json.loads(response.text)
    ...
    def pullTemplates(self, **kwargs):
        ...
        response = requests.request("GET", url, headers=headers, verify=self.verifySSL, timeout=self.timeout)
        data = json.loads(response.text)
        templateslist = []
        ...
        for template in templatesList:
            ...
            with open(template['paramsFile']) as templateData:
                ...
                if projectExists == False:
                    ...
                    response = requests.request("POST", url, headers=headers, json=payload, verify=self.verifySSL, timeout=self.timeout)
                    data = json.loads(response.text)
                ...
                #Since the template exists, PUT the file from directory to make sure the active template is same version.
                    ...
                    requests.post(versionUrl, headers=headers, json=versionPayload, verify=self.verifySSL, timeout=self.timeout)
                    ...
                    response = requests.request("PUT", url, headers=headers, json=payload, verify=self.verifySSL, timeout=self.timeout)
                    data = json.loads(response.text)
