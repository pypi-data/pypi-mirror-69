...
def case_sync(case: dict, ukw: list) -> Union[dict, bool]:
    ...
    if r == '创建案例':
        ...
        case['role_order_reviewer_id'] = None
...
def order_review():
    ...
    for case in case_db.find():
        ...
        text += ' 销售：' + (sales_worker['name'] if customer else '无')
        text += ' 设计：' + (product_designer['name'] if customer else '无')
        text += ' 评审：' + (order_reviewer['name'] if customer else '无')
...
if __name__ == '__main__':
    ...
    vmi.app.setApplicationVersion('1.1.0')
