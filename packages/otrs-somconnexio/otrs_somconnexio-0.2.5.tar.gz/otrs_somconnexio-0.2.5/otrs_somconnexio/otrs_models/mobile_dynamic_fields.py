...
class MobileDynamicFields():
    ...
    def _df_previous_provider(self):
        if not self.eticom_contract.mobile_telecom_company:
            return DynamicField(name="operadorDonantMobil", value='None')
        key = self.eticom_contract.mobile_telecom_company.name
        return DynamicField(
            name="operadorDonantMobil",
            value=ServiceMappingTelecomCompanies.telecom_company(key))
