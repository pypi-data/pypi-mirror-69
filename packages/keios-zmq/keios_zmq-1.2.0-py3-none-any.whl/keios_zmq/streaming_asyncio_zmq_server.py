...
class StreamingAsyncioZmqServer(ZMQServer):
    ...
    def __init__(self, port: int, handle: Callable[[List[KeiosMessage]], Generator[KeiosMessage, None, None]]):
        self._port = port
        self._handle = handle
    ...
    def stream_process(self, bulk: BulkMessage, outbound_queue: Queue):
        try:
            for m in self._handle(bulk.messages):
            ...
            outbound_queue.put(
                BulkMessage(bulk.identifier, [KeiosMessage({'message_type': 'error'}, "Kaputt".encode())]))
