...
class ProxyRequests:
    ...
    def acquire_sockets(self):
        response = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=http&timeout=7000&country=BR&anonymity=elite&ssl=no').text
...
class Browser:
    ...
    def verify_proxy(self, url, proxies, data):
        with self.session as s:
            payload = data
            self.response = s.post(url=url, data=payload, proxies=proxies, verify=False)
    ...
    def open(self, url, referer=None, is_response=False, **kwargs):
        ...
        with self.session as s:
            if kwargs:
                payload = kwargs
                self.response = s.post(url=url, data=payload, proxies=self.proxies, headers=headers, verify=False).text
            else:
                self.request = s.get(url, proxies=self.proxies, headers=headers, verify=False)
...
class ChannelsNetwork(Browser):
    ...
    def search(self, parameter=None, description=True):
        ...
        return self.search_filmes(url_search, description=description, **data)
    ...
    def search_filmes(self, url, description, **kwargs):
        url_genre = url
