...
class ProxyRequests:
    ...
    def acquire_sockets(self):
        ...
        response = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=https&timeout=7000&country=BR&anonymity=elite&ssl=yes').text
        self.sockets = response.split('\r')
...
class Browser:
    ...
    def open(self, url, referer=None, is_response=False, **kwargs):
        if self.proxies:
            print('OPEN WITH PROXIES: ', self.proxies)
        if referer:
