...
def match_coordinate_system(
    ...
    :return: a tuple structured as (match_score, SRS or None)
    ...
    if in_epsg_code is not None and in_epsg_code.isdigit():
        ...
        srs_match = [d for d in group_srs if int(d.get("code")) == int(in_epsg_code)]
        if len(srs_match):
            logger.debug(
            ...
            # use Isogeo name instead
            out_coordsys = CoordinateSystem(
                code=in_epsg_code, name=srs_match[0].get("name")
            )
            has_srs = 1
        else:
            logger.debug(
            ...
            # use SRS name found into lookup
            out_coordsys = CoordinateSystem(code=in_epsg_code, name=in_srs_name)
            has_srs = 2
    elif (
        "FME_0" in in_srs_name and in_srs_name == "" and md_lookup.envelope is not None
    ):
        logger.warning(
            "FME failed to recognize SRS but still calculated an envelope for: {}. "
            "Update your FME configuration.".format(md_lookup.name)
        )
        has_srs = 3
    else:
        # fallback
