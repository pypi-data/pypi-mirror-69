...
class SpotList:
    ...
    def __init__(self, r: loc.LocResultList, pixelsize, outdir, IBg=None, IBg_crlb=None):
        self.result = r
...
#        print(f"ep{ep}. depth={depth}, phase={phase}")
    ...
    def simflux_fit(self,mod, ctx:Context, spotfilter, draw_silm_compare=False):
        ...
        with Context(ctx.smlm) as ctx:
            ...
            psf = sf.CreateEstimator_Gauss2D(self.result.cfg['sigma'], mod, 
                                                 roisize, len(mod),
            ...
            """            
            queue = EstimQueue(psf,numStreams=2)
            ...
            est = psf.Estimate(rois, roipos=sf_roipos)[0]
            iterations = np.zeros(len(est))
            crlb,chisq = psf._CRLBAndChiSquare(est, samples=rois, roipos=sf_roipos)
            #crlb = psf.CRLB(est, roipos=sf_roipos)
        ...
        data[:, loc.DataColumns.ITERATIONS] = iterations
...
#        plt.tight_layout()
    ...
    def excitation_matrix(self, mod, sel):
        xyI = self.result.get_xyI()[sel,None]
        mod_ = mod[None]
        kx = mod_['k'][:,:,0]
        ky = mod_['k'][:,:,1]
