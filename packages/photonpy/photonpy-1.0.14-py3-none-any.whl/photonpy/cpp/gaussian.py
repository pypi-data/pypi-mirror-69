...
class Gaussian:
    ...
    def CreatePSF_XYIBg(self, roisize, sigma, cuda) -> estimator.Estimator:
        ...
        return estimator.Estimator(self.ctx, inst, sigma)
    
    def CreatePSF_XYIBgSigma(self, roisize, initialSigma, cuda) -> estimator.Estimator:
        inst = self._Gauss2D_CreatePSF_XYIBgSigma(roisize, initialSigma, cuda, self.ctx.inst)
        return estimator.Estimator(self.ctx, inst, initialSigma)

    def CreatePSF_XYIBgSigmaXY(self, roisize, initialSigma, cuda) -> estimator.Estimator:
        inst = self._Gauss2D_CreatePSF_XYIBgSigmaXY(roisize, initialSigma[0], initialSigma[1], cuda, self.ctx.inst)
        return estimator.Estimator(self.ctx, inst, initialSigma)

    def CreatePSF_XYI(self, roisize, sigma, cuda) -> estimator.Estimator:
    ...
    def CreatePSF_XYZIBg(self, roisize, calib: Gauss3D_Calibration, cuda) -> estimator.Estimator:
