...
@LastEditTime: 2020-05-27 18:27:57
...
class Logger:
    ...
    def __init__(self, log_file_name, log_level, logger, host_ip="", project_name=None, db_connect=None, db_table_name="python_log_tb"):
        """
        @description: 
        ...
        if db_connect:
            self.handler_db = LoggerMysqlHandler(
                db_connect, db_table_name, project_name, host_ip)
...
class LoggerMysqlHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            self.db.fetch_and_commit(
                f"INSERT INTO {self.table_name}(project_name,level,host_ip,record_time,log_msg) VALUE('{self.project_name}','{record.levelname}','{self.host_ip}',{int(record.created)},'{pymysql.escape_string(record.message)}');")
        except Exception as ex:
            print(ex)
            pass
