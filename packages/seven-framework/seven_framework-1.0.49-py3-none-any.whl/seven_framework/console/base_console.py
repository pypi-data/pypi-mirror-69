...
@LastEditTime: 2020-05-25 14:43:04
...
if log_config and type(log_config) == dict and "db_log" in log_config:
    ...
    log_error_to_db = log_config["log_info_to_db"] if "log_info_to_db" in log_config else True

    if log_error_to_db:
        logger_error = Logger("logs/log_error", "ERROR", "log_error",
                              HostHelper.get_host_ip(), db_connect_dict=db_log, db_log_table_name=db_log_table_name).get_logger()
    if log_info_to_db:
