...
def constant(x, val=0):
    return np.ones(x.shape) * val
...
def hpeak(peaktype, x, qs=0, mgs=0, mes=0, amplitude=-1.0, center=0.0, H=1.0, Nu=0.5):
    amplitude *= np.array([3, 2, 1, 1, 2, 3]) / 12
    mes_l = np.array([- 1.5, -0.5, .5, -0.5, 0.5, 1.5]) * mes
    mgs_l = np.array([-0.5, -0.5, -0.5, 0.5, 0.5, 0.5]) * mgs
    msl = mes_l + mgs_l
...
class BkgModel(Model):
    def __init__(self, *args, **kwargs):
        super(BkgModel, self).__init__(constant, *args, **kwargs)
        self.set_param_hint('val', value=0.5, min=0.0)

    def guess(self, y):
        self.set_param_hint('val', value=sum(y[:5] + y[-5:]) / 10)
    ...
    def iplot(self, x, canvas=None):
        ...
        if hasattr(self, 'line'):
            ...
            self.line, = canvas.ax.plot(x, y, picker=5,
                                        label=self.prefix)
            canvas.ax.legend()
...
class iPModel(Model):
    ...
    def iplot(self, x, bkg_val=0, canvas=None):
        if bkg_val:
            self.bkg_val = bkg_val
        pars = self.make_params()
        y = self.eval(x=x, params=pars)
        if hasattr(self, 'line'):
            self.line.set_ydata(y + self.bkg_val)
        else:
            self.line, = canvas.ax.plot(x, y + self.bkg_val, picker=5,
                                        label=self.prefix)
            canvas.ax.legend()
...
class DPseudoVoigtModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def Q_PVoigt(x, qs=0.0, amplitude=1.0, center=0.0, H=1.0, Nu=0.5):
            return dpeak(peaktype='pvoigt', x=x, qs=qs, amplitude=amplitude,
                         center=center, H=H, Nu=Nu)
        super(DPseudoVoigtModel, self).__init__(Q_PVoigt, *args, **kwargs)
        self.set_param_hint('Nu', value=0.5, min=0.0, max=1.0)
...
class DLorentzianModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def Q_Lorentzian(x, qs=0, amplitude=1.0, center=0.0, H=1.0):
            return dpeak('lorentzian', x, qs, amplitude, center, H)
        super(DLorentzianModel, self).__init__(Q_Lorentzian, *args, **kwargs)
...
class DGaussianModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def Q_Gaussian(x, qs=0, amplitude=1.0, center=0.0, H=1.0):
            return dpeak('gaussian', x, qs, amplitude, center, H)
        super(DGaussianModel, self).__init__(Q_Gaussian, *args, **kwargs)
        self.set_param_hint('qs', value=0.0, min=0.0)
...
class HPseudoVoigtModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def M_PVoigt(x, mgs=0, mes=0, amplitude=1.0, center=0.0, H=1.0, Nu=0.5):
            return hpeak(peaktype='pvoigt', x=x, mgs=mgs, mes=mes,
                         amplitude=amplitude, center=center, H=H, Nu=Nu)
        super(HPseudoVoigtModel, self).__init__(M_PVoigt, *args, **kwargs)
        self.set_param_hint('Nu', value=0.5, min=0.0, max=1.0)
        self.set_param_hint('mgs', value=0.0, min=0.0)
        self.set_param_hint('mes', value=0.0, min=0.0)
...
class HLorentzianModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def M_Lorentzian(x, mgs=0, mes=0, amplitude=1.0, center=0.0, H=1.0):
            return hpeak('lorentzian', x=x, mgs=mgs, mes=mes, amplitude=amplitude,
                         center=center, H=H)
        super(HLorentzianModel, self).__init__(M_Lorentzian, *args, **kwargs)
        self.set_param_hint('mgs', value=0.0, min=0.0)
        self.set_param_hint('mes', value=0.0, min=0.0)
...
class HGaussianModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def M_Gaussian(x,mgs=0, mes=0, amplitude=1.0, center=0.0, H=1.0):
            return hpeak('gaussian', x=x,  mgs=mgs, mes=mes, amplitude=amplitude,
                         center=center, H=H)
        super(HGaussianModel, self).__init__(M_Gaussian, *args, **kwargs)
        self.set_param_hint('mgs', value=0.0, min=0.0)
        self.set_param_hint('mes', value=0.0, min=0.0)
...
class HDPseudoVoigtModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def MQ_PVoigt(x, qs=0.0, mgs=0, mes=0, amplitude=1.0, center=0.0, H=1.0, Nu=0.5):
            return hpeak(peaktype='pvoigt', x=x, qs=qs, mgs=mgs, mes=mes,
                         amplitude=amplitude, center=center, H=H, Nu=Nu)
        super(HDPseudoVoigtModel, self).__init__(MQ_PVoigt, *args, **kwargs)
        self.set_param_hint('Nu', value=0.5, min=0.0, max=1.0)
        self.set_param_hint('qs', value=0.0)
        self.set_param_hint('mgs', value=0.0, min=0.0)
        self.set_param_hint('mes', value=0.0, min=0.0)
...
class HDLorentzianModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def MQ_Lorentzian(x, qs=0, mgs=0, mes=0, amplitude=1.0, center=0.0, H=1.0):
            return hpeak('lorentzian', x=x, qs=qs, mgs=mgs, mes=mes,
                         amplitude=amplitude, center=center, H=H)
        super(HDLorentzianModel, self).__init__(MQ_Lorentzian, *args, **kwargs)
        self.set_param_hint('qs', value=0.0)
        self.set_param_hint('mgs', value=0.0, min=0.0)
        self.set_param_hint('mes', value=0.0, min=0.0)
...
class HDGaussianModel(iPModel, Model):
    ...
    def __init__(self, *args, **kwargs):
        def MQ_GaussianModel(x, qs=0, mgs=0, mes=0, amplitude=1.0, center=0.0, H=1.0):
            return hpeak('gaussian', x=x, qs=qs, mgs=mgs, mes=mes,
                         amplitude=amplitude, center=center, H=H)
        super(HDGaussianModel, self).__init__(MQ_GaussianModel, *args, **kwargs)
        self.set_param_hint('qs', value=0.0)
        self.set_param_hint('mgs', value=0.0, min=0.0)
        self.set_param_hint('mes', value=0.0, min=0.0)
