...
class HPProcess():
    ...
    def parse_hp_set(self, text, search_type):
        hp_set = None
    ...
    def hp_set_to_sweeps_line(self, name, hp_set):
        ...
        if isinstance(hp_set, HPList):
            #print("hp_set.values=", hp_set.values)
            text = "{} = ".format(name)
            text += ", ".join((str(v) for v in hp_set.values))
        else:
            #print("hp_set.min=", hp_set.min, ", hp_set.max=", hp_set.max)
    ...
    def generate_hparam_args(self, orig_cmd_parts, max_gen=None, search_type="grid"):
        ...
        for i, part in enumerate(cmd_parts):
            ...
            if "=[" in part and "]" in part:
                ...
                if part.endswith("]"):
                    ...
                    #print("part=", part, ", cmd_part=", cmd_parts[i])
                    found = True
            ...
            if found:   
                hp_set = self.parse_hp_set(part, search_type)
        ...
        if hp_sets:
            ...
            while len(arg_sets) < max_gen:
                ...
                for hp_set in hp_sets:
                    value = hp_set.next()
                    #print("generate value: ", value)
                    values.append(value)

                arg_sets.append(values)

        return using_hp, arg_sets, cmd_parts, sweeps_text
    ...
    def fill_in_template(self, template_parts, values):
        ...
        #print("ACTUAL cmd_parts=", cmd_parts)
        return cmd_parts
