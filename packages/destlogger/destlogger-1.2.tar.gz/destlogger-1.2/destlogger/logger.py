...
class Logger:
    ...
    def printl(self, *text, group='INFO', fore='white', back='reset', style='normal', log_time=None, sep=' ', end='\n', flush=False, file=None):
        ...
        if len(text) > 120 - len(info):
            size = 120 - len(info)
            texts = split_string(text, size)

            for i in range(1, len(texts)):
                texts[i] = texts[i].rjust(120) if len(texts[i]) == size else texts[i].rjust(120 - size + len(texts[i]))

            text = '\n'.join(texts)

        print(fore_color + back_color + style + info + text + self.styles['reset'], end=end, flush=flush, file=file)
