...
class Model:
    ...
    BKPI_PAYLOAD_HISTORY_CSV_FILENAME = 'history_bkpi_payloads.csv'
    BKPI_TXN_BATCHES_ID_MAP_HISTORY_FILENAME = 'history_bkpi_transaction_batches_id_map.json'
    ...
    def get_fairness_history(self, num_day):
        ...
        if history_file:
            ...
            for hour in range(24):
                ...
                full_records_list.append({'timestamp': score_time, 'value': fairness_values[index]})
        return full_records_list
    ...
    def get_debias_history(self, num_day):
        ...
        if history_file:
            ...
            for hour in range(24):
                ...
                full_records_list.append({'timestamp': score_time, 'value': debias_values[index]})
        return full_records_list
    ...
    def get_quality_monitor_history(self, num_day):
        ...
        if history_file:
            ...
            for hour in range(24):
                ...
                full_records_list.append(MeasurementRecord(metrics=quality_value['metrics'], sources=quality_value['sources'], timestamp=score_time))
        return full_records_list
    ...
    def get_bkpi_metrics_history(self, num_day):
        ...
        if history_file:
            with open(history_file) as f:
                ...
                today = bkpi_records[num_day]
                bkpi_metrics_list.append( { 'timestamp': record_time, 'metrics': [ today['kpi'] ] } )
                for record in today['drift']:
                    bkpi_drift_list.append( { 'timestamp': record_time, 'metrics': [ record ] } )
        return [{'kpi': bkpi_metrics_list, 'drift': bkpi_drift_list}]

    def get_bkpi_payload_history(self, num_day):
        # each 'bkpi_payload_history_N.json' file contains a full day of bkpi payloads specific to this day
        history_file = self._get_file_path(Model.BKPI_PAYLOAD_HISTORY_CSV_FILENAME.replace('.csv', ('_' + str(num_day) + '.csv')))
        bkpi_payload_history = []
        if history_file:
            bkpi_payload_data_type = self._get_bkpi_payload_data_type()
            bkpi_payload_history = pd.read_csv(history_file, dtype=bkpi_payload_data_type).to_dict('records') # make a list-style DICT
            for i in range(len(bkpi_payload_history)):
                hour = int(bkpi_payload_history[i]['transaction_id'].split('-')[1]) # "day-hour-txn"
                bkpi_payload_history[i]['timestamp'] = str(self._get_score_time(num_day, hour))
        return bkpi_payload_history

    def get_bkpi_txn_batches_id_map_history(self, num_day):
        # each 'bkpi_txn_batches_id_map_history_N.json' file contains a full day of bkpi transaction batches id maps specific to this day
        full_records_list = []
        history_file = self._get_file_path(Model.BKPI_TXN_BATCHES_ID_MAP_HISTORY_FILENAME.replace('.json', ('_' + str(num_day) + '.json')))
        if history_file:
            with open(history_file) as f:
