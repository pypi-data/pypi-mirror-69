...
from ...util import EoqError,NoLogging
...
def AsyncCallProcessMain(actiondir,actionname,callId,argStr,cmdQueue,resQueue):
    #connect to domain
    serializer = JsonSerializer()
    domain = MultiprocessingQueueDomainClient(cmdQueue,resQueue)
    stdoutRedirector = WriteRedirector(callId,'STDOUT',domain.callManager,legacyWriteFcn=sys.stdout.write)
    stderrRedirector = WriteRedirector(callId,'STDERR',domain.callManager,legacyWriteFcn=sys.stderr.write)
    ...
    if(actionFunction):
        ...
        domain.callManager.ChangeCallStatus(callId,CallStatus.RUN)
        ...
        domain.callManager.ChangeCallStatus(callId,finalCallStatus)
    ...
    def __ScanForActions(self):
        ...
        for pythonfile in pythonfiles:
            ...
            try: #prevent server crash by broken action scripts
                ...
                action = Action(actionname,actionArguments,actionResults,actionDescription)
...
#         #scan for file based actions
    ...
    def HandleSync(self, name, callId, domain, args, callManager):
        ...
        callManager.ChangeCallStatus(callId,CallStatus.RUN)
        value = None
        ...
        if(stdoutOutput.getvalue()):
            callManager.AddCallOutput(callId,'STDOUT',stdoutOutput.getvalue())
        callManager.ChangeCallStatus(callId,CallStatus.FIN)
        return value
    
    def HandleAsync(self, name, callId, domain, args, callManager):
        ...
        observerThread = threading.Thread(target=self.__AsyncCallObserverThread, args=(name,runningAction,domain,args,callManager,))
    ...
    def __AsyncCallObserverThread(self,name,runningAction,domain,args,callManager):
        #runningAction = self._runningActions[callId]
        serializer = JsonSerializer()
        argsStr = serializer.Ser(args)
        domainWrapper = MultiprocessingQueueDomainHost(domain,callManager)
        proc = Process(target=AsyncCallProcessMain, args=(self._basedir,name,runningAction.callId,argsStr,domainWrapper.cmdQueue,domainWrapper.resQueue))
