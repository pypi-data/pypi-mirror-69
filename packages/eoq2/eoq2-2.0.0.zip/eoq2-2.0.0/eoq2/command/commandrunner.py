...
class CmdRunner(EvtProvider):
    def __init__(self,mdbAccessor,maxChanges=100,logger=NoLogging()):
        ...
        self.callManager = CallManager(self)        
    ...
    def ExecHel(self,args,tid):
        ...
        sessionId = ''
        #check identificaiton
        if(1): #TODO: validate user and password
            session = None
            if(transaction.session):
                session = transaction.session
                sessionId = session.sessionId #retrieve the exising session id
            else: #new session is needed
                sessionId = str(uuid4())
                if(sessionId in self.sessions):
                    raise EoqError(0,'Dublicated session id')
    ...
    def ExecGaa(self,args,tid):
        ...
        #res = None  
        #history = self.GetHistory(tid)
        res = [[a.name,[[p.name,p.type,p.min,p.max,p.default,p.description,[[o.value,o.description] for o in p.options]] for p in a.args],[[r.name,r.type,r.min,r.max,r.default,r.description,[[o.value,o.description] for o in r.options]] for r in a.results],a.description] for a in self.callManager.GetAllActions()]
        self.AddToHistory(res,tid)
    ...
    def ExecCal(self,args,tid):
        ...
        #call the handler
        res = self.callManager.RunCall(name, actionArgs, tid)
        self.AddToHistory(res,tid)
        return Res(CmdTypes.CAL,status,res,tid,self.latestChangeId)
    ...
    def ExecAsc(self,args,tid):
        ...
        for qry in args[1]:
            aa = self.qryEvaluator.Eval(qry,history)
            actionArgs.append(aa)
        res = self.callManager.RunCallAsnyc(name, actionArgs, tid)
        self.AddToHistory(res,tid)
