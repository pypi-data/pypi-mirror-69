...
class Versiointi:
  ...
  Args:
    ...
    master (str): master-haaran nimi, oletus `origin/master`
  ...
  def __init__(
    ...
    versio=None, aliversio=None, oksaversio=None,
    historian_pituus=None,
    **kwargs
  ):
    if kwargs:
      warnings.warn(
  ...
  def _oksan_tiedot(self, ref=None):
    master, oksa = self.tietovarasto.oksa(ref, master=self.master)
    if not oksa:
