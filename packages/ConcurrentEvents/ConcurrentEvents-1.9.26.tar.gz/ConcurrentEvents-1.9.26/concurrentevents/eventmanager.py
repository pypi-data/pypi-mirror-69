...
class EventManager:
    ...
    def __init__(self,
                 ...
                 debug=False):
        """Constructor Method"""
        if threads < 0 if isinstance(threads, int) else True:
            raise ValueError(f"EventManager logger argument must be a int greater than 0 not {threads}")

        if not isinstance(logger, logging.Logger):
            raise TypeError(f"EventManager logger argument must be a logger not {logger}")
    ...
    def start(self):
        ...
        while not (self.exit or len(self.__futures.keys()) == 0):
            try:
                ...
                _base.log.critical("Event Manager Main Loop Error")
        ...
        if len(waited.not_done) > 0:
            _base.log.error("Final Cleaning Timed Out")
