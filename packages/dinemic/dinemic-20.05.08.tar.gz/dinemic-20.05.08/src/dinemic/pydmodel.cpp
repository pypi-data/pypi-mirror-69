...
void PyDModel::map_fields() {
    ...
    for (int i = 0; i < PyList_Size(attrs); i++) {
        ...
        if (string(field->ob_type->tp_name) == "DField") {
            ...
            bool encrypted_bool = PyBool_Check(encrypted);
...
//            boost::python::object(boost::python::ptr(this));
//            boost::python::object types = boost::python::import("dinemic");
//            boost::python::object type = types.attr("DField");

//            PyObject *field_name = PyObject_GetAttrString(field, "field_name");
//            PyObject *is_encrypted = PyObject_GetAttrString(field, "is_encrypted");

//            char *field_name_str = PyUnicode_AsUTF8(field_name);
//            bool is_encrypted_bool = PyBool_Check(is_encrypted);

//            boost::python::object new_obj = type.attr("__new__")(string(field_name_str), is_encrypted_bool);

//            PyObject_SetAttrString(new_obj.ptr(), "_caller_id", clr_id);
//            PyObject_SetAttrString(new_obj.ptr(), "_object_id", obj_id);
//            PyObject_SetAttr(self, attr, new_obj.ptr());
