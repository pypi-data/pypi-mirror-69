...
from .config import Auth, Auth0Authentication, BasicAuthentication, Role
...
class JavaApi(metaclass=ApiMetaClass):
    ...
    def configure_authentication(self, auth: Auth, session_name: str, session_id: str):
        ...
        elif isinstance(auth, Auth0Authentication):
            check_auth0_config(auth)
            self.java_api.addOAuthSecurityProvider(
                auth.issuer,
                auth.audience,
                auth.client_id,
                auth.client_secret,
    ...
    def update_hierarchy_coordinate(
        self, cube: Cube, old_dim: str, old_hier: str, new_dim: str, new_hier: str
    ):
        """Change the coordinate of a hierarchy.

        Args:
            cube: The cube
            old_dim: The previous dimension name
            old_hier: The previous hierarchy name
            new_dim: The new dimension name
            new_hier: The new hierarchy name.
        """
        self.java_api.updateHierarchyCoordinate(
            cube.name, old_dim, old_hier, new_dim, new_hier
        )
    ...
    def update_hierarchy_slicing(self, hierarchy: Hierarchy, slicing: bool):
        ...
        self.java_api.setHierarchySlicing(
            hierarchy._cube.name, hierarchy.dimension, hierarchy.name, slicing
        )
    ...
    def drop_hierarchy(self, cube: Cube, dimension: str, hierarchy_name: str):
        """Drop a hierarchy from the cube.

        Args:
            cube: The cube to drop the hierachy from
            dimension: The dimension to drop the hierachy from
            hierarchy_name: The name of the hierarchy to drop

        """
        self.java_api.dropHierarchy(cube.name, dimension, hierarchy_name)

    def retrieve_hierarchies(self, cube: Cube) -> Mapping[str, Hierarchy]:
    ...
    def retrieve_hierarchy(self, cube: Cube, name: str) -> Optional[Hierarchy]:
        """Retrieve a cube's hierarchy."""
        from .hierarchy import Hierarchy  # pylint: disable=redefined-outer-name

        # Get the hierarchy from the java side.
        java_hierarchy = self.java_api.retrieveHierarchy(cube.name, name)
        if not java_hierarchy:
            return None
    ...
    def agg_siblings(
        ...
        """
        hierarchies_java_list = ListConverter().convert(
            [hierarchy], self.gateway._gateway_client
        )
        return self.java_api.aggSiblings(
            cube.name,
            measure_name,
            underlying,
            hierarchies_java_list,
            agg,
            exclude_self,
        )
    ...
    def parent_value(
        ...
        Args:
            ...
            measure_name: The name for the measure we are creating
            underlying: The name of the underlying measure
            hierarchies: The hierarchies to drill up.
    ...
    def shift(
        ...
        Args:
            ...
            measure_name: The name for the measure we are creating.
            underlying: The name of the underlying measure.
            level: The level to shift on.
            offset: The number of element to shift.

        Returns:
            The name of the newly created measure.
        """
        return self.java_api.shift(cube.name, measure_name, underlying, level, offset)
...
def check_auth0_config(auth0: Auth0Authentication):
    """Check that the authentication is valid."""
    if auth0.issuer is None:
        raise ValueError("Missing issuer in auth0 configuration")
    if auth0.client_secret is None:
        raise ValueError("Missing client_secret in auth0 configuration")
    if auth0.audience is None:
        raise ValueError("Missing audience in auth0 configuration")
    if auth0.client_id is None:
        raise ValueError("Missing client_id in auth0 configuration")
    if auth0.role_mapping is None:
        raise ValueError("Missing role_mapping in auth0 configuration")
