...
class KickBanMixin(MixinMeta):
    ...
    async def ban(
        ...
        *,
        ...
        if result is True:
            await ctx.send(_(f"Banned **{user.name}**"))
            await ctx.message.add_reaction("<:ban:697512426442326026>")
        ...
        for entry in ban_list:
            for user_id in user_ids:
                if entry.user.id == user_id:
                    errors[user_id] = _(f"User **{user_id}** is already banned.").format(
                        user_id=user_id
                    )
    ...
    async def tempban(
        ...
        try:
            ...
            try:
                ...
                await ctx.send(e)
            await ctx.send(_(f"Banned **{user.name}** until {date}"))
    ...
    async def softban(self, ctx: commands.Context, user: discord.Member, *, reason: str = None):
        ...
        try:
            ...
            try:
                ...
                await ctx.send(e)
            message = await ctx.send(_(f"Softbanned **{user.name}**"))
            await asyncio.sleep(3)
            await message.edit(content="...clearing messages....")
            await asyncio.sleep(1)
            await message.edit(content=".....clearing messages...rektaf™...")
            await asyncio.sleep(1)
            await message.edit(content="...clearing messages..")
            await asyncio.sleep(1.5)
            await message.edit(content=f"Messages cleared — **{user.name}** has been reinvited")
            await ctx.tick()
    ...
    async def unban(self, ctx: commands.Context, user_id: RawUserIds, *, reason: str = None):
        ...
        try:
            ...
            try:
                ...
                await ctx.send(e)
            await ctx.send(_(f"Unbanned {user.name} from this server"))
