...
class DatasetUpload():
    ...
    def __init__(self, client, datasetUploadId=None, status=None, datasetId=None, datasetInstanceId=None, parts=None, createdAt=None):
        self.client = client
    ...
    def __repr__(self):
        return f"DatasetUpload(dataset_upload_id={repr(self.dataset_upload_id)}, status={repr(self.status)}, dataset_id={repr(self.dataset_id)}, dataset_instance_id={repr(self.dataset_instance_id)}, parts={repr(self.parts)}, created_at={repr(self.created_at)})"

    def __eq__(self, other):
        return self.__class__ == other.__class__ and self.id == other.id

    def to_dict(self):
        return {'dataset_upload_id': self.dataset_upload_id, 'status': self.status, 'dataset_id': self.dataset_id, 'dataset_instance_id': self.dataset_instance_id, 'parts': self.parts, 'created_at': self.created_at}
