...
def opcode_occurrence(opcode_list, collapse=0):
    ...
    stat_opcode_list = collapse_opcode(opcode_list=opcode_list, collapse=collapse)
...
def function_to_signature(function_name):
    ...
    '''
    import sha3
    signature = sha3.keccak_256(str(function_name).encode('utf-8')).hexdigest().lower()[:8]
    return signature
...
def signature_to_function(function_signature):
    ...
    #     import subprocess
    #     subprocess.run(['git', 'clone', 'https://github.com/yzstr/Function-Signatures.git'])
