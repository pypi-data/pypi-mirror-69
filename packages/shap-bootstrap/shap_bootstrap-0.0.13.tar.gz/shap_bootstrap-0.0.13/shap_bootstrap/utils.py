...
def assign_instances(X, X_train, X_val, y, shapley_values):
    ...
    y_instanced = pd.concat(
        (pd.DataFrame(y), pd.DataFrame(columns=["instance"])), axis=1
    )
    ...
    shap_instanced = pd.concat(
        (shapley_values, pd.DataFrame(columns=["instance"])), axis=1
    )
...
def prepare_pipeline_reduced_data(
    X, y, method, test_proportion=0.2, validation_proportion=0.25
):

    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_proportion, random_state=0
    )
    ...
    X_train_tr, X_train_val, y_train_tr, y_train_val = train_test_split(
        X_train, y_train, test_size=validation_proportion, random_state=0
    )
...
def set_branch_best(dataframe, *branchs):
    ...
    for i, branch in enumerate(branchs):
        ...
        for j in range(len(dataframe_temp["Dataset_name"])):
            dataframe_temp[branch].iloc[j] = branch_best.loc[
                dataframe_temp["Dataset_name"].iloc[j]
            ][0]
