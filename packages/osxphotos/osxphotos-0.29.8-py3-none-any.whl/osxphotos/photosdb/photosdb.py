...
class PhotosDB:
    ...
    def _process_database5(self):
        ...
        stop_t = time.time()
        logging.warning(f"process_database5 took {stop_t-start_t} seconds")

        # process search info
        start_t = time.time()
        self._process_searchinfo()
        stop_t = time.time()
        logging.warning(f"searchinfo took {stop_t-start_t} seconds")

        # process exif info
        start_t = time.time()
        self._process_exifinfo()
        stop_t = time.time()
        logging.warning(f"exifinfo took {stop_t-start_t} seconds")
    ...
    def _album_folder_hierarchy_list_5(self, album_uuid):
        ...
        try:
            ...
            logging.debug(f"Caught _dbalbum_folders KeyError for album: {album_uuid}")
            # logging.debug(pformat(self._dbalbum_details[album_uuid]))
            return []
