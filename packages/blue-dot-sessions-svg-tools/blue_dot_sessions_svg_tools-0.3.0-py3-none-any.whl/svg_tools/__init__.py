...
def rotate_if_polygon2svg(paths: Dim2FloatType, *args) -> str:
    return straight_paths2svg(
        rotate_if_polygon(paths, *args)
    )


def added_vertices_polygon2svg(paths: Dim2FloatType, *args) -> str:
    return straight_paths2svg(
        added_vertices_polygon(paths, *args)
    )


def jagged_line_polygon2svg(paths: Dim2FloatType, *args) -> str:
    return straight_paths2svg(
        jagged_line_polygon(paths, *args)
    )


def rounded_corner_polygon2svg(paths: Dim2FloatType, *args) -> str:
    return curved_paths2svg(
        rounded_corner_polygon(paths, *args)
    )


_primtive_prefix = "http://www.w3.org/2000/svg"
_primtive_prefix_g = "{" + _primtive_prefix + "}"


def _vertices_from_points_str(points_str: str) -> Dim1FloatType:
    """
    Convert SVG polygon element `points` field string to flat list of floats
    """
    if " " in points_str:
        points = []
...
def polygon_iter(file_path: str):
    ...
    generated SVG.
    """
    ET.register_namespace("", _primtive_prefix)
    tree = ET.parse(file_path)
    root = tree.getroot()

    g = root.find(f"{_primtive_prefix_g}g")
    if g is None:
        raise ValueError("Cannot find group element!")

    for child in g.iter(f"{_primtive_prefix_g}polygon"):
        yield child
...
def modify_svg(file_path: str, polygon_modify_fn: typing.Callable):
    ...
    for polygon in polygon_iter(file_path):
        points_str = polygon.attrib["points"]
        points_flat = _vertices_from_points_str(points_str)
        points_pairs = util.flat2pair(points_flat)
        paths_str = polygon_modify_fn(points_pairs)
        polygon.tag = f"{_primtive_prefix_g}path"
        polygon.attrib["d"] = paths_str
        del polygon.attrib["points"]


polygon2svg_lookup = {
    "rotate_if": rotate_if_polygon2svg,
