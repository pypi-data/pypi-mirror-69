...
def _download_artifact(run, download_dir, artifact_name, extension, explanation_id=None, file_type=IO.JSON):
    ...
    if explanation_id is not None:
        artifact_path = 'explanation/{}/'.format(explanation_id[:History.ID_PATH_2004])
    else:
...
class ExplanationClient(object):
    ...
    def upload_model_explanation(self,
        ...
        if is_viz_turned_on and has_eval_y_predicted_proba and eval_data_viz_uploaded:
            ...
            temp_path = os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN, self._run.id, upload_dir,
                                                              'ys_pred_proba_viz'))
            artifact_list.append({History.PREFIX: temp_path})
            viz_dict['probabilityY'] = temp_path
        ...
        if is_viz_turned_on:
            if enable_local_viz:
                if index_getter is None:
                    ...
                    artifact_list.append({History.PREFIX: temp_path})
                viz_dict['precomputedExplanations.localFeatureImportance.scores'] = temp_path
            else:
                temp_path = None
                for prefix_dict in artifact_list:
                ...
                if temp_path is not None:
                    viz_dict['precomputedExplanations.globalFeatureImportance.scores'] = temp_path
        ...
        if is_viz_turned_on:
            ...
            if History.EXPECTED_VALUES in meta_dict:
                ...
                if not enable_local_viz:
                    viz_dict['precomputedExplanations.globalFeatureImportance.intercept'] = expected_path
                else:
                    viz_dict['precomputedExplanations.localFeatureImportance.intercept'] = expected_path
        ...
        meta_dict = {
            ...
            History.VERSION: History.EXPLANATION_ASSET_TYPE_V7,
        ...
        if experiment_name is not None:
            meta_dict[History.EXPERIMENT_NAME] = experiment_name
        if experiment_id is not None:
            meta_dict[History.EXPERIMENT_ID] = experiment_id
        if model_id is not None:
            meta_dict[History.MODEL_ID] = model_id

        props = {History.TYPE: History.EXPLANATION,
                 History.EXPLANATION_ID: explanation.id}
        if sparse_eval_data:
            props['sparse_data'] = True
    ...
    def _download_explanation_as_batch(self, explanation_id=None, comment=None, raw=None, engineered=None):
        ...
        explanation_assets = list(assets_client.get_assets_by_run_id_and_name(self._run.id, History.EXPLANATION_ASSET))
