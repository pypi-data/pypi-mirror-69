...
class MgraphBase:
    ...
    def __init__(self, params='config.json', database=None, args=None):
    ...
    def connect(self):
        ...
        result = self.app.acquire_token_silent(self.params["scope"], account=None)

        if not result:
            logging.info("No suitable token exists in cache. Let's get a new one from AAD.")
            result = self.app.acquire_token_for_client(scopes=self.params["scope"])
    ...
    def get(self, url, **kwargs):
        ...
        if 'error' in graph_data:
            raise MgraphApiError(graph_data, f'MsGraph GET Error: {url}{params}')
