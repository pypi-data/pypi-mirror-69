...
class Message(ABC):
    ...
    def check_values(self, expect_val, actual_val):
        """
        check response data
        @expect_val : 预期值
        @actual_val : 实际值,取自response data
        """
        assert actual_val == expect_val, f'[校验失败,实际值:{actual_val} == 预期值：{expect_val}]'
        logger.info(f'[校验成功,实际值:{actual_val} == 预期值：{expect_val}]')
        return self
