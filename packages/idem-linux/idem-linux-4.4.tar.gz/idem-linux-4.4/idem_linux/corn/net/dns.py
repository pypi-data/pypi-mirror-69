...
async def load_dns(hub):
    ...
    hub.corn.CORN.dns.nameservers = resolv.nameservers
    hub.corn.CORN.dns.ip4_nameservers = [
        ip for ip in resolv.nameservers if await _is_ipv4_address(ip)
    ]

    hub.corn.CORN.dns.ip6_nameservers = [
        ip for ip in resolv.nameservers if await _is_ipv6_address(ip)
    ]

    hub.corn.CORN.dns.sortlist = await _get_sortlist(resolv.nameservers)
    hub.corn.CORN.dns.domain = str(resolv.domain)
    hub.corn.CORN.dns.search = [str(x) for x in resolv.search]
    hub.corn.CORN.dns.options = await _parse_options(resolv.__dict__)
