    ...
    for line in dmi_raw:
        if re.match(r"\t[^\s]+", line):
            ...
            if (clean and key == "header_and_data") or (fields and key not in fields):
                ...
                key_data = [await _dmi_cast(key, val.strip(), clean), []]
        elif key is None:
            continue
            ...
            #        PCI is supported
            #        PNP is supported
            val = await _dmi_cast(key, line.strip(), clean)
            if val is not None:
...
async def _dmi_parse(
    data, clean: bool = True, fields: str = None
) -> List[Dict[str, Any]]:
    """
    ...
    for handle, dmi_raw in zip(dmi_raw, dmi_raw):
        ...
        dmi_data = await _dmi_data(dmi_raw, clean, fields)
        if dmi_data:
...
async def _dmi_cast(key: str, val, clean=True) -> int or None:
    """
    Simple caster thingy for trying to fish out at least ints & lists from strings
    """
    if clean and not await _dmi_isclean(key, val):
        return
...
async def _dmi_isclean(key: str, val: str or None) -> bool:
    """
...
async def records(
    ...
    return await _dmi_parse((await _dmi_decode(hub, args)), clean, fields)
