...
if sys.version_info<(3,0):
    import pickle as pickle # Python2
else:
...
class Worker(object):
    ...
    def invalid_dict_pickle_keys(self, msg_dict):
        ...
        no_pickle_keys = list()
        for key, val in list(msg_dict.items()):
    ...
    def invalid_obj_pickle_attrs(self, thisobj):
        ...
        for attr, val in list(vars(thisobj).items()):
            try:
...
class TaskMgr(object):
    ...
    def supervise(self):
        ...
        if not hot_loop:
            self.kill_workers()
            for w_id, p in list(self.workers.items()):
                p.join()
    ...
    def is_finished(self):
        if (len(self.work_todo) == 0) and (len(list(self.worker_assignments.keys())) == 0):
            return True
