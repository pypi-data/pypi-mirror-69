...
from .base import BaseDataClass, model_property, DefaultPrimitiveView
...
from spintop.utils import utcnow_aware, is_aware, dict_ops
...
class PersistenceRecordView(object):
    ...
    def __init__(self, data_view=None, include_index=True, data_prefix=None, index_prefix=None):
        self.include_index = include_index

        if data_view:
            self.data_view = DataClassPrimitiveView(data_view)
        else:
            self.data_view = self.default_view
    ...
    def apply(self, record, flatten_dict=True):
        ...
        if self.include_index:
            index_data = self.apply_index(record, flatten_dict=flatten_dict, key_prefix=self.index_prefix, attribute_error_policy='raise')
