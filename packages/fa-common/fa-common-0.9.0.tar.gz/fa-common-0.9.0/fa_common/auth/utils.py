...
async def decode_token(auth: str):
    ...
    jwks = await async_get(
        f"https://{settings.AUTH0_DOMAIN}/.well-known/jwks.json", json_only=True
    )
    ...
    if "sub" in payload:
        if (
            ...
            try:
                ...
                LOG.warning(
                    f"Something went wrong retieving user profile falling back to creating user from "
