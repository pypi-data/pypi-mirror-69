...
def _normalize_file_patterns(raw_cfg):
    ...
    for filepath, patterns in list(file_patterns.items()):
        if not os.path.exists(filepath):
            log.warning('Invalid config, no such file: {0}'.format(filepath))
        normalized_patterns = []
        for pattern in patterns:
            ...
            if version_pattern == '{pycalver}':
                ...
                log.warning("Invalid config, cannot match '{0}' for '{1}'."
                    .format(pattern, filepath))
                log.warning("No mapping of '{0}' to '{1}'".format(
...
def parse(ctx):
    ...
    try:
        ...
        log.warning("Couldn't parse {0}: {1}".format(cfg_path, str(ex)))
        return None
