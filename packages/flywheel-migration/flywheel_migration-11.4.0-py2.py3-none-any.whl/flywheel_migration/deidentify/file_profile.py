    ...
    def load_config(self, config):
        ...
        if self.filenames:
            for i, filename in enumerate(self.filenames):
                ...
                self._add_identity_fields(filename)

    def _add_identity_fields(self, filename):
        """Add identity field for certain keyword of filename.output

        Elements of filename['output'] that are not in, fields, groups or extracted from the
        filename['input-regexp'] are set as identify field"""
        regex = re.compile(r'\{(.*?)\}')
        ...
        for out_var in out_vars:
            if out_var not in self.field_map and out_var not in group_names:
                field_config = {'name': out_var, 'identity': True}
                self.add_field(DeIdField.factory(field_config))
