...
class TIFFRecord:
    ...
    def validate(self):
        ...
        if self.image.n_frames > 1:
            raise ValueError(f'{self.image.n_frames} frames found in {self.path}. '
                             f'Currently only supporting 1 frames')

    def save_as(self, filepath, file_type=None):
        """Save deid image
        ...
        if self.metadata:
            self.image.save(filepath, format=file_type, tiffinfo=self.metadata)
        else:
            self.image.save(filepath, format=file_type)
