...
class ExpressionAnalyser:
    ...
    def get_expression_score(self, asr, pinyin, audio_path, use_pinyin=False):
        ...
        recognition_duration, pause_moment = self.__get_duration_information(audio_path)
        text, text_length, particle_count, repeat_one_count, ranhou_count = self.__disfluency_detection(raw_text, pinyin, use_pinyin)
    ...
    def __parse_expression_score(
        ...
        self.middle_value.update(dict(recognition_duration_per_pause_count=recognition_duration_per_pause_count,
                                      pause_duration_ratio=pause_duration_ratio,
                                      ranhou_count_per_length=ranhou_count_per_length,
                                      length_per_count=length_per_count, repeat_one_count=repeat_one_count))
        ...
        return (0.75 * pause_duration_ratio_score + 0.75 *
                ...
                1.5 * repeat_one_count_score + ranhou_count_per_length_score * 0.5) * decay_factor * (0.8 + 0.1 * mandarin_score/100)
    ...
    def __disfluency_detection(self, text, pinyin, use_pinyin=False):
        ...
        if not use_pinyin:
            # 语气词
            particle_count = len(re.findall(config.voice_modal_particles_pattern,
                                            re.sub(config.text_clean, '', text)))
            # 文本清理
            ...
            # 语气词
            particle_count = len(re.findall(config.voice_modal_particles_pattern_pinyin, ' ' + pinyin + ' '))
            # 文本清理
            repeat_one_count = len(self.__get_repeat_record_by_offset(pinyin, 1))
...
if __name__ == '__main__':
    ...
    print(ea.get_expression_score(asr=fake_asr, pinyin=fake_pinyin, audio_path = 'temp/10.wav'))
    print(ea.middle_value)
    print(ea.get_expression_score(asr=fake_asr, pinyin=fake_pinyin, audio_path = 'temp/10.wav', use_pinyin=True))
    print(ea.middle_value)
