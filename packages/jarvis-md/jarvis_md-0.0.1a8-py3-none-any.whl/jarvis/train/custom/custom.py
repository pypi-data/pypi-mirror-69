...
def softmax_ce_sens(weights=1.0, threshold=0.5, epsilon=1):
    """
    Method to implement sensitivity (recall) on raw softmax cross-entropy logits
...
def softmax_ce_ppv(weights=1.0, threshold=0.5, epsilon=1):
    """
    Method to implement PPV (precision) on raw softmax cross-entropy logits
    ...
    def softmax_ce_ppv(y_true, y_pred):
        ...
        num = tf.math.count_nonzero(tp) 
        den = tf.math.count_nonzero(pp) + epsilon
