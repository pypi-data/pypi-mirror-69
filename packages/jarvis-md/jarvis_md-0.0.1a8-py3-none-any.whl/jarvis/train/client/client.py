...
class Client():
    ...
    def __init__(self, client=None, configs=None, pattern='client*', load=None, *args, **kwargs):
        ...
        self.load_yml(client, configs, pattern, *args, **kwargs)
    ...
    def load_yml(self, client, configs, pattern, *args, **kwargs):
        """
        ...
        # --- (1) Find client (passed via *args)
        for arg in args:
            if type(arg) is str:
                if arg.split('.')[-1] == 'yml':
                    client = arg

        # --- (2) Find client (default)
        client = client or './client.yml'

        # --- (3) Find client (using glob pattern)
        if not os.path.exists(client):

            _id = self.parse_ids(**kwargs)
            configs['paths'] = jtools.get_paths(_id['project'], _id['version']) 
            subdir = kwargs.get('subdir', 'data')

            try: 
                client = glob.glob('{}/{}/ymls/{}.yml'.format(
                    configs['paths']['code'], subdir, pattern))
                client = sorted(client)[0]
                assert os.path.exists(client)
            except:
                assert False, 'ERROR client.yml location could not be inferred'
    ...
    def parse_ids(self, **kwargs):
        """
        Method to parse kwargs and ENV variables for project_id and version_id
        ...
        if 'pid' in kwargs:
            _id['project'] = kwargs.pop('pid')
            _id['version'] = kwargs.pop('vid', None)
            ...
            for key, spec in self.specs[k].items():
                ...
                if spec['loads'] in self.db.fnames.columns:
                    ...
                    if arrays[k][key] is None:
                        arrays[k][key] = np.zeros(spec['shape']['saved'], dtype=spec['dtype'])
