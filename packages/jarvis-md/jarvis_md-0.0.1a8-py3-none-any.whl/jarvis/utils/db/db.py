...
class DB():
    ...
    def __init__(self, *args, **kwargs):
        ...
        :params
          ...
          (str)  pid     : project_id (used when defining new DB object); otherwise extracted from *.yml
          (str)  vid     : version_id (used when defining new DB object); otherwise extracted from *.yml
    ...
    def parse_args(self, *args, **kwargs):
        ...
        args_['_id'] = self.parse_ids(**kwargs)
        ...
        if 'prefix' in kwargs:
            ...
            if 'code' not in paths:
                paths = jtools.get_paths(args_['_id']['project'], args_['_id']['version']) 

            assert paths['code'] != '', 'ERROR use of prefix/subdir requires valid project_id'
            files['csv'] = '{}/{}/csvs/{}.csv.gz'.format(paths['code'], subdir, prefix)
            files['yml'] = '{}/{}/ymls/{}.yml'.format(paths['code'], subdir, prefix)

        # --- (2) Use explicit yml / csv files
        for k in ['csv', 'yml']:
            if k in kwargs:
                files[k] = kwargs[k]
