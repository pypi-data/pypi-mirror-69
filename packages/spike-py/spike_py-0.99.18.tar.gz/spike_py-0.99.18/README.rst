...
libraries. It is compatible and fully tested with python 3.7 (many parts
are still compatible with python 2.7 but this is not tested)
...
Or, if you want to play with the code, and get the bleeding edge
version.
...
Spectroscopies are defined in the ``NMR.py``, ``FTICR.py`` and
...
-  NPKData.py the main library, allows all processing for any kind of
