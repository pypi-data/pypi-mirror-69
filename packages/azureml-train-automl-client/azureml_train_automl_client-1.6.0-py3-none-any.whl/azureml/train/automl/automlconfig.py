...
def _automl_static_submit(automl_config_object: 'AutoMLConfig',
    ...
    if in_process is True:
        try:
            ...
            automl_run = _default_execution(experiment, settings, fit_params, True, show_output)
...
def _local_managed(experiment: Experiment,
    ...
    automl_run.add_properties({constants.SCRIPT_RUN_ID_PROPERTY: script_run.id})
    if show_output:
...
class AutoMLConfig(object):
    ...
    def __init__(self,
        ...
        force_curated_environment = kwargs.get('force_curated_environment', False)
        try:
