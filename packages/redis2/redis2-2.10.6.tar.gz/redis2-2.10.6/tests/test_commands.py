...
from redis2._compat import (unichr, u, b, ascii_letters, iteritems, iterkeys,
                            itervalues)
from redis2.client import parse_info
from redis2 import exceptions
...
class TestResponseCallbacks(object):
    ...
    def test_response_callbacks(self, r):
        assert r.response_callbacks == redis2.Redis.RESPONSE_CALLBACKS
        assert id(r.response_callbacks) != id(redis2.Redis.RESPONSE_CALLBACKS)
...
class TestRedisCommands(object):
    ...
    def test_command_on_invalid_key_type(self, r):
        r.lpush('a', '1')
        with pytest.raises(redis2.ResponseError):
            r['a']
