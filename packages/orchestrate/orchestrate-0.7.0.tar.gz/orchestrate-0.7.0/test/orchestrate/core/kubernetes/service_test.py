...
from orchestrate.core.exceptions import MissingGpuNodesException, NodesNotReadyError
from orchestrate.core.kubernetes.service import ORCHESTRATE_NAMESPACE, KubernetesService
...
class TestKubernetesService(object):
  ...
  def test_check_nodes_are_ready(self, kubernetes_service):
    ...
    kubernetes_service._get_nodes = MagicMock()
    kubernetes_service._get_nodes().items = [node_mock1, node_mock2]
  ...
  def test_check_nodes_are_not_ready_no_nodes(self, kubernetes_service):
    kubernetes_service._get_nodes = MagicMock()
    kubernetes_service._get_nodes().items = []

    with pytest.raises(NodesNotReadyError):
      kubernetes_service.check_nodes_are_ready()

  def test_check_gpu_nodes_raises_if_less_than_num_gpus(self, kubernetes_service):
    gpu_node_mock = Mock(status=Mock(capacity={'nvidia.com/gpu': 1}))
    kubernetes_service._get_nodes = MagicMock()
    kubernetes_service._get_nodes().items = [gpu_node_mock]

    with pytest.raises(MissingGpuNodesException):
      kubernetes_service.check_gpu_nodes(2)

  def test_check_gpu_nodes_raises_if_gpu_not_specified(self, kubernetes_service):
    gpu_node_mock = Mock(status=Mock(capacity={}))
    kubernetes_service._get_nodes = MagicMock()
    kubernetes_service._get_nodes().items = [gpu_node_mock]

    with pytest.raises(MissingGpuNodesException):
      kubernetes_service.check_gpu_nodes(1)

  def test_check_gpu_nodes_does_not_raise_if_enough_capacity(self, kubernetes_service):
    gpu_node_mock = Mock(status=Mock(capacity={'nvidia.com/gpu': 1}))
    kubernetes_service._get_nodes = MagicMock()
    kubernetes_service._get_nodes().items = [gpu_node_mock]
