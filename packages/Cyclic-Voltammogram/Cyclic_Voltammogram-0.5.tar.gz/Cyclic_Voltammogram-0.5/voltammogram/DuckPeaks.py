...
class Peaks(Voltammogram):
    ...
    def get_baseline_slope(self, inc = 2):
            ...
            for i in range(slicer-inc):
                s_swipe = start_scan+i
                x = self.x_values[s_swipe:s_swipe+inc]
                y = self.y_values[s_swipe:s_swipe+inc]
                res = linregress(x,y)
                if res.slope > 0:
                    slope_list.append(res.slope)
                    swiper_list.append(s_swipe)
            self.bslope = min(slope_list)
            self.s_flat = swiper_list[slope_list.index(self.bslope)]
            return self.bslope, self.s_flat
