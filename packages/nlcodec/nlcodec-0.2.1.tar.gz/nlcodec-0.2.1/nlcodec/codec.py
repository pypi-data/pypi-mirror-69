...
class WordScheme(EncoderScheme):
    ...
    @classmethod
    def term_frequencies_parallel(cls, data: Iterator[str]) -> Tuple[Dict[str, int], int]:
        line_count = 0
        def _line_counter():
            nonlocal line_count
            for line in tqdm(data, mininterval=1):
                line_count += 1
                yield line
        from nlcodec.mapred import WordCounter
        stats = WordCounter().multiprocess(_line_counter())
        log.info(f"Found {len(stats):,} types and {sum(stats.values()):,} tokens")
        return stats, line_count
