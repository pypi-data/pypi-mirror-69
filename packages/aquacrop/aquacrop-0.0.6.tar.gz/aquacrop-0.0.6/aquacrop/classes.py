...
__all__ = ['ClockStructClass', 'OutputClass', 'ParamStructClass', 'SoilClass', 'CropClass', 'FarmerClass',
           'FieldMngtClass', 'GwClass', 'InitWCClass', 'InitCondClass', 'WevapClass', 'spec', 'SoilCompClass', 'spec',
           'SoilLayerClass', 'SoilProfileClass', 'spec', 'TAWClass', 'spec', 'DrClass', 'spec', 'thRZClass', 'spec',
           'KswClass', 'spec', 'KstClass', 'spec', 'CO2Class', 'spec']
...
class ClockStructClass:
    ...
    `SimulationEndDate` : `np.Datetime64`:
        Date of simulation end

    `TimeStep` : `int`
        time step (not sure why this is in yet)

    `nSteps` : `int`
        total number of days of simulation

    `TimeSpan` : `np.array`
        all dates (np.Datetime64) that lie within the start and end dates of simulation

    `StepStartTime` : `np.Datetime64`
        Date at start of timestep

    `StepEndTime` : `np.Datetime64`
    ...
    `SimOffSeason` : `str`
        'Y' if you want to simulate the off season, 'N' otherwise

    `PlantingDates` : `list-like`
        list of planting dates in datetime format

    `HarvestDates` : `list-like`
        list of harvest dates in datetime format

    `nSeasons` : `int`
        Total number of seasons to be simulated

    `SeasonCounter` : `int`
        counter to keep track of which season we are currenlty simulating
    ...
    def __init__(self):
        ...
        self.TimeStep = 0 # time step (not sure why this is in yet)
        ...
        self.StepEndTime = 0 # Number of time-steps (per day) for soil evaporation calculation
        self.EvapTimeSteps = 20 # 'Y' if you want to simulate the off season, 'N' otherwise
        self.SimOffSeason = 0
        self.PlantingDates = []
        self.HarvestDates = []
        self.nSeasons = 0
...
class ParamStructClass:
    ...
    **Attributes**:\n
    ...
    `Crop List` : `list` : List of Crop Objects which contain paramaters for all crops used in simulations

    `FallowFieldMngt` : `pandas.Series` : Dictionary containing field management variables for the off season (fallow periods)
    ...
    `CropChoices` : `list` : List of crop type names in each simulated season, to be matched with planting and harvest dates, dtype str

    `CO2data` : `list` : CO2 data

    `CO2RefConc` : `float` : reference co2 concentration

    `CO2CurrentConc` : `float` : current co2 concentration

    `WaterTable` : `int` : Water table present (1=yes, 0=no)

    `zGW` : `pandas.Series` : WaterTable depth (mm) for each day of simulation
...
class SoilClass:
    ...
    **Attributes**:\n
    ...
    `profile` : `pandas.DataFrame` : holds soil profile information take from SoilProfile.txt
    ...
    A number of float attributes specified in the initialisation of the class
        ...
        '''
    def __init__(self,soilType,dz=[0.1]*6+[0.2]*4+[0.3]*2,CalcSHP = 0,nLayer=0,
                 zSoil=0.0,nComp=0,AdjREW= 1,REW= 9.0,CalcCN=1,CN=61.0,zRes=-999,
        ...
        # define Soil attributes that will be assigned in AOSP_ReadModelParameters

        self.CalcSHP= CalcSHP # Calculate soil hydraulic properties (0 = No, 1 = Yes)
        self.zSoil= zSoil # Total thickness of soil profile (m)
        self.nComp= nComp # Total number of soil compartments
        self.nLayer= nLayer # Total number of soil layers
        ...
        self.CalcCN= CalcCN # adjust Curve number
        self.CN= CN # Curve number
        self.zRes= zRes # Depth of restrictive soil layer (set to negative value if not present)

        if soilType == 'custom':
            self.create_df(dz)

        # estimates taken from saxton rawls 2006

        elif soilType == 'sand':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),88,5,2.5,100)

        elif soilType == 'loamy_sand':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),80,5,2.5,100)

        elif soilType == 'sandy_loam':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),65,10,2.5,100)

        elif soilType == 'loam':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),40,20,2.5,100)

        elif soilType == 'silt_loam':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),20,15,2.5,100)

        elif soilType == 'silt':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),10,5,2.5,100)

        elif soilType == 'sandy_clay_loam':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),60,25,2.5,100)

        elif soilType == 'clay_loam':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),30,35,2.5,100)

        elif soilType == 'silt_clay_loam':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),10,35,2.5,100)

        elif soilType == 'silt_clay':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),10,45,2.5,100)

        elif soilType == 'sandy_clay':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),50,40,2.5,100)

        elif soilType == 'clay':
            self.create_df(dz)
            self.add_layer_from_texture(sum(dz),25,50,2.5,100)

        elif soilType == 'ac_SandyLoam':
            self.CN = 46
            self.CalcCN = 0
            self.REW = 7
            self.create_df(dz)
        ...
        elif soilType == 'default':
            self.profile = pd.DataFrame(np.empty((12,10)),
                                        columns=["Comp","Layer","dz","dzsum","th_dry","th_wp","th_fc","th_s","Ksat","penetrability"])

            ksat = 500

            self.profile.dz = [0.1]*2 + [0.2]*8 + [0.3]*2
            self.profile.dzsum = np.cumsum(self.profile.dz)
            self.profile.Comp = np.arange(len(dz))
            self.profile.Layer = 1
            self.profile.th_dry = 0.15/2
            self.profile.th_wp = 0.15
            self.profile.th_fc = 0.31
            self.profile.th_s = 0.46
            self.profile.Ksat = ksat
            self.profile.penetrability = 100
    ...
    def fill_nan(self,):
        ...
        self.zSoil = round(self.profile.dzsum.iloc[-1],2)
...
class CropClass:
    ...
    **Attributes**:\n
    ...
    `PlantingDate` : `str` : Planting Date (dd/mm)

    `HarvestDate` : `str` : Latest Harvest Date (dd/mm)

    `IrrMngt`: `dict` :  dictionary containting irrigation management information. Added in AOSP_ReadIrrigationManagement
    ...
    `Params` : `Dict` :  Dictionary containing crop paramaters read from file of type float
    ...
    def __init__(self,c_name,**kwargs):
        ...
        self.Name = ''
        self.CropFilename = ''
        self.IrrigationFilename = ''
        self.FieldMngtFilename = ''
        ...
        if c_name == 'maize':
            ...
            self.PlantingDate= '05/01' # Planting Date (mm/dd)
            self.HarvestDate= '10/30' # Latest Harvest Date (mm/dd)
            ...
            self.SwitchGDD= 0; self.PlantingDate= '10/15'; self.HarvestDate= '04/30'
            ...
            self.SwitchGDD= 0; self.PlantingDate= '04/25'; self.HarvestDate= '08/30'
            ...
            self.SwitchGDD= 0; self.PlantingDate= '08/01'; self.HarvestDate= '11/30'
    ...
    def calculate_additional_params(self,):
        ...
        self.p_up = [self.p_up1,self.p_up2,self.p_up3,self.p_up4]

        self.p_lo = [self.p_lo1,self.p_lo2,self.p_lo3,self.p_lo4]

        self.fshape_w = [self.fshape_w1,self.fshape_w2,self.fshape_w3,self.fshape_w4]
...
class FarmerClass:
    ...
    **Attributes:**\n
    ...
    def get_irr_depth(self,NewCond):
        ...
        elif self.IrrMethod ==1:

            Dr  = NewCond.Depletion / NewCond.TAW

            index = int(NewCond.GrowthStage)-1
            #index = int(NewCond.DAP/10)


            if Dr > 1-self.SMT[index]/100:
                # Irrigation occurs
        ...
        elif self.IrrMethod ==2: # Irrigation - fixed interval
        ...
        elif self.IrrMethod == 3: # Irrigation - pre-defined schedule
            # Get current date
            CurrentDate = NewCond.CurrentDate
            # Find irrigation value corresponding to current date
            Irr = self.Schedule.loc[CurrentDate].Depth

            assert Irr>0; assert Irr < 10_000

            return min(self.MaxIrr,IrrReq)


        elif self.IrrMethod ==4: # Irrigation - net irrigation
        # Net irrigation calculation performed after transpiration, so
        # irrigation is zero here

            return 0

        elif self.IrrMethod ==5: # depth applied each day (usually specified outside of model)

            return self.depth

        else:
            print(f'somethings gone wrong in irrigation method:{self.IrrMethod}')
...
class FieldMngtClass:
    ...
    **Attributes:**\n
    ...
    `Mulches` : `str` :  Soil surface covered by mulches (Y or N)

    `Bunds` : `str` :  Surface bunds present (Y or N)

    `CNadj` : `str` : Field conditions affect curve number (Y or N)

    `SRinhb` : `str` : Management practices fully inhibit surface runoff (Y or N)
    ...
    '''
    def __init__(self,Mulches='N',Bunds='N',CNadj='N',SRinhb='N',
...
@jitclass(spec)
class SoilCompClass(object):
    """
    Stores informaiton about an individual soil compartment

    These are extracted from the pandas dataframe at initialization to speed up the model

    **Attributes:**\n


    `Comp` : `float` : compartment number

    `dz` : `float` : thickness (m)

    `Layer` : `float` : layer of compartment

    `dzsum` : `float` : depth of compartment

    `th_fc` : `float` : water content at field capacity

    `th_s` : `float` : water content at saturation

    `th_wp` : `float` : water content at wilting point

    `Ksat` : `float` : saturated conductivity

    `Penetrability` : `float` : penetrability

    `th_dry` : `float` : water content at air dry

    `tau` : `float` : drainage characteristic

    `zBot` : `float` : depth at bottom of compartment

    `zTop` : `float` : depth at top of compartment

    `zMid` : `float` : depth at mid point of compartment

    `th_fc_Adj` : `float` : adjusted water content at field capacity
...
class SoilLayerClass:
    """

    Stores information about each soil layer


    **Attributes:**\n


    `dz` : `float` : thickness of layer
    ...
    `aCR` : `float` : capillary rise param a

    `bCR` : `float` : capillary rise param b
