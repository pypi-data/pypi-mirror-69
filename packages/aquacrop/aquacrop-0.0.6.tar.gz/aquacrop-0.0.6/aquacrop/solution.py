    ...
    if (NewCond.ParamStruct.WaterTable == 1) and (NewCond.ParamStruct.WTMethod == 'Variable'):
        ...
        if NewCond.WTinSoil == True:
            ...
            for ii in range(idx,len(Soil.Profile.Comp)):
                NewCond.th[ii] = Soil.Profile.th_s[ii]
...
def root_development(Crop,Soil,InitCond,GDD,GrowingSeason):
    ...
    *Returns:*
    ...
    if GrowingSeason == True:
        ...
        if Zr > Crop.Zmin:
            layeri = 1
            l_idx = np.argwhere(Soil.Profile.Layer==layeri)
            Zsoil = Soil.Profile.dz[l_idx].sum()
            while (round(Zsoil,2) <= Crop.Zmin) and (layeri < Soil.nLayer):
                layeri = layeri+1
                l_idx = np.argwhere(Soil.Profile.Layer==layeri)
                Zsoil = Zsoil+Soil.Profile.dz[l_idx].sum()


            soil_layer_dz = Soil.Profile.dz[l_idx].sum()
            layer_comp = l_idx[0]
            ...
            EndProf = False
            while EndProf == False:
                ZrTest = ZrAdj+(ZrRemain*(Soil.Profile.Penetrability[layer_comp]/100))
                if (layeri == Soil.nLayer) or \
                (Soil.Profile.Penetrability[layer_comp]==0) or \
                (ZrTest<=Zsoil):
                    ZrOUT = ZrTest
                    EndProf = True
                else:
                    ZrAdj = Zsoil
                    ZrRemain = ZrRemain-(deltaZ/(Soil.Profile.Penetrability[layer_comp]/100))
                    layeri = layeri+1
                    l_idx = np.argwhere(Soil.Profile.Layer==layeri)
                    layer_comp = l_idx[0]
                    soil_layer_dz = Soil.Profile.dz[l_idx].sum()
...
def irrigation(InitCond,IrrMngt,Crop,Soil,ClockStruct,GrowingSeason,Rain,Runoff,Farmer):
    ...
    *Returns:*
    ...
    if GrowingSeason == True:
        ...
        WrAct,Dr_,TAW_,thRZ = root_zone_water(Soil.Profile,float(NewCond.Zroot),NewCond.th,Soil.zTop,float(Crop.Zmin),Crop.Aer)
...
def capillary_rise(Soil,InitCond,FluxOut):
    ...
    *Returns:*
    ...
    if NewCond.ParamStruct.WaterTable == 0: # No water table present
        ...
        zTopLayer = 0
        for layeri in np.sort(Soil.Profile.Layer.unique()):
            # Calculate layer thickness
            l_idx = np.argwhere(Soil.Profile.Layer==layeri)

            LayThk = Soil.Profile.dz[l_idx].sum()
        ...
        layeri = Soil.Profile.Layer[-1]

        assert layeri == Soil.nLayer, "needs fixing"
        ...
        while (round(MaxCR*1000)>0) and (compi > -1) and (round(FluxOut[compi]*1000) == 0):
            ...
            if (NewCond.th[compi] >= prof.th_wp[compi]) and (Soil.fshape_cr > 0):
                Df = 1-(((NewCond.th[compi]-prof.th_wp[compi])/ \
                         (NewCond.th_fc_Adj[compi]-prof.th_wp[compi]))**Soil.fshape_cr)
                if Df > 1:
                    Df = 1
                elif Df < 0:
                    Df = 0
            ...
            if compi > -1:
                ...
                if (prof.Ksat > 0) and (zGW > 0) and ((zGW-zBotMid) < 4):
                    if zBotMid >= zGW:
                        LimCR = 99
                    else:
...
# Cell
def germination(InitCond,Soil_zGerm,prof,Crop,GDD,GrowingSeason):
    """
...
def canopy_cover(Crop,Soil,InitCond,GDD,Et0,GrowingSeason):
    ...
    *Returns:*
    ...
    if GrowingSeason == True:
        ...
        _,Dr,TAW,_ = root_zone_water(Soil.Profile,float(NewCond.Zroot),NewCond.th,Soil.zTop,float(Crop.Zmin),Crop.Aer)
...
def soil_evaporation(ClockStruct,Soil,Crop,IrrMngt,FieldMngt,InitCond,Et0,Infl,Rain,Irr,GrowingSeason):
    ...
    *Returns:*
    ...
    if (ClockStruct.TimeStepCounter == 0) or ((NewCond.DAP == 1) and \
        ...
        Wevap.Sat,Wevap.Fc,Wevap.Wp,Wevap.Dry,Wevap.Act = evap_layer_water_content(NewCond.th,NewCond.EvapZ,Soil.Profile)
    ...
    if (ExtractPotStg1 > 0):
        # Find soil compartments covered by evaporation layer
        comp_sto = np.sum(Soil.Profile.dzsum<Soil.EvapZmin)+1
        comp = -1
        prof=Soil.Profile
        while (ExtractPotStg1 > 0) and (comp < comp_sto):
            ...
            if prof.dzsum[comp] > Soil.EvapZmin:
                factor = 1-((prof.dzsum[comp]-Soil.EvapZmin)/ prof.dz[comp])
            else:
                factor = 1
    ...
    if ToExtract > 0:
        ...
        for jj in range(int(ClockStruct.EvapTimeSteps)):
            ...
            comp_sto = np.sum(Soil.Profile.dzsum<NewCond.EvapZ)+1
            comp = -1
            prof=Soil.Profile
...
def transpiration(Soil,Crop,IrrMngt,InitCond,Et0,CO2,GrowingSeason,GDD):
    ...
    *Returns:*
    ...
    if GrowingSeason == True:
        ...
        # content
        _,Dr,TAW,thRZ = root_zone_water(Soil.Profile,float(NewCond.Zroot),NewCond.th,Soil.zTop,float(Crop.Zmin),Crop.Aer)
        # Check whether to use root zone or top soil depletions for calculating
        # water stress
        if (Dr.Rz/TAW.Rz) <= (Dr.Zt/TAW.Zt):
            # Root zone is wetter than top soil, so use root zone value
        ...
        comp_sto = min(np.sum(Soil.Profile.dzsum<rootdepth)+1,int(Soil.nComp))
        ...
        if (IrrMngt.IrrMethod == 4) and (TrPot > 0):
            ...
            _,_Dr,_TAW,thRZ = root_zone_water(Soil.Profile,float(NewCond.Zroot),NewCond.th,Soil.zTop,float(Crop.Zmin),Crop.Aer)
            ...
            if thRZ.Act < thCrit:
                ...
                for ii in range(comp_sto):
                    # Get soil layer
                    layeri = Soil.Profile.Layer[ii]
                    if layeri > prelayer:
                        # If in new layer, update critical water content for
                        # net irrigation
                        thCrit = prof.th_wp[ii]+((IrrMngt.NetIrrSMT/100)* \
...
def groundwater_inflow(Soil,InitCond):
    ...
    *Returns:*
    ...
    if NewCond.WTinSoil == True:
        ...
        idx = np.argwhere([zMid >= zGW])[0].flatten()[0]
