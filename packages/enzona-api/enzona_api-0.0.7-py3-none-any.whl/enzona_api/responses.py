...
class response_payments():
    ...
    def transaction_uuid(self):
        try: return self.response["transaction_uuid"]
        except: raise EnzonaError(self.response["fault"]["message"])
    def link_confirm(self):
        try: return self.response["links"][0]["href"]
        except: raise EnzonaError(self.response["fault"]["message"])
    def link_complete(self):
        try: return self.response["links"][1]["href"]
        except: raise EnzonaError(self.response["fault"]["message"])
    def link_cancel(self):
        try: return self.response["links"][2]["href"]
        except: raise EnzonaError(self.response["fault"]["message"])
...
class response_operation_payments():
    ...
    def status_denom(self):
        try: return self.response["status_denom"][0]
        except: raise EnzonaError(self.response["fault"]["message"])
...
class response_return_payments():
    ...
    class payment():
        ...
        def amount_discount(self): return self.response["amount"]["details"]["discount"]
        def amount_shipping(self): return self.response["amount"]["details"]["shipping"]
...
class response_refound():
    ...
    def created_at(self):
        try: return self.response["created_at"]
        except: raise EnzonaError(self.response["fault"]["message"])
    def amount_total(self):
        try: return self.response["amount"]["total"]
        except: raise EnzonaError(self.response["fault"]["message"])
    def description(self):
        try: return self.response["description"]
        except: raise EnzonaError(self.response["fault"]["message"])
    def refund_fullname(self):
        try: return self.response["refund_name"] + " " + self.response["refund_lastname"]
        except: raise EnzonaError(self.response["fault"]["message"])
        ...
        try:
            ...
            for refund in refunds:
                lst_refunds.append({
                    ...
                    "total_refunded": refund["amount"]["details"]["total_refunded"],
