...
from sympy import Symbol,log,E
...
class Least_squares:
    ...
    参数x：x轴坐标
    参数y：y轴坐标
    参数n：拟合函数类型，包括None（一阶线性方程），EXPONENTIAL_FUNCTION（指数方程）
    '''
    ...
    def get_mean_square_error(self):
        ...
        if self.n == None:
            ...
            for i in range(len(self.y)):
                self.__M += self.y[i] - (self.K * (E ** (self.M * self.x[i])))
            return self.__M ** 0.5

if __name__ == '__main__':
    l = Least_squares([3,6,9,12,15,18,21,24],[57.6,41.9,31,22.7,16.6,12.2,8.9,6.5],n=EXPONENTIAL_FUNCTION)
