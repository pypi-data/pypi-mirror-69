...
def import_auth():
 try:
  ...
  raise WMCYX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=WMCYU)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=WMCYO)
def get_params(args,names=[],mandatory=WMCYU):
 result=()
 for name in names:
  value=WMCYf
  if name in args:
  ...
  if not value and mandatory:
   raise WMCYX('Please provide %s=... parameter'%name)
  result+=(value)
 if WMCYm(result)==1:
  return result[0]
 return result
