...
def import_auth():
 try:
  ...
  raise spxAE('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=spxAR)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=spxAc)
def get_params(args,names=[],mandatory=spxAR):
 result=()
 for name in names:
  value=spxAH
  if name in args:
  ...
  if not value and mandatory:
   raise spxAE('Please provide %s=... parameter'%name)
  result+=(value)
 if spxAa(result)==1:
  return result[0]
 return result
