...
class AttrDict(HDBFz):
 def __init__(self,*args,**kwargs):
  HDBFO(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and HDBFY(socket,'IPV6_V6ONLY'):
...
class Half(HDBFy):
 def __init__(self,opt,sock,addr,HDBFa):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.HDBFa=HDBFa
  self.name='peer' if self.HDBFa else 'client'
  self.queue=[]
  self.dest=HDBFp
  self.err=HDBFp
  self.ready=HDBFG
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':HDBFG,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':HDBFG,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':HDBFG,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':HDBFG,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
