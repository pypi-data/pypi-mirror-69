...
def import_auth():
 try:
  ...
  raise xcqWK('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=xcqWY)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=xcqWt)
def get_params(args,names=[],mandatory=xcqWY):
 result=()
 for name in names:
  value=xcqWE
  if name in args:
  ...
  if not value and mandatory:
   raise xcqWK('Please provide %s=... parameter'%name)
  result+=(value)
 if xcqWD(result)==1:
  return result[0]
 return result
