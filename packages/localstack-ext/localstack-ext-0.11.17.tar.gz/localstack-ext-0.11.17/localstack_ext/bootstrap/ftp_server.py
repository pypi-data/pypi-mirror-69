...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':CrgAj(perm='m',auth=CrgAz,arg=CrgAz,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=CrgAz)
def do_start_ftp(port,asynchronous=CrgAz):
...
def main():
 do_start_ftp(CrgAW(sys.argv[1]),asynchronous=CrgAG)
