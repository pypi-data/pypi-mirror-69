...
def import_auth():
 try:
  ...
  raise nHRfX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=nHRfL)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=nHRfk)
def get_params(args,names=[],mandatory=nHRfL):
 result=()
 for name in names:
  value=nHRfA
  if name in args:
  ...
  if not value and mandatory:
   raise nHRfX('Please provide %s=... parameter'%name)
  result+=(value)
 if nHRfW(result)==1:
  return result[0]
 return result
