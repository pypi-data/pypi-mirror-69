...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':nCkLs(perm='m',auth=nCkLc,arg=nCkLc,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=nCkLc)
def do_start_ftp(port,asynchronous=nCkLc):
...
def main():
 do_start_ftp(nCkLT(sys.argv[1]),asynchronous=nCkLx)
