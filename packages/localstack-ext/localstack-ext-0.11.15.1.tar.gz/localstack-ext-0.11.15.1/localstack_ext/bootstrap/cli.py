...
def import_auth():
 try:
  ...
  raise pTrjx('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=pTrjS)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=pTrjz)
def get_params(args,names=[],mandatory=pTrjS):
 result=()
 for name in names:
  value=pTrjU
  if name in args:
  ...
  if not value and mandatory:
   raise pTrjx('Please provide %s=... parameter'%name)
  result+=(value)
 if pTrjH(result)==1:
  return result[0]
 return result
