...
def import_auth():
 try:
  ...
  raise RJPeg('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RJPeM)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RJPeE)
def get_params(args,names=[],mandatory=RJPeM):
 result=()
 for name in names:
  value=RJPea
  if name in args:
  ...
  if not value and mandatory:
   raise RJPeg('Please provide %s=... parameter'%name)
  result+=(value)
 if RJPep(result)==1:
  return result[0]
 return result
