...
def import_auth():
 try:
  ...
  raise dSxuX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=dSxuM)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=dSxuA)
def get_params(args,names=[],mandatory=dSxuM):
 result=()
 for name in names:
  value=dSxuz
  if name in args:
  ...
  if not value and mandatory:
   raise dSxuX('Please provide %s=... parameter'%name)
  result+=(value)
 if dSxuq(result)==1:
  return result[0]
 return result
