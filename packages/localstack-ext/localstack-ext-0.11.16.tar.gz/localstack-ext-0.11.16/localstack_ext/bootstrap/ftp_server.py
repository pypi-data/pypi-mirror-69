...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':vUQgy(perm='m',auth=vUQgi,arg=vUQgi,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=vUQgi)
def do_start_ftp(port,asynchronous=vUQgi):
...
def main():
 do_start_ftp(vUQgP(sys.argv[1]),asynchronous=vUQgu)
