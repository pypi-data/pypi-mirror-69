...
def import_auth():
 try:
  ...
  raise UrBJg('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=UrBJG)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=UrBJt)
def get_params(args,names=[],mandatory=UrBJG):
 result=()
 for name in names:
  value=UrBJj
  if name in args:
  ...
  if not value and mandatory:
   raise UrBJg('Please provide %s=... parameter'%name)
  result+=(value)
 if UrBJn(result)==1:
  return result[0]
 return result
