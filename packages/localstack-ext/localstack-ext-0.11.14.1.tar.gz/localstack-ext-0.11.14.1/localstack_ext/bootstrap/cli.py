...
def import_auth():
 try:
  ...
  raise rIkQY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=rIkQb)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=rIkQq)
def get_params(args,names=[],mandatory=rIkQb):
 result=()
 for name in names:
  value=rIkQt
  if name in args:
  ...
  if not value and mandatory:
   raise rIkQY('Please provide %s=... parameter'%name)
  result+=(value)
 if rIkQu(result)==1:
  return result[0]
 return result
