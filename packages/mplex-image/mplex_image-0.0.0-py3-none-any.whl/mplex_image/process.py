...
def add_exposure_roundscyles(df_tc, df_expc,es_standard,s_dapi = 'DAPI12_Nuclei'):
    """
    ...
    df_t['location'] = ''
    df_t.drop( s_dapi.split('_')[0], inplace=True)
...
def af_subtract_images(df_t,d_channel={'c2':['L488','L488'],'c3':['L555','L555'],'c4':['L647','L647'],'c5':['L750','L750']},s_dapi='DAPI11_Nuclei',b_mkdir=True):
    ...
    for s_slide in ls_slide:
        ...
        for s_marker_loc in ls_sub:
            ...
            a_img = io.imread(s_image)
            a_AF = io.imread(s_background)
...
def check_seg(s_sample= 'sampleID',ls_find=['Cell Segmentation Full Color'], i_rows=2, t_figsize=(20,10)):
    ...
    for s_find in ls_find:
        ...
        for idx, s_scene in enumerate(ls_scene):
            ...
            im_low = io.imread(ls_dapis[idx])#,plugin='simpleitk'
            im = exposure.rescale_intensity(im_low,in_range=(np.quantile(im_low,0.02),np.quantile(im_low,0.98)+np.quantile(im_low,0.98)/2))
