...
def add_celltype(df_data, ls_cell_names, s_type_name):
    ...
    if not os.path.exists('./Gating_Record.json'):
        ...
        with open(f'Gating_Record.json','w') as f: 
            json.dump(d_current, f)

def plot_cluster(s_sample, df_pos, df_xy,figsize = (10,10),s_label='Label'):
    ...
    ls_scene = sorted(set(df_pos.scene))#[df_pos.tissue==s_sample]
    for s_scene in ls_scene:
        #negative cells = all cells even before dapi filtering
        #plot negative cells
        fig,ax = plt.subplots(figsize=figsize)
        sns.scatterplot(data=df_xy[(df_xy.scene==s_scene)],x='DAPI_X',
                                                                    y='DAPI_Y',
                                                                    color='gray',
                                                                    #size=0.5,
                                                                    ax=ax,
                                                                    )
        for idx, s_color in enumerate(list(set(df_pos.loc[:,s_label]))):
            #print(idx+1)
            #positive cells = positive cells based on high and low thresholds
            df_color_pos_loc = df_pos[df_pos.loc[:,s_label] == s_color]
            if len(df_color_pos_loc[(df_color_pos_loc.scene==s_scene)])>=1:
                #plot positive cells
                #print(len(df_color_pos_loc[(df_color_pos_loc.scene==s_scene) & (df_color_pos_loc.tissue==s_sample)]))
                sns.scatterplot(data=df_color_pos_loc[(df_color_pos_loc.scene==s_scene)],x='DAPI_X',
                                                                    y='DAPI_Y',
                                                                    hue=idx+1,
                                                                    label=f'{s_color}',
                                                                    ax=ax,
        ...
        ax.axis('equal')
        ax.set_title(f'{s_sample}_{s_scene}')
        ax.set_ylim(ax.get_ylim()[::-1])
        plt.legend(markerscale=3) 
        fig.savefig(f'{s_sample}_{s_scene}_Cluster_scatterplot.png')

def measure_tissue_intensity(df_img, a_dapi):
    '''
    #calculate mean intensity in pixels with R1 dapi (c1) above 500
    '''
    #load the dapi image and create a mask of dapi>500
    d_result = {}
    ...
    for s_index in df_img.index:
        ...
        a_img_total = a_img[a_dapi]
        i_img_meanint = a_img_total.sum()/a_img_total.size
        d_img = {s_index:i_img_meanint}
        d_result.update(d_img)

    #save results to csv    
    se_result = pd.Series(d_result)
    df_img['result']=se_result
    #df_img.to_csv(f'result_{s_tissue}.csv')

    return(df_img)
    #save an png showing area of measurement
    #skimage.io.imsave(f'area_of_measurement_{s_tissue}.png',arr=a_dapi_thresh)

def load_thresh():
    """
    load the manual thresholds  (saved in Jenny's jupyter notebook)
    
    """

def load_pos(s_sample='SampleID',s_dapi='DAPI11_Nuclei',dapi_thresh=1000):
    """
    load the positive dataframe (output of Jenny's jupyter notebook)
    """
    #load positive dataframe
    print(f'{s_sample}_{s_dapi}{dapi_thresh}_ManualPositive.txt')
    df_pos = pd.read_csv(
        f'{s_sample}_{s_dapi}{dapi_thresh}_ManualPositive.txt',
        sep='\t',
        index_col=0
    )
    return(df_pos)

def make_border(s_sample,df_pos,ls_color,segmentdir,savedir,b_images=True): #,b_makedir=True
    """
    ...
    for s_file in ls_cellseg:
        s_scene_num = s_file.split('Scene')[1].split(' ')[1].split('_')[0]
        print(s_file)
        ...
        if b_images:
            ...
            for idx, s_color in enumerate(ls_color):
                ...
                if len(df_color_pos[(df_color_pos.scene==s_scene)])>=1:
                    ...
                    #filename hack
                    print('saving image')
                    io.imsave(f'{savedir}/R{idx+100}_{s_color.replace("_",".")}_{df_color_pos.index[0].split("_")[0]}-{s_scene.replace("scene","Scene-")}_border_c2_ORG.tif',a_pos_border)
                else:
        ...
        if b_images:
            ...
            #    print(f'Processing {s_color}')
                ...
                if len(df_color_pos[(df_color_pos.scene==s_scene)])>=1:
                    ...
                    io.imsave(f'{savedir}/R{idx+100}_all.all_{df_color_pos.index[0].split("_")[0]}-{s_scene.replace("scene","Scene-")}_border_c3_ORG.tif',a_pos_border)
