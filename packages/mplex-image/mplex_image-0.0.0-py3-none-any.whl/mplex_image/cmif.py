...
from cmIF import preprocess, mpimage, getdata, process, analyze
import copy
import os
import javabridge
import bioformats
...
def prepare_dataframe(s_sample,s_dapi,dapi_thresh,d_channel,ls_exclude,segdir,codedir,s_af='none', b_afsub=False):
    '''
    ...
    df_filter_mi, es_standard = process.filter_standard(df_dapi_mi,d_channel,s_dapi=s_dapi)
    ...
    df_tt = process.add_exposure_roundscyles(df_t, df_exp,es_standard, s_dapi = s_dapi)
    ...
    if b_afsub:
        ...
        df_sub.to_csv(f'features_{s_sample}_AFSubtractedMeanIntensityNegative{s_af}_{s_dapi}{dapi_thresh}.csv')
        df_out.to_csv(f'features_{s_sample}_AFSubtractedMeanIntensity{s_af}_{s_dapi}{dapi_thresh}.csv')
