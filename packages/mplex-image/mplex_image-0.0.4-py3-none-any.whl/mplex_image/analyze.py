...
def make_border(s_sample,df_pos,ls_color,segmentdir,savedir,b_images=True): 
    """
        ...
        if b_images:
            ...
            for idx, s_color in enumerate(ls_color):
                ...
                if len(df_color_pos[(df_color_pos.scene==s_scene)])>=1:
                    ...
                    #filename hack
                    print('saving image')
                    skimage.io.imsave(f'{savedir}/R{idx+100}_{s_color.replace("_",".")}_{df_color_pos.index[0].split("_")[0]}-{s_scene.replace("scene","Scene-")}_border_c2_ORG.tif',a_pos_border)
                else:
