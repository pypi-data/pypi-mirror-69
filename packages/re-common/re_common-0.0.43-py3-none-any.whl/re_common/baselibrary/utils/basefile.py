...
class BaseFile(_io.TextIOWrapper):
    ...
    @classmethod
    def single_write_file(cls, value, files, encoding="utf-8"):
        """
        单次将内容写入文件
    ...
    @classmethod
    def single_write_file_lines(cls, value_list, files, encoding="utf-8"):
        """
        单次将内容写入文件
        写入值到文件 每次写入会覆盖原来的文件  文件不存在会自动创建
        :param value:
    ...
    @classmethod
    def single_write_wb_file(cls, value, files):
        """
        单次将内容写入文件
        写入值到文件 每次写入会覆盖原来的文件  文件不存在会自动创建
        :param value:
    ...
    @classmethod
    def single_write_ab_file(cls, value, files):
        """
        单次将内容写入文件
    ...
    @classmethod
    def add_file_ab_mode(cls, value, filePath):
        """
        追加文件中的所有值
        :param filePath:
        :param encoding:
    ...
    @classmethod
    def get_filename_not_extsep(cls, filename):
        """
        获取文件名  去除后缀和路径
        :param filename:
        :return:
        ...
        if not cls.is_file_exists(filepath):
            ...
            if cls.get_file_size(filepath) >= size:
                ...
                if cls.is_file_exists(path):
                    return cls.change_file(path, sign=sign, size=size)
                return path
            else:
                return filepath
