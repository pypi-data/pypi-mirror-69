...
class ExtractTable:
    ...
    def trigger_process(self, fp: BinaryIO, dup_check: bool = False, **kwargs) -> dict:
        ...
        if 'JobId' in resp and max_wait_time > 0:
    ...
    def lookup_form_values(self, seperator: list = None, find_for: list = None) -> list:
        """
        To look up form values from the text
        :param seperator: keyvalue seperator like :, =, - et al
        :param find_for: keyword specific like Name, Total Amount et al
        :return: pagewise list of captured key-value pairs
        """
        result = {}
        ...
        find_for = find_for if find_for else []

        for each_page in self.Lines:
            temp = []
            page = each_page['Page']
            for each_line in each_page['LinesArray']:
                temp.append(each_line['Line'].strip())

            result[page] = []

            for clue in seperator:
                tmp = temp.copy()
                if clue not in tmp:
                    continue
                clue_idx = 0
                tmp = tmp[clue_idx:]
                while tmp.index(clue):
                    clue_idx = tmp.index(clue)
                    result[page].append({tmp[clue_idx-1]: tmp[clue_idx+1]})
                    tmp = tmp[clue_idx+1:]
                    if clue not in tmp:
                        break

            for clue in find_for:
                tmp = temp.copy()
                if clue not in tmp:
                    continue
                clue_idx = 0
                ...
                while tmp.index(clue):
                    clue_idx = tmp.index(clue)
                    result[page].append({clue: tmp[clue_idx+1]})
                    tmp = tmp[clue_idx+1:]
                    if clue not in tmp:
                        break

        return [{'Page': k, 'KeyValueList': v} for k, v in result.items()]
