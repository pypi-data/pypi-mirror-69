...
class DatabaseWorker:
    ...
    def RegisterDBURI(self, dburi):
        ...
        self._db = dataset.connect(
            self._dburi,
            engine_kwargs={'connect_args': {'check_same_thread': False}} if 'sqlite' in self._dburi else None # to avoid error; ProgrammingError: SQLite objects created in a thread can only be used in that ame thread.The object was created in thread id 23508 and this is thread id 640
        )
    ...
    def _WorkToDo(self):
        ...
        for thisDict in [
            # self._inUseQ, # there is no work to be done with inUseQ,
            # its just holding items until they are placed in the commit Q
            self._commitQ,
            self._deleteQ,
        ]:
