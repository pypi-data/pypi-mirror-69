...
class StructuredLog(object):
    ...
    _EVENT_SANITIZATION_SUBSTRINGS = ['authentication', 'authorization', 'secret', 'token',
                                      'password', 'cookie']
    ...
    def sanitize_event(self) -> None:
        if isinstance(self._log['event'], Iterable):
            self._log['event'] = self._make_sanitized_copy(self._log['event'])
    ...
    @classmethod
    def _make_sanitized_copy(cls, input: Iterable) -> Iterable:
        input_copy = copy.deepcopy(input)
        cls._recursive_sanitize(input_copy)
        return input_copy

    @classmethod
    def _recursive_sanitize(cls, input: Iterable) -> Iterable:
        if isinstance(input, dict):
            for key in input:
                if cls._key_might_be_secret(key):
                    input[key] = "<SANITIZED>"
            for val in input.values():
                cls._recursive_sanitize(val)
        elif isinstance(input, Iterable) and not isinstance(input, str):
            for val in input:
                cls._recursive_sanitize(val)

    @classmethod
    def _key_might_be_secret(cls, input: str) -> bool:
        key_lower = input.lower()
        for substr in cls._EVENT_SANITIZATION_SUBSTRINGS:
