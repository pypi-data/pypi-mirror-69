...
class KnowledgePack(VisualizeMixin):
    ...
    def get_positive_predictive_indexes(self, class_name, metric_set="validation"):
        """
        ...
        count = 0
        indexes = []
        for i, pred in enumerate(self.model_results["metrics"][metric_set]["y_pred"]):
            if pred == class_name:
                indexes.append(i)

        return indexes

    def get_false_positive_indexes(self, class_name, metric_set="validation"):
        """
        ...
        for i, pred in enumerate(self.model_results["metrics"][metric_set]["y_pred"]):
            if (
                pred == class_name
                and self.model_results["metrics"][metric_set]["y_true"][i] != class_name
            ):
                indexes.append(i)

        return indexes

    def get_true_positive_indexes(self, class_name, metric_set="validation"):
        """
