...
class DCLProject:
    ...
    def _list_table(self, tablename, fields, fk_fields=None, query_filter=None):
        ...
        if query_filter:
            query_where = (
                query_filter
            )  # + "AND {}.local_status != 2 ".format(tablename)
        else:
    ...
    def list_capture_segments(self, capture=None, segmenter=None):

        query_filter = ""

        if capture and segmenter:
            query_filter = 'WHERE Capture.name="{}" '.format(
                capture
            ) + 'AND Segmenter.name = "{0}" '.format(segmenter)

        elif capture:
            query_filter = 'WHERE Capture.name="{}" '.format(capture)
        elif segmenter:
            query_filter += 'WHERE Segmenter.name = "{0}" '.format(segmenter)
    ...
    def get_capture_segments(self, capture, segmenter):

        labels = self.list_capture_segments(capture=capture, segmenter=segmenter)
