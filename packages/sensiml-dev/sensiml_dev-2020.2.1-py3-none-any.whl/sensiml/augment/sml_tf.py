...
def visualize_features(event_index, x_train, y_train, row_size, num_cols):
...
def visualize_tf_data(
    data, index, row_size, num_cols,
):
    count = 0
...
def plot_training_results(history, figsize=(32, 10)):
    fig, axs = plt.subplots(2, 2, figsize=figsize)
    epochs = range(1, len(train_history["loss"]) + 1)
    axs[0][0].plot(epochs, train_history["loss"], "g.", label="Training los    s")
    axs[0][0].plot(epochs, train_history["val_loss"], "b", label="Validation loss")
    axs[0][0].set_title("Training and validation loss")
    axs[0][0].set_xlabel("Epochs")
    axs[0][0].set_ylabel("Loss")
    axs[1][0].plot(epochs, train_history["accuracy"], "g.", label="Training Accuracy")
    axs[1][0].plot(
        epochs, train_history["val_accuracy"], "b", label="Validation Accuracy"
    )
