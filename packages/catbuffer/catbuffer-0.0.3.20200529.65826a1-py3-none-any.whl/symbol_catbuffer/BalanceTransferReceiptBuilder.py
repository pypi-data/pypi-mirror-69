...
class BalanceTransferReceiptBuilder(ReceiptBuilder):
    ...
    Attributes:
        mosaic: Mosaic.
        senderPublicKey: Mosaic sender public key.
        recipientAddress: Mosaic recipient address.
    """

    def __init__(self, size: int, version: int, type: ReceiptTypeDto, mosaic: MosaicBuilder, senderPublicKey: KeyDto, recipientAddress: AddressDto):
        """Constructor.
        Args:
            size: Entity size.
    ...
    def loadFromBinary(cls, payload: bytes) -> BalanceTransferReceiptBuilder:
        ...
        senderPublicKey = KeyDto.loadFromBinary(bytes_)  # kind:CUSTOM1
        bytes_ = bytes_[senderPublicKey.getSize():]
        recipientAddress = AddressDto.loadFromBinary(bytes_)  # kind:CUSTOM1
        bytes_ = bytes_[recipientAddress.getSize():]
        return BalanceTransferReceiptBuilder(superObject.size, superObject.version, superObject.type, mosaic, senderPublicKey, recipientAddress)
    ...
    def getSenderPublicKey(self) -> KeyDto:
    ...
    def getSize(self) -> int:
        ...
        size += self.senderPublicKey.getSize()
    ...
    def serialize(self) -> bytes:
        ...
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, self.senderPublicKey.serialize())  # kind:CUSTOM
