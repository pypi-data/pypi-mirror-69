	...
	def at (self, at, func, args = ()):
		...
		with self._lock:
			...
			if last_at and at < last_at:
				sort (self.onces, key = lambda x: x [1])
		return call_id
...
PREFER_EPOLL = False
ALLOWED_POLL_ERROR = 8
poll_fun = asyncore.poll
if os.name == 'posix' and '__pypy__' not in sys.builtin_module_names:
	poll_fun = asyncore.epoll
	PREFER_EPOLL = True
poll_errs = 0

def maybe_poll_fallback ():
	global _logger, poll_fun, poll_errs

	poll_errs += 1
	if poll_errs > ALLOWED_POLL_ERROR:
		if poll_fun is asyncore.poll:
			raise SystemError ('Too many polling error')
		_logger and _logger ('fallback to asyncore.poll', 'warn')
		poll_fun = asyncore.poll # fallback
		poll_errs = 0
...
def poll_fun_wrap (timeout, map = None):
	...
	try:
		...
		_logger and _logger.trace ()
		maybe_poll_fallback ()
		remove_notsocks (map) # WSAENOTSOCK
		...
		if poll_errs:
			poll_errs = 0 # reset error count
			if PREFER_EPOLL:
				_logger and _logger ('reinstate asyncore.epoll', 'info')
				poll_fun = asyncore.epoll
