...
import boto3
from sdc_engine_helpers.maintenance.date_utils import DateUtils
from sdc_engine_helpers.maintenance.campaign_state_manager import CampaignStateManager
from sdc_engine_helpers.maintenance.solution_state_manager import SolutionStateManager
...
class MaintenanceManager:
    ...
    def perform_maintenance(self):
        ...
        for client_id, subscription_properties in subscriptions_properties.items():
            ...
            solutions = subscription_properties.get('solutions', None)
            if solutions is not None:
                ...
                if response.get('solutions_updated') is True:
                    subscription_properties['solutions'] = response.get('solutions')
                    update_subscriptions = True


            # If the subscription contains campaigns, perform campaign maintenance
            campaigns = subscription_properties.get('campaigns', None)
            if campaigns is not None:
                ...
                if response.get('campaigns_updated') is True:
                    subscription_properties['campaigns'] = response.get('campaigns')
                    update_subscriptions = True
