...
class FeatureScaler:
    """\
    Transform feature numbers into a standardised measure for the NN
    """
    ...
    def transform(self, vals):
        return (vals-self.means)/self.stds

    def inverse_transform(self, vals):
        return vals*self.stds + self.means

    def __repr__(self):
        return "salami.FeatureScaler<size={}>".format(len(self.means))
...
class XsecLnScaler:
    ...
    def __repr__(self):
        return "salami.XsecLnScaler<offset={}>".format(self.offset)
...
class KPred:
    ...
    def __init__(self, energy, process):
        ...
        try:
            import pickle
            with open(getpath("kfactor_objects.pkl"), "rb") as pf:
                tools = pickle.load(pf)
            self.feature_indices = tools["FEATURE_INDICES"].tolist()
            self.feature_indices.remove(149)
            self.feature_ranges = tools["FEATURE_RANGES"]
            fsparams = tools["FEATURE_SCALE_PARAMS"]
            self.feature_scaler = FeatureScaler(fsparams["MEANS"], fsparams["VARS"]**0.5)
            xsparams = tools["LOXSEC_SCALE_PARAMS"]
            self.loxsec_scaler = XsecLnScaler(xsparams)
