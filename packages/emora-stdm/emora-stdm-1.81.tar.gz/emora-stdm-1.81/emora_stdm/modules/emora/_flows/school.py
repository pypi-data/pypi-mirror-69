...
system = {
    'state': 'start',
    'enter': '#GATE #GCOM(school) #GOAL(school) #IF($life_in=True)',
    ...
    '#GSRET(default_start) #GATE':{
        ...
        '[{#AGREE,[!-not,{correct,true,right}],[!i am,-not],[!-not,[i,#LEM(take),#LEM(class,course)]]}]'
        '#SET($is_student=True)':{
        ...
        '[#NOT(not),{college,university}]'
        '#SET($is_student=True,$in_college=True)':{
            'state': 'user_is_student',
            'score': 1.1
        },

        '[#NOT(not),{high school,high schooler}]'
        '#SET($is_student=True,$in_high_school=True)': {
            'state': 'user_is_student',
            'score': 1.1
        },

        '[#NOT(not),{middle school,middle schooler,[!{sixth,seventh,eighth} grader],[!{sixth,seventh,eighth} grade]}]'
        '#SET($is_student=True,$in_middle_school=True)': {
            'state': 'user_is_student',
            'score': 1.1
        },

        '[#NOT(not),{elementary school,elementary schooler,[!{first,second,third,fourth,fifth} grader],[!{first,second,third,fourth,fifth} grade]}]'
