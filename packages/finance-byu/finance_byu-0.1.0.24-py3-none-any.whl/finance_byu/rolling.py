...
def roll_sum(input,win,minp,errors='raise'):
    ...
    if eflg:
        r = pd.Series(np.nan,columns=input.columns,index=input.index)
    else:
...
def roll_cov(x,y,win,minp,ddof=1,idx='x',errors='raise'):
    ...
    if eflg:
        ...
        r = pd.Series(_roll_cov(x.values,y.values,win,minp,ddof=ddof),myidx)
    return r
...
def roll_idio(y,x,win,minp,ddof=1,idx='x',errors='raise'):
    ...
    if eflg:
        ...
        r = pd.Series(_roll_idio(y.values,x.values,win,minp,ddof=ddof),myidx)
    return r
...
def roll_beta(y,x,win,minp,ddof=1,idx='x',errors='raise'):
    ...
    if eflg:
        ...
        r = pd.Series(_roll_beta(y.values,x.values,win,minp,ddof=ddof),myidx)
    return r
