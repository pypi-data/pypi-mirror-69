...
def DrawForces(o):
    ...
    """
    f = o.Forces
    nf = o.NetForce.components
    p = o.Position.components
...
def DrawTrajectory(o,t):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(p[0],p[1],p[2],c='#CC0033',marker='o',s=120,label="Object")
    ...
    for tt in range(1,t):
        ...
        tx.append(o.Future("position",tt).components[0]) 
        ty.append(o.Future("position",tt).components[1])
        tz.append(o.Future("position",tt).components[2])
    ax.scatter(tx,ty,tz,c=colors,s=[10*x**2+x for x in np.arange(1,t)],label='Trajectory')
    ax.set_xlabel('X axis')
    ax.set_ylabel('Y axis')
    ax.set_zlabel('Z axis')
    plt.legend()
