...
import psutil
from prometheus_client import generate_latest, REGISTRY, multiprocess, CollectorRegistry, Counter, Gauge
...
def gauge_gen(name, comment, fn):
    m = prometheus_client.Gauge(name, comment, ["generation", "process", "process_group"])
    for l in (0, 1, 2):
...
class ProcessStat:
    ...
    def mem(self):
        ...
        print(f"!!! INIT {psutil.virtual_memory()}")
        return psutil.virtual_memory().total
