...
def parse_arguments(argsv: str = None) -> argparse.Namespace:
    ...
    ggroup.add_argument(
        ...
        action="store",
        nargs="?",
        help="With data_submission sub-command, full path to html form. "
        "With process sub-command, override path to `.csv` file in config, "
        "listing newly delivered sequencing data from the genome center core.",
    )
    ...
    pgroup.add_argument(
        ...
        "file name instead of the default `jobs.yml`. When used with `run` "
        "the provided jobs filename will override the default `jobs.yml`.",
    )
    ...
    data_submission_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="switch to project intake form parsing",
    )
    ...
    uploadstats_parser = subparsers.add_parser(
        ...
        "This "
        "sub-command accepts one or more space separated positional arguments "
    ...
    run_parser = subparsers.add_parser(
        ...
        "independently, "
        "conditional on having a jobs.yml file in path defined by config.",
    )

    run_parser.add_argument(
        "tool_set",
        choices=["seqc", "scata", "sera", "sharp"],
        help="Choose between `seqc`, `scata`, `sera`, or `sharp` to nun SeqC, Cell "
        "Ranger, ATAC or HashTag using the defined jobs.yml/labels.json file in the "
        "{tool}/config. MUST PROVIDE PATH TO THE TOOL BEING USED TO PROCESS using "
        "`-tp`.",
    )
